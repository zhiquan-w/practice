...
void insert_student_info(MYSQL *connect, student *input) {
  ...
  char query[1024];
  sprintf(query,
          "insert into student_information (id,name,sex,academy,time) values "
          "(%d, \"%s\", \"%s\", \"%s\", \"%s\");",
          input->get_id(), input->get_name().c_str(), input->get_sex().c_str(),
          input->get_academy().c_str(), input->get_time().c_str());
...
void get_student_info_by_id(MYSQL *connect, student *input) {
  ...
  char query[256];
  int num_fileds;
  sprintf(query, "SELECT * FROM student_information where id = %d",
          input->get_id());
