 ...
 * value) and a flag (or reference on a flag) indicating whether the element
 ...
 * types) and reference type for containers with CT and CB being reference
 * types. In other words, it serves as a reference proxy.
 *
 */
...
class xoptional {
 public:
  ...
  // Constructors
  inline xoptional() : m_value(), m_flag(false) {}
  ...
  template <class T,
            std::enable_if_t<
                conjunction<
                    negation<std::is_same<xoptional<CT, CB>, std::decay_t<T>>>,
                    std::is_constructible<CT, T&&>,
                    negation<std::is_convertible<T&&, CT>>>::value,
                bool> = false>
      ...
      std::enable_if_t<
          conjunction<
              ...
              std::is_constructible<
                  CT, std::add_lvalue_reference_t<std::add_const_t<CTO>>>,
              std::is_constructible<
                  CB, std::add_lvalue_reference_t<std::add_const_t<CBO>>>,
              conjunction<
                  std::is_convertible<
                      std::add_lvalue_reference_t<std::add_const_t<CTO>>, CT>,
                  std::is_convertible<
                      std::add_lvalue_reference_t<std::add_const_t<CBO>>, CB>>,
              negation<detail::converts_from_xoptional<CT, CTO, CBO>>>::value,
          bool> = true>
  ...
  template <
      ...
      std::enable_if_t<
          conjunction<
              negation<std::is_same<xoptional<CT, CB>, xoptional<CTO, CBO>>>,
              std::is_constructible<
                  CT, std::conditional_t<std::is_reference<CT>::value,
                                         const std::decay_t<CTO>&,
                                         std::decay_t<CTO>&&>>,
              std::is_constructible<
                  CB, std::conditional_t<std::is_reference<CB>::value,
                                         const std::decay_t<CBO>&,
                                         std::decay_t<CBO>&&>>,
          ...
          conjunction<
              ...
              std::is_constructible<
                  CB, std::conditional_t<std::is_reference<CB>::value,
                                         const std::decay_t<CBO>&,
                                         std::decay_t<CBO>&&>>,
              disjunction<negation<std::is_convertible<
                              std::conditional_t<std::is_reference<CT>::value,
                                                 const std::decay_t<CTO>&,
                                                 std::decay_t<CTO>&&>,
                              CT>>,
  ...
  std::enable_if_t<
      conjunction<
          negation<std::is_same<xoptional<CT, CB>, std::decay_t<T>>>,
          std::is_assignable<std::add_lvalue_reference_t<CT>, T>>::value,
      xoptional&> inline
  operator=(T&& rhs) {
  ...
  std::enable_if_t<
      conjunction<
          ...
          negation<detail::assigns_from_xoptional<CT, CTO, CBO>>>::value,
      xoptional&> inline
  operator=(const xoptional<CTO, CBO>& rhs) {
    m_value = rhs.value();
  ...
  std::enable_if_t<
      conjunction<
          ...
          negation<detail::assigns_from_xoptional<CT, CTO, CBO>>>::value,
      xoptional&> inline
  operator=(xoptional<CTO, CBO>&& rhs) {
  ...
  std::conditional_t<std::is_reference<CT>::value, apply_cv_t<CT, value_type>&,
                     value_type>
  value() && noexcept;
  std::conditional_t<std::is_reference<CT>::value, const value_type&,
                     value_type>
  ...
  std::conditional_t<std::is_reference<CB>::value, apply_cv_t<CB, flag_type>&,
                     flag_type>
  has_value() && noexcept;
...
template <class CT, class CB>
auto xoptional<CT, CB>::value() const& noexcept
    -> std::add_lvalue_reference_t<std::add_const_t<CT>> {
  return m_value;
}

template <class CT, class CB>
auto xoptional<CT, CB>::value() && noexcept
    -> std::conditional_t<std::is_reference<CT>::value,
                          apply_cv_t<CT, value_type>&, value_type> {
  return m_value;
}
...
template <class CT, class CB>
template <class U>
auto xoptional<CT, CB>::value_or(U&& default_value) const& noexcept
    -> value_type {
...
// Access
template <class CT, class CB>
auto xoptional<CT, CB>::has_value() & noexcept
    -> std::add_lvalue_reference_t<CB> {
  return m_flag;
}

template <class CT, class CB>
auto xoptional<CT, CB>::has_value() const& noexcept
    -> std::add_lvalue_reference_t<std::add_const_t<CB>> {
  return m_flag;
}
...
auto xoptional<CT, CB>::has_value() && noexcept
    -> std::conditional_t<std::is_reference<CB>::value,
                          apply_cv_t<CB, flag_type>&, flag_type> {
  return m_flag;
}

template <class CT, class CB>
auto xoptional<CT, CB>::has_value() const&& noexcept
    -> std::conditional_t<std::is_reference<CB>::value, const flag_type&,
                          flag_type> {
  return m_flag;
}
...
template <class CT, class CB>
inline auto xoptional<CT, CB>::operator&()
...
inline std::basic_ostream<OC, OT>& operator<<(std::basic_ostream<OC, OT>& out,
                                              const xoptional<T, B>& v) {
  if (v.has_value()) {
    out << v.value();
  } else {
    out << "N/A";
...
#define TERNARY_OPTIONAL_1(NAME)                                              \
  template <class T1, class B1, class T2, class T3,                           \
            XTL_REQUIRES(is_not_xoptional_nor_xmasked_value<T2>,              \
                         is_not_xoptional_nor_xmasked_value<T3>)>             \
...
#define TERNARY_OPTIONAL_3(NAME)                                              \
  template <class T1, class T2, class T3, class B3,                           \
            XTL_REQUIRES(is_not_xoptional_nor_xmasked_value<T1>,              \
                         is_not_xoptional_nor_xmasked_value<T2>)>             \
...
#define TERNARY_OPTIONAL_12(NAME)                                             \
  template <class T1, class B1, class T2, class B2, class T3,                 \
            XTL_REQUIRES(is_not_xoptional_nor_xmasked_value<T3>)>             \
  inline auto NAME(const xoptional<T1, B1>& e1, const xoptional<T2, B2>& e2,  \
                   const T3& e3)                                              \
      ->common_optional_t<T1, T2, T3> {                                       \
    using std::NAME;                                                          \
...
template <class B, class T1, class T2,
          XTL_REQUIRES(at_least_one_xoptional<B, T1, T2>)>
inline common_optional_t<T1, T2> select(const B& cond, const T1& v1,
                                        const T2& v2) noexcept {
