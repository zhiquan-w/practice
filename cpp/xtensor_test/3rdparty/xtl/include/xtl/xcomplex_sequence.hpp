...
class xcomplex_sequence {
 public:
  ...
  using const_iterator =
      xcomplex_iterator<typename C::const_iterator, ieee_compliant>;
  using reverse_iterator =
      xcomplex_iterator<typename C::reverse_iterator, ieee_compliant>;
  using const_reverse_iterator =
      xcomplex_iterator<typename C::const_reverse_iterator, ieee_compliant>;
...
bool operator==(const xcomplex_sequence<C, B>& lhs,
                const xcomplex_sequence<C, B>& rhs);
...
class xcomplex_array
    : public xcomplex_sequence<std::array<T, N>, ieee_compliant> {
 public:
...
class xcomplex_vector
    : public xcomplex_sequence<std::vector<T, A>, ieee_compliant> {
 public:
...
struct xcomplex_iterator_traits {
  ...
  using value_type = xcomplex<typename IT::value_type, typename IT::value_type,
                              ieee_compliant>;
  using reference =
      xcomplex<typename IT::reference, typename IT::reference, ieee_compliant>;
  using pointer = xclosure_pointer<reference>;
...
class xcomplex_iterator
    : public xrandom_access_iterator_base2<xcomplex_iterator_traits<IT, B>> {
 public:
  using self_type = xcomplex_iterator<IT, B>;
  using base_type =
      xrandom_access_iterator_base2<xcomplex_iterator_traits<IT, B>>;
...
inline xcomplex_sequence<C, B>::xcomplex_sequence(size_type s,
                                                  const xcomplex<TR, TC, B2>& v)
    : m_real(make_sequence<container_type>(s, v.real())),
      m_imag(make_sequence<container_type>(s, v.imag())) {}

template <class C, bool B>
inline xcomplex_sequence<C, B>::xcomplex_sequence(
    std::initializer_list<value_type> init)
    : m_real(make_sequence<container_type>(init.size())),
      m_imag(make_sequence<container_type>(init.size())) {
  std::transform(init.begin(), init.end(), m_real.begin(),
                 [](const auto& v) { return v.real(); });
  std::transform(init.begin(), init.end(), m_imag.begin(),
                 [](const auto& v) { return v.imag(); });
}
...
template <class C, bool B>
inline auto xcomplex_sequence<C, B>::operator[](size_type i) const
    -> const_reference {
...
template <class C, bool B>
inline auto xcomplex_sequence<C, B>::rbegin() const noexcept
    -> const_reverse_iterator {
  return crbegin();
}

template <class C, bool B>
inline auto xcomplex_sequence<C, B>::rend() const noexcept
    -> const_reverse_iterator {
  return crend();
}

template <class C, bool B>
inline auto xcomplex_sequence<C, B>::crbegin() const noexcept
    -> const_reverse_iterator {
  return const_reverse_iterator(m_real.crbegin(), m_imag.crbegin());
}

template <class C, bool B>
inline auto xcomplex_sequence<C, B>::crend() const noexcept
    -> const_reverse_iterator {
...
template <class C, bool B>
inline auto xcomplex_sequence<C, B>::real() const& noexcept
...
template <class C, bool B>
inline auto xcomplex_sequence<C, B>::imag() const& noexcept
    -> const container_type& {
  return m_imag;
}

template <class C, bool B>
inline bool operator==(const xcomplex_sequence<C, B>& lhs,
                       const xcomplex_sequence<C, B>& rhs) {
...
inline xcomplex_array<T, N, B>::xcomplex_array(size_type s) : base_type(s) {}
...
inline xcomplex_vector<T, B, A>::xcomplex_vector(size_type s) : base_type(s) {}
...
template <class T, bool B, class A>
inline xcomplex_vector<T, B, A>::xcomplex_vector(
    std::initializer_list<value_type> init)
    : base_type(init) {}
...
template <class T, bool B, class A>
template <class TR, class TI, bool B2>
inline void xcomplex_vector<T, B, A>::resize(size_type s,
                                             const xcomplex<TR, TI, B2>& v) {
  this->m_real.resize(s, v.real());
  this->m_imag.resize(s, v.imag());
}
...
template <class IT, bool B>
inline auto xcomplex_iterator<IT, B>::operator+=(difference_type n)
    -> self_type& {
  m_it_real += n;
  m_it_imag += n;
  return *this;
...
template <class IT, bool B>
inline auto xcomplex_iterator<IT, B>::operator-(const self_type& rhs) const
    -> difference_type {
