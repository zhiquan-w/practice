...
class xdynamic_bitset_base {
 public:
  ...
  using temporary_type =
      xdynamic_bitset<block_type, std::allocator<block_type>>;

  using allocator_type =
      typename container_internals<storage_type>::allocator_type;
...
template <class X>
inline xdynamic_bitset_view<X>::xdynamic_bitset_view(block_type* ptr,
                                                     std::size_t size)
    : base_class(storage_type(ptr, detail_bitset::integer_ceil(
                                       size, base_class::s_bits_per_block)),
                 size) {
...
auto operator&(const xdynamic_bitset_base<L>& lhs,
               const xdynamic_bitset_base<R>& rhs);
...
template <class B>
void swap(const xdynamic_bitset_base<B>& lhs,
          const xdynamic_bitset_base<B>& rhs);
...
class xbitset_reference {
 public:
  ...
  using pointer =
      std::conditional_t<is_const, const xclosure_pointer<const self_type>,
 ...
 private:
  using block_type = typename xdynamic_bitset_traits<B>::block_type;
  using closure_type =
      std::conditional_t<is_const, const block_type&, block_type&>;
...
template <class B, bool is_const>
class xbitset_iterator
    : public xrandom_access_iterator_base<
          xbitset_iterator<B, is_const>,
 ...
 public:
  ...
  using base_type =
      xrandom_access_iterator_base<self_type, value_type, difference_type,
                                   pointer, reference>;
...
class xdynamic_bitset
 ...
 public:
  ...
  xdynamic_bitset(size_type count, bool b,
                  const allocator_type& alloc = allocator_type());
  explicit xdynamic_bitset(size_type count,
                           const allocator_type& alloc = allocator_type());
  xdynamic_bitset(std::initializer_list<bool> init,
                  const allocator_type& alloc = allocator_type());

  template <class BlockInputIt>
  xdynamic_bitset(BlockInputIt first, BlockInputIt last,
                  const allocator_type& alloc = allocator_type());
...
template <class B, class A>
inline xdynamic_bitset<B, A>::xdynamic_bitset(size_type count, bool b,
                                              const allocator_type& alloc)
    : base_type(storage_type(this->compute_block_count(count),
                             b ? ~block_type(0) : block_type(0), alloc),
                count) {
  this->zero_unused_bits();
}

template <class B, class A>
inline xdynamic_bitset<B, A>::xdynamic_bitset(size_type count,
                                              const allocator_type& alloc)
    : base_type(
          storage_type(this->compute_block_count(count), block_type(0), alloc),
          count) {}

template <class B, class A>
template <class BlockInputIt>
inline xdynamic_bitset<B, A>::xdynamic_bitset(BlockInputIt first,
                                              BlockInputIt last,
                                              const allocator_type& alloc)
    : base_type(storage_type(first, last, alloc),
                size_type(std::distance(first, last)) *
                    base_type::s_bits_per_block) {}

template <class B, class A>
inline xdynamic_bitset<B, A>::xdynamic_bitset(std::initializer_list<bool> init,
                                              const allocator_type& alloc)
...
template <class Y>
inline xdynamic_bitset<B, A>::xdynamic_bitset(
    const xdynamic_bitset_base<Y>& rhs)
    : base_type(storage_type(rhs.block_begin(), rhs.block_end()), rhs.size()) {}
...
template <class B, class A>
template <class BlockInputIt>
inline void xdynamic_bitset<B, A>::assign(BlockInputIt first,
                                          BlockInputIt last) {
...
inline void xdynamic_bitset<B, A>::pop_back() {
  ...
  size_type new_block_count =
      base_type::compute_block_count(base_type::m_size - 1);
...
template <class B>
inline auto xdynamic_bitset_base<B>::operator[](size_type i) const
    -> const_reference {
...
template <class B>
inline auto xdynamic_bitset_base<B>::rbegin() const noexcept
    -> const_reverse_iterator {
  return crbegin();
}

template <class B>
inline auto xdynamic_bitset_base<B>::rend() const noexcept
    -> const_reverse_iterator {
  return crend();
}

template <class B>
inline auto xdynamic_bitset_base<B>::crbegin() const noexcept
    -> const_reverse_iterator {
  return const_reverse_iterator(cend());
}

template <class B>
inline auto xdynamic_bitset_base<B>::crend() const noexcept
    -> const_reverse_iterator {
  return const_reverse_iterator(cbegin());
}

template <class B>
inline auto xdynamic_bitset_base<B>::block_begin() const noexcept
    -> const_block_iterator {
  return m_buffer.begin();
}

template <class B>
inline auto xdynamic_bitset_base<B>::block_end() const noexcept
    -> const_block_iterator {
  return m_buffer.end();
}
...
template <class B>
template <class R>
inline auto xdynamic_bitset_base<B>::operator&=(
    const xdynamic_bitset_base<R>& rhs) -> self_type& {
...
template <class B>
inline auto xdynamic_bitset_base<B>::operator<<(size_type pos)
    -> temporary_type {
...
inline auto xdynamic_bitset_base<B>::operator>>=(size_type pos) -> self_type& {
  ...
  if (pos > 0) {
    ...
    std::fill_n(
        m_buffer.begin() + static_cast<std::ptrdiff_t>(block_count() - div),
        div, block_type(0));
  }
  return *this;
}
...
template <class B>
inline auto xdynamic_bitset_base<B>::set(size_type pos, value_type value)
    -> self_type& {
  if (value) {
...
inline bool xdynamic_bitset_base<B>::all() const noexcept {
  ...
  for (size_type i = 0; i < size; ++i) {
    if (m_buffer[i] != all_ones) return false;
  }

  if (extra_bits != 0) {
    block_type mask = ~(~block_type(0) << extra_bits);
    if (m_buffer.back() != mask) return false;
  }
...
inline auto xdynamic_bitset_base<B>::count() const noexcept -> size_type {
  ...
  const unsigned char* p =
      static_cast<const unsigned char*>(static_cast<const void*>(&m_buffer[0]));
...
template <class B>
inline auto xdynamic_bitset_base<B>::data() const noexcept
    -> const block_type* {
  return m_buffer.data();
}
...
template <class B>
template <class Y>
inline bool xdynamic_bitset_base<B>::operator==(
    const xdynamic_bitset_base<Y>& rhs) const noexcept {
...
template <class B>
inline auto xdynamic_bitset_base<B>::derived_cast() const
    -> const derived_class& {
  return *(reinterpret_cast<const derived_class*>(this));
}

template <class B>
inline xdynamic_bitset_base<B>::xdynamic_bitset_base(const storage_type& buffer,
                                                     std::size_t size)
    : m_size(size), m_buffer(buffer) {}

template <class B>
inline auto xdynamic_bitset_base<B>::compute_block_count(
    size_type bits_count) const noexcept -> size_type {
  return bits_count / s_bits_per_block +
         static_cast<size_type>(bits_count % s_bits_per_block != 0);
}

template <class B>
inline auto xdynamic_bitset_base<B>::block_index(size_type pos) const noexcept
    -> size_type {
  return pos / s_bits_per_block;
}

template <class B>
inline auto xdynamic_bitset_base<B>::bit_index(size_type pos) const noexcept
    -> size_type {
  return pos % s_bits_per_block;
}

template <class B>
inline auto xdynamic_bitset_base<B>::bit_mask(size_type pos) const noexcept
    -> block_type {
  return block_type(1) << bit_index(pos);
}

template <class B>
inline auto xdynamic_bitset_base<B>::count_extra_bits() const noexcept
    -> size_type {
...
template <class L, class R>
inline auto operator&(const xdynamic_bitset_base<L>& lhs,
                      const xdynamic_bitset_base<R>& rhs) {
...
template <class B>
inline void swap(const xdynamic_bitset_base<B>& lhs,
                 const xdynamic_bitset_base<B>& rhs) {
  return lhs.swap(rhs);
}
...
inline xbitset_reference<B, C>::xbitset_reference(closure_type block,
                                                  block_type pos)
...
template <class B, bool C>
inline auto xbitset_reference<B, C>::operator=(const self_type& rhs) noexcept
    -> self_type& {
  assign(rhs);
  return *this;
}
...
template <class B, bool C>
inline auto xbitset_reference<B, C>::operator=(self_type&& rhs) noexcept
    -> self_type& {
  assign(rhs);
  return *this;
}
...
template <class B, bool C>
inline auto xbitset_reference<B, C>::operator=(bool rhs) noexcept
    -> self_type& {
  assign(rhs);
  return *this;
}
...
template <class B, bool C>
inline xbitset_iterator<B, C>::xbitset_iterator(container_reference c,
...
template <class B, bool C>
inline auto xbitset_iterator<B, C>::operator+=(difference_type n)
    -> self_type& {
...
template <class B, bool C>
inline auto xbitset_iterator<B, C>::operator-(const self_type& rhs) const
    -> difference_type {
