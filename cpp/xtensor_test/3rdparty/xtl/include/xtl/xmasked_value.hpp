...
class xmasked_value {
 public:
  ...
  std::conditional_t<std::is_reference<T>::value,
                     apply_cv_t<T, std::decay_t<T>>&, std::decay_t<T>>
  value() && noexcept;
  std::conditional_t<std::is_reference<T>::value, const std::decay_t<T>&,
                     std::decay_t<T>>
  value() const&& noexcept;

  std::add_lvalue_reference_t<B> visible() & noexcept;
  std::add_lvalue_reference_t<std::add_const_t<B>> visible() const& noexcept;
  std::conditional_t<std::is_reference<B>::value,
                     apply_cv_t<B, std::decay_t<B>>&, std::decay_t<B>>
  visible() && noexcept;
  std::conditional_t<std::is_reference<B>::value, const std::decay_t<B>&,
                     std::decay_t<B>>
...
template <class T, class B>
inline auto xmasked_value<T, B>::value() & noexcept
    -> std::add_lvalue_reference_t<T> {
  return m_value;
}

template <class T, class B>
inline auto xmasked_value<T, B>::value() const& noexcept
...
template <class T, class B>
inline auto xmasked_value<T, B>::visible() & noexcept
    -> std::add_lvalue_reference_t<B> {
  return m_visible;
}

template <class T, class B>
inline auto xmasked_value<T, B>::visible() const& noexcept
    -> std::add_lvalue_reference_t<std::add_const_t<B>> {
  return m_visible;
}
...
template <class T, class B>
template <class T1, class B1>
inline bool xmasked_value<T, B>::equal(
    const xmasked_value<T1, B1>& rhs) const noexcept {
  return (!m_visible && !rhs.visible()) ||
         (m_value == rhs.value() && (m_visible && rhs.visible()));
}
...
inline bool operator==(const xmasked_value<T1, B1>& lhs,
                       const xmasked_value<T2, B2>& rhs) noexcept {
  return lhs.equal(rhs);
}

template <class T1, class T2, class B2,
          XTL_REQUIRES(negation<is_xmasked_value<T1>>)>
...
template <class T1, class B1, class T2,
          XTL_REQUIRES(negation<is_xmasked_value<T2>>)>
inline bool operator==(const xmasked_value<T1, B1>& lhs,
...
inline auto operator+(const xmasked_value<T, B>& e) noexcept
  ...
  return xmasked_value<std::decay_t<T>, std::decay_t<B>>(e.value(),
                                                         e.visible());
}
...
template <class T, class B>
inline auto operator!(const xmasked_value<T, B>& e) noexcept
    -> xmasked_value<decltype(!e.value())> {
...
#define DEFINE_OPERATOR(OP)                                                 \
  template <class T1, class B1, class T2, class B2>                         \
  inline auto operator OP(const xmasked_value<T1, B1>& e1,                  \
                          const xmasked_value<T2, B2>& e2) noexcept         \
  ...
  template <class T1, class B1, class T2,                                   \
            XTL_REQUIRES(negation<is_xmasked_value<T2>>)>                   \
  inline auto operator OP(const xmasked_value<T1, B1>& e1,                  \
                          const T2& e2) noexcept                            \
      ->xmasked_value<promote_type_t<std::decay_t<T1>, std::decay_t<T2>>> { \
    using value_type = promote_type_t<std::decay_t<T1>, std::decay_t<T2>>;  \
    return e1.visible() ? masked_value(e1.value() OP e2)                    \
                        : masked<value_type>();                             \
  }                                                                         \
                                                                            \
  template <class T1, class T2, class B2,                                   \
            XTL_REQUIRES(negation<is_xmasked_value<T1>>)>                   \
  inline auto operator OP(const T1& e1,                                     \
                          const xmasked_value<T2, B2>& e2) noexcept         \
      ->xmasked_value<promote_type_t<std::decay_t<T1>, std::decay_t<T2>>> { \
    using value_type = promote_type_t<std::decay_t<T1>, std::decay_t<T2>>;  \
    return e2.visible() ? masked_value(e1 OP e2.value())                    \
                        : masked<value_type>();                             \
  }
...
#define DEFINE_BOOL_OPERATOR(OP)                                          \
  ...
  inline auto operator OP(                                                \
      const xmasked_value<T1, B1>& e1,                                    \
      const T2& e2) noexcept->xmasked_value<decltype(e1.value() OP e2)> { \
    return e1.visible() ? masked_value(e1.value() OP e2)                  \
                        : masked<decltype(e1.value() OP e2)>();           \
  }                                                                       \
                                                                          \
  template <class T1, class T2, class B2,                                 \
            XTL_REQUIRES(negation<is_xmasked_value<T1>>)>                 \
  inline auto operator OP(const T1& e1,                                   \
    ...
    return e2.visible() ? masked_value(e1 OP e2.value())                  \
                        : masked<decltype(e1 OP e2.value())>();           \
  }
...
#define DEFINE_UNARY_OPERATOR(OP)                                          \
  ...
  inline xmasked_value<std::decay_t<T>> OP(const xmasked_value<T, B>& e) { \
    using std::OP;                                                         \
    return e.visible() ? masked_value(OP(e.value()))                       \
                       : masked<std::decay_t<T>>();                        \
  }
...
#define DEFINE_UNARY_BOOL_OPERATOR(OP)                      \
  ...
  inline auto OP(const xmasked_value<T, B>& e) {            \
    using std::OP;                                          \
    return e.visible() ? masked_value(OP(e.value()))        \
                       : masked<decltype(OP(e.value()))>(); \
  }

#define DEFINE_BINARY_OPERATOR(OP)                                \
  template <class T1, class B1, class T2, class B2>               \
  inline auto OP(const xmasked_value<T1, B1>& e1,                 \
                 const xmasked_value<T2, B2>& e2) {               \
    using std::OP;                                                \
    return e1.visible() && e2.visible()                           \
  ...
  inline auto OP(const xmasked_value<T1, B1>& e1, const T2& e2) { \
    using std::OP;                                                \
    return e1.visible() ? masked_value(OP(e1.value(), e2))        \
                        : masked<decltype(OP(e1.value(), e2))>(); \
  }                                                               \
                                                                  \
  template <class T1, class T2, class B2>                         \
  inline auto OP(const T1& e1, const xmasked_value<T2, B2>& e2) { \
    using std::OP;                                                \
    return e2.visible() ? masked_value(OP(e1, e2.value()))        \
                        : masked<decltype(OP(e1, e2.value()))>(); \
  }
...
#define DEFINE_TERNARY_OPERATOR_MMT(OP)                              \
  ...
  inline auto OP(const xmasked_value<T1, B1>& e1,                    \
                 const xmasked_value<T2, B2>& e2, const T3& e3) {    \
    using std::OP;                                                   \
    return (e1.visible() && e2.visible())                            \
...
#define DEFINE_TERNARY_OPERATOR_TTM(OP)                               \
  ...
  inline auto OP(const T1& e1, const T2& e2,                          \
                 const xmasked_value<T3, B3>& e3) {                   \
    using std::OP;                                                    \
    return e3.visible() ? masked_value(OP(e1, e2, e3.value()))        \
...
#define DEFINE_TERNARY_OPERATOR_TMT(OP)                               \
  ...
  inline auto OP(const T1& e1, const xmasked_value<T2, B2>& e2,       \
                 const T3& e3) {                                      \
    using std::OP;                                                    \
    return e2.visible() ? masked_value(OP(e1, e2.value(), e3))        \
...
#define DEFINE_TERNARY_OPERATOR_MTT(OP)                               \
  ...
  inline auto OP(const xmasked_value<T1, B1>& e1, const T2& e2,       \
                 const T3& e3) {                                      \
    using std::OP;                                                    \
    return e1.visible() ? masked_value(OP(e1.value(), e2, e3))        \
