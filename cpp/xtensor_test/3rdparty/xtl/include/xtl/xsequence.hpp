...
struct sequence_builder {
  ...
  inline static S make(size_type size) { return S(size); }
...
struct sequence_forwarder_impl<
    R, A,
    void_t<decltype(std::declval<R>().resize(std::declval<std::size_t>()))>> {
  template <class T>
...
template <class R, class A>
struct sequence_forwarder : sequence_forwarder_impl<R, A> {};
...
template <class R, class A>
using forwarder_type =
    detail::sequence_forwarder<std::decay_t<R>,
                               std::remove_cv_t<std::remove_reference_t<A>>>;
}  // namespace detail

template <class R, class A>
inline decltype(auto) forward_sequence(
    typename std::remove_reference<A>::type& s) {
