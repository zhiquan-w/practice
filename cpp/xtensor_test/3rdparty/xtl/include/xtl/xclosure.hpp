...
struct closure_type {
  ...
  using type =
      typename std::conditional<std::is_lvalue_reference<S>::value,
...
class xclosure_wrapper {
 public:
  ...
  std::add_lvalue_reference_t<std::add_const_t<closure_type>> get()
      const& noexcept;
 ...
 private:
  ...
  template <class T>
  std::enable_if_t<std::is_lvalue_reference<CT>::value,
  ...
  template <class T>
  std::enable_if_t<std::is_lvalue_reference<CT>::value, T> get_pointer(
      T val) const;
  ...
  std::enable_if_t<std::is_lvalue_reference<CT>::value, T> get_storage_init(
      CTA&& e) const;
...
template <class CT>
inline auto xclosure_wrapper<CT>::operator=(const self_type& rhs)
    -> self_type& {
...
template <class CT>
inline xclosure_wrapper<CT>::operator typename xclosure_wrapper<
    CT>::closure_type() noexcept {
  return deref(m_wrappee);
}

template <class CT>
inline xclosure_wrapper<CT>::operator typename xclosure_wrapper<
    CT>::const_closure_type() const noexcept {
  return deref(m_wrappee);
}

template <class CT>
inline auto xclosure_wrapper<CT>::get() & noexcept
    -> std::add_lvalue_reference_t<closure_type> {
  return deref(m_wrappee);
}

template <class CT>
inline auto xclosure_wrapper<CT>::get() const& noexcept
    -> std::add_lvalue_reference_t<std::add_const_t<closure_type>> {
  return deref(m_wrappee);
}
...
template <class CT>
template <class T>
inline std::enable_if_t<std::is_lvalue_reference<CT>::value,
                        std::add_lvalue_reference_t<std::remove_pointer_t<T>>>
...
template <class CT>
inline bool operator==(const xclosure_wrapper<CT>& lhs,
                       const xclosure_wrapper<CT>& rhs) {
...
template <class CT>
inline xclosure_pointer<CT>::xclosure_pointer(reference e) : m_wrappee(e) {}
...
template <class CT>
inline auto xclosure_pointer<CT>::operator*() const noexcept
    -> const_reference {
  return m_wrappee;
}
