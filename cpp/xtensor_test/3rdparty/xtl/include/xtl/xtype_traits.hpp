...
struct promote_type<std::chrono::time_point<C, D1>,
                    std::chrono::time_point<C, D2>> {
  using type = std::chrono::time_point<C, typename promote_type<D1, D2>::type>;
};

template <class T0, class T1>
struct promote_type<T0, T1> {
  using type = decltype(std::declval<std::decay_t<T0>>() +
                        std::declval<std::decay_t<T1>>());
};

template <class T0, class... REST>
struct promote_type<T0, REST...> {
  using type = decltype(std::declval<std::decay_t<T0>>() +
                        std::declval<typename promote_type<REST...>::type>());
};
...
template <class T1, class T2>
struct promote_type<std::complex<T1>, T2> : promote_type<T2, std::complex<T1>> {
};
...
struct promote_type<bool, REST...> {
  using type =
      typename promote_type<bool, typename promote_type<REST...>::type>::type;
};
 ...
 *
 * For example, <tt>big_promote_type<unsigned char>::type</tt> is <tt>unsigned
 * long long</tt>. The default implementation only supports built-in types and
 * <tt>std::complex</tt>. All other types remain unchanged unless
 * <tt>big_promote_type</tt> gets specialized for them.
 */
template <class T>
...
struct bool_promote_type {
  using type =
      typename std::conditional<std::is_same<T, bool>::value, uint8_t, T>::type;
};
...
struct disjunction<Arg1, Arg2, Args...>
    : std::conditional_t<Arg1::value, Arg1, disjunction<Arg2, Args...>> {};
...
template <class Arg1, class Arg2, class... Args>
struct conjunction<Arg1, Arg2, Args...>
    : std::conditional_t<Arg1::value, conjunction<Arg2, Args...>, Arg1> {};
...
template <class... C>
using check_disallow =
    std::enable_if_t<xtl::negation<xtl::conjunction<C...>>::value, int>;

template <class... C>
using check_disallow_one =
    std::enable_if_t<xtl::negation<xtl::disjunction<C...>>::value, int>;
...
// Adds const to the underlying type of a reference or pointer, or to the type
// itself if it's not a reference nor a pointer
