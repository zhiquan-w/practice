...
using converts_from_xoptional =
    disjunction<std::is_constructible<CT, const xoptional<CTO, CBO>&>,
...
using assigns_from_xoptional = disjunction<
    std::is_assignable<std::add_lvalue_reference_t<CT>,
                       const xoptional<CTO, CBO>&>,
...
struct common_optional_impl<T1, T2> {
  ...
  using type1 =
      xtl::mpl::eval_if_t<xtl::is_fundamental<decay_t1>, identity<decay_t1>,
                          get_value_type<decay_t1>>;
  using type2 =
      xtl::mpl::eval_if_t<xtl::is_fundamental<decay_t2>, identity<decay_t2>,
                          get_value_type<decay_t2>>;
...
struct common_optional_impl<T1, T2, Args...> {
  using type =
      typename common_optional_impl<typename common_optional_impl<T1, T2>::type,
                                    Args...>::type;
};
}  // namespace detail
...
template <class E>
struct is_not_xoptional_nor_xmasked_value
    : negation<disjunction<is_xoptional<E>, is_xmasked_value<E>>> {};
}  // namespace xtl
