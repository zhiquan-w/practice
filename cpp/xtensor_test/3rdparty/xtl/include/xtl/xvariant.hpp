...
struct xgetter<const T&> {
  ...
  static constexpr const T& get(const xtl::variant<Ts...>&& v) {
    using cl_type = xtl::xclosure_wrapper<const T&>;
    return get_impl(std::move(v),
                    xtl::mpl::contains<xtl::mpl::vector<Ts...>, cl_type>());
  }
 ...
 private:
  template <class... Ts>
  static constexpr const T& get_impl(const xtl::variant<Ts...>& v,
                                     xtl::mpl::bool_<true>) {
  ...
  static constexpr const T& get_impl(const xtl::variant<Ts...>& v,
                                     xtl::mpl::bool_<false>) {
    return static_cast<const xtl::xclosure_wrapper<T&>&>(
               xtl::get<xtl::xclosure_wrapper<T&>>(v))
        .get();
  }
  ...
  static constexpr const T& get_impl(const xtl::variant<Ts...>&& v,
                                     xtl::mpl::bool_<false>) {
    return static_cast<const xtl::xclosure_wrapper<T&>&&>(
               xtl::get<xtl::xclosure_wrapper<T&>>(std::move(v)))
        .get();
  }
};
}  // namespace detail
...
// This hierarchy is required since ellipsis in using declarations are not
...
struct overloaded<T1, T2, Ts...> : T1, overloaded<T2, Ts...> {
  template <class... Us>
  overloaded(T1 t1, T2 t2, Us... args)
      : T1(t1), overloaded<T2, Ts...>(t2, args...) {}
