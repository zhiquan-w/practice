...
template <class executor, class base_lhs, class lhs_type_list,
          class return_type = void, class symmetric = antisymmetric_dispatch,
          class base_rhs = base_lhs, class rhs_type_list = lhs_type_list>
class static_dispatcher {
 private:
  template <class lhs_type, class rhs_type>
  static return_type invoke_executor(lhs_type& lhs, rhs_type& rhs,
                                     executor& exec, std::false_type) {
    return exec.run(lhs, rhs);
  }

  template <class lhs_type, class rhs_type>
  static return_type invoke_executor(lhs_type& lhs, rhs_type& rhs,
                                     executor& exec, std::true_type) {
    return exec.run(rhs, lhs);
  }

  template <class lhs_type>
  static return_type dispatch_rhs(lhs_type& lhs, base_rhs& rhs, executor& exec,
                                  mpl::vector<>) {
  ...
  static return_type dispatch_rhs(lhs_type& lhs, base_rhs& rhs, executor& exec,
    ...
    if (T* p = dynamic_cast<T*>(&rhs)) {
      constexpr std::size_t lhs_index =
          mpl::index_of<lhs_type_list, lhs_type>::value;
      constexpr std::size_t rhs_index = mpl::index_of<rhs_type_list, T>::value;

      using invoke_flag = std::integral_constant<
          bool, std::is_same<symmetric, symmetric_dispatch>::value &&
                    (rhs_index < lhs_index)>;
  ...
  static return_type dispatch_lhs(base_lhs& lhs, base_rhs& rhs, executor& exec,
                                  mpl::vector<>) {
    return exec.on_error(lhs, rhs);
  }
...
template <class type_list, class return_type, class undispatched_type_list,
          class callback_type>
class basic_dispatcher;

template <class return_type, class callback_type, class... B, class... T>
class basic_dispatcher<mpl::vector<B...>, return_type, mpl::vector<T...>,
                       callback_type> {
 private:
...
class recursive_container : public recursive_container_impl<
                                recursive_container<callback_type, level - 1>> {
};
...
template <class return_type, class callback_type, class... B, class... T>
class basic_fast_dispatcher<mpl::vector<B...>, return_type, mpl::vector<T...>,
                            callback_type> {
 private:
  static constexpr std::size_t nb_args = sizeof...(B);

  using storage_type =
      detail::recursive_container<callback_type, sizeof...(B) - 1>;
  using index_type = std::array<std::size_t, nb_args>;
  using index_ref_type =
      std::array<std::reference_wrapper<std::size_t>, nb_args>;
  ...
  template <std::size_t I, class C>
  std::enable_if_t<I + 1 == nb_args> insert_impl(callback_type&& cb, C& c,
                                                 const index_ref_type& index) {
  ...
  template <std::size_t I, class C>
  std::enable_if_t<I + 1 == nb_args, return_type> dispatch_impl(
      C& c, const index_type& index, B&... args, T&... udargs) const {
 ...
 public:
  inline basic_fast_dispatcher() : m_next_index(0) {}
...
struct static_caster {
  static T& cast(F& f) { return static_cast<T&>(f); }
};

template <class T, class F>
struct dynamic_caster {
  static T& cast(F& f) { return dynamic_cast<T&>(f); }
};
...
template <
    class type_list, class return_type, class undispatched_type = mpl::vector<>,
...
template <class return_type, template <class, class> class casting_policy,
          template <class, class, class, class> class dispatcher, class... B,
          class... T>
class functor_dispatcher<mpl::vector<B...>, return_type, mpl::vector<T...>,
                         casting_policy, dispatcher> {
 private:
  using functor_type = std::function<return_type(B&..., T&...)>;
  using backend = dispatcher<mpl::vector<B...>, return_type, mpl::vector<T...>,
                             functor_type>;
  backend m_backend;
