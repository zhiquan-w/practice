...
class span {
 ...
 public:
  ...
  template <
      typename Container,
      typename std::enable_if<detail::is_container<Container>::value &&
                                  detail::is_container_element_type_compatible<
  ...
  TCB_SPAN_CONSTEXPR11 subspan_return_t<Offset, Count> subspan() const {
    ...
    return {data() + Offset, Count != dynamic_extent ? Count
...
template <typename T, std::size_t N>
TCB_SPAN_ARRAY_CONSTEXPR span<T, AS_SIGNED(N)> make_span(
    std::array<T, N>& arr) noexcept {
  return {arr};
}

template <typename T, std::size_t N>
TCB_SPAN_ARRAY_CONSTEXPR span<const T, AS_SIGNED(N)> make_span(
...
class tuple_size<tcb::span<E, S>>
    : public integral_constant<size_t, static_cast<size_t>(S)> {};
