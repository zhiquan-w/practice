...
struct norm_of_vector_impl {
  ...
  using squared_norm_type =
      typename norm_of_array_elements_impl<T>::squared_norm_type;
};
 ...
 *   - @tparam T is an arithmetic type: 'type' is the result type of
 * <tt>abs(t)</tt>.
 *   - @tparam T is a container of 'long double' elements: 'type' is <tt>long
 * double</tt>.
 *   - @tparam T is a container of another arithmetic type: 'type' is
 * <tt>double</tt>.
 *   - @tparam T is a container of some other type: 'type' is the element's norm
 * type,
 *
 ...
 *   - @tparam T is a container of 'long double' elements: 'type' is <tt>long
 * double</tt>.
 *   - @tparam T is a container of another floating-point type: 'type' is
 * <tt>double</tt>.
 *   - @tparam T is a container of integer elements: 'type' is
 * <tt>uint64_t</tt>.
 *   - @tparam T is a container of some other type: 'type' is the element's
 * squared norm type,
 *
 *  Containers are recognized by having an embedded typedef 'value_type'.
 *  To change the behavior for a case not covered here, specialize the
...
#define XTENSOR_DEFINE_UNSIGNED_NORMS(T)                                       \
  inline T norm_lp(T t, double p) noexcept { return p == 0.0 ? (t != 0) : t; } \
  inline auto norm_lp_to_p(T t, double p) noexcept {                           \
    using rt = xtl::real_promote_type_t<T>;                                    \
...
#define XTENSOR_NORM_FUNCTION(NAME, RESULT_TYPE, REDUCE_EXPR, REDUCE_OP, \
                              MERGE_FUNC)                                \
...
XTENSOR_NORM_FUNCTION(norm_l1, xtl::big_promote_type_t<value_type>,
                      XTENSOR_EMPTY, +, std::plus)
XTENSOR_NORM_FUNCTION(norm_sq, xtl::big_promote_type_t<value_type>,
                      XTENSOR_EMPTY, +, std::plus)
XTENSOR_NORM_FUNCTION(norm_linf,
                      decltype(norm_linf(std::declval<value_type>())),
                      (std::max<result_type>), XTENSOR_COMMA, math::maximum)
 ...
 *
 * Returns an \ref xreducer for the L0 pseudo-norm of the elements across given
 * \em axes.
 ...
 * When no axes are provided, the norm is calculated over the entire array. In
 * this case, the reducer represents a scalar result, otherwise an array of
 ...
 *
 * Returns an \ref xreducer for the L1 norm of the elements across given \em
 * axes.
 * @param e an \ref xexpression
 * @param axes the axes along which the norm is computed (optional)
 * @param es evaluation strategy to use (lazy (default), or immediate)
 ...
 *
 * Returns an \ref xreducer for the squared L2 norm of the elements across given
 * \em axes.
 * @param e an \ref xexpression
 * @param axes the axes along which the norm is computed (optional)
 * @param es evaluation strategy to use (lazy (default), or immediate)
 ...
 *
 * Returns an \ref xreducer for the L2 norm of the elements across given \em
 * axes.
 * @param e an \ref xexpression
 * @param es evaluation strategy to use (lazy (default), or immediate)
 * @param axes the axes along which the norm is computed
 * @return an \ref xreducer (specifically: <tt>sqrt(norm_sq(e, axes))</tt>) (or
 * xcontainer, depending on evaluation strategy)
 */
template <class E, class X, class EVS = DEFAULT_STRATEGY_REDUCERS,
...
inline auto norm_l2(E&& e, const I (&axes)[N], EVS es = EVS()) noexcept {
  ...
  return sqrt(norm_sq(std::forward<E>(e),
                      xtl::forward_sequence<axes_type, decltype(axes)>(axes),
                      es));
}
 ...
 *
 * Returns an \ref xreducer for the infinity norm of the elements across given
 * \em axes.
 * @param e an \ref xexpression
 * @param axes the axes along which the norm is computed (optional)
 * @param es evaluation strategy to use (lazy (default), or immediate)
 ...
 *
 * Returns an \ref xreducer for the p-th power of the Lp norm of the elements
 * across given \em axes.
 * @param e an \ref xexpression
 * @param p
 * @param axes the axes along which the norm is computed (optional)
 * @param es evaluation strategy to use (lazy (default), or immediate)
...
template <class E, class I, std::size_t N,
          class EVS = DEFAULT_STRATEGY_REDUCERS>
inline auto norm_lp_to_p(E&& e, double p, const I (&axes)[N],
                         EVS es = EVS()) noexcept {
  using axes_type = std::array<typename std::decay_t<E>::size_type, N>;
  return norm_lp_to_p(std::forward<E>(e), p,
                      xtl::forward_sequence<axes_type, decltype(axes)>(axes),
                      es);
}
 ...
 *
 * Returns an \ref xreducer for the Lp norm (p != 0) of the elements across
 * given \em axes.
 * @param e an \ref xexpression
 * @param p
 * @param axes the axes along which the norm is computed (optional)
...
inline auto norm_lp(E&& e, double p, X&& axes, EVS es = EVS()) {
  ...
  return pow(norm_lp_to_p(std::forward<E>(e), p, std::forward<X>(axes), es),
             1.0 / p);
}
...
inline auto norm_lp(E&& e, double p, const I (&axes)[N], EVS es = EVS()) {
  using axes_type = std::array<typename std::decay_t<E>::size_type, N>;
  return norm_lp(std::forward<E>(e), p,
                 xtl::forward_sequence<axes_type, decltype(axes)>(axes), es);
}
 ...
 *
 * Returns an \ref xreducer for the induced L1 norm (i.e. the maximum of the L1
 * norms of e's columns).
...
inline auto norm_induced_l1(E&& e, EVS es = EVS()) {
  XTENSOR_PRECONDITION(e.dimension() == 2,
                       "norm_induced_l1(): only applicable to matrices "
                       "(e.dimension() must be 2).");
 ...
 *
 * Returns an \ref xreducer for the induced L-infinity norm (i.e. the maximum of
 * the L1 norms of e's rows).
 * @param e a 2D \ref xexpression
 * @param es evaluation strategy to use (lazy (default), or immediate)
 * @return an \ref xreducer (or xcontainer, depending on evaluation strategy)
 */
...
inline auto norm_induced_linf(E&& e, EVS es = EVS()) {
  XTENSOR_PRECONDITION(e.dimension() == 2,
                       "norm_induced_linf(): only applicable to matrices "
                       "(e.dimension() must be 2).");
