...
template <template <class...> class A, class... AX, class X,
          XTL_REQUIRES(is_evaluation_strategy<AX>...,
                       is_evaluation_strategy<X>)>
...
struct xinitial : xt::detail::option_base {
  constexpr xinitial(T val) : m_val(val) {}
...
struct tuple_idx_of {
  static constexpr std::ptrdiff_t value =
      tuple_idx_of_impl<0, std::decay_t<T>,
                        typename decay_all<Tuple>::type>::value;
};
...
struct reducer_options {
  ...
  reducer_options(const T& tpl) {
    xtl::mpl::static_if<initial_val_idx != std::tuple_size<T>::value>(
        [this, &tpl](auto no_compile) {
          // use no_compile to prevent compilation if initial_val_idx is out of
          // bounds!
          this->initial_value =
  ...
  using keep_dims =
      std::conditional_t<tuple_idx_of<xt::keep_dims_type, d_t>::value != -1,
                         std::true_type, std::false_type>;

  constexpr static bool has_initial_value =
      initial_val_idx != std::tuple_size<d_t>::value;
...
inline void shape_computation(
    RS& result_shape, R& result, E& expr, const AX& axes,
...
inline void shape_computation(
    RS&, R&, const S&, const AX&,
    std::enable_if_t<detail::is_fixed<RS>::value, int> = 0) {}
}  // namespace detail

template <class F, class E, class R,
          XTL_REQUIRES(std::is_convertible<typename E::value_type,
...
inline void copy_to_reduced(F&, const E& e, R& result) {
  if (e.layout() == layout_type::row_major) {
    std::copy(e.template cbegin<layout_type::row_major>(),
              e.template cend<layout_type::row_major>(), result.data());
  } else {
    std::copy(e.template cbegin<layout_type::column_major>(),
              e.template cend<layout_type::column_major>(), result.data());
  }
}

template <class F, class E, class R,
          XTL_REQUIRES(xtl::negation<std::is_convertible<
                           typename E::value_type, typename R::value_type>>)>
inline void copy_to_reduced(F& f, const E& e, R& result) {
  if (e.layout() == layout_type::row_major) {
    std::transform(e.template cbegin<layout_type::row_major>(),
                   e.template cend<layout_type::row_major>(), result.data(), f);
  } else {
...
inline auto reduce_immediate(F&& f, E&& e, X&& axes, O&& raw_options) {
  ...
  using shape_type =
      typename xreducer_shape_type<typename std::decay_t<E>::shape_type,
                                   std::decay_t<X>,
  ...
  dynamic_shape<std::size_t> iter_shape =
      xtl::forward_sequence<dynamic_shape<std::size_t>, decltype(e.shape())>(
          e.shape());
  dynamic_shape<std::size_t> iter_strides(e.dimension());

  // using std::less_equal is counter-intuitive, but as the standard says
  // (24.4.5): A sequence is sorted with respect to a comparator comp if for any
  ...
  // comp(*(i + n), *i) == false. Therefore less_equal is required to detect
  // duplicates.
  if (!std::is_sorted(axes.cbegin(), axes.cend(), std::less_equal<>())) {
    XTENSOR_THROW(
        std::runtime_error,
        "Reducing axes should be sorted and should not contain duplicates");
  }
  ...
  if (e.dimension() == axes.size()) {
    result_type tmp =
        options_t::has_initial_value ? options.initial_value : init_fct();
    result.data()[0] = std::accumulate(e.storage().begin(), e.storage().end(),
                                       tmp, reduce_fct);
    return result;
  }

  std::size_t leading_ax =
      axes[(e.layout() == layout_type::row_major) ? axes.size() - 1 : 0];
  auto strides_finder =
      e.strides().begin() + static_cast<std::ptrdiff_t>(leading_ax);
  // The computed strides contain "0" where the shape is 1 -- therefore find the
  // next none-zero number
  std::size_t inner_stride = static_cast<std::size_t>(*strides_finder);
  auto iter_bound = e.layout() == layout_type::row_major
                        ? e.strides().begin()
                        : (e.strides().end() - 1);
  while (inner_stride == 0 && strides_finder != iter_bound) {
    (e.layout() == layout_type::row_major) ? --strides_finder
                                           : ++strides_finder;
  ...
  // The following code merges reduction axes "at the end" (or the beginning for
  // col_major) together by increasing the size of the outer loop where
  // appropriate
  ...
  if (e.layout() == layout_type::row_major) {
    ...
    iter_shape.erase(iter_shape.begin() + std::ptrdiff_t(last_ax),
                     iter_shape.end());
    iter_strides.erase(iter_strides.begin() + std::ptrdiff_t(last_ax),
                       iter_strides.end());
    ...
    iter_shape.erase(iter_shape.begin(),
                     iter_shape.begin() + std::ptrdiff_t(last_ax + 1));
  ...
  auto next_idx = [&iter_shape, &iter_strides, &temp_idx]() {
    ...
    for (; i > 0; --i) {
      if (std::ptrdiff_t(temp_idx[i - 1]) >=
          std::ptrdiff_t(iter_shape[i - 1]) - 1) {
        temp_idx[i - 1] = 0;
      } else {
        temp_idx[i - 1]++;
        break;
    ...
    return std::make_pair(
        i == 0, std::inner_product(temp_idx.begin(), temp_idx.end(),
  ...
  // Remark: eventually some modifications here to make conditions faster where
  // merge + accumulate is the same function (e.g. check
  // std::is_same<decltype(merge_fct), decltype(reduce_fct)>::value) ...

  auto merge_border = out;
  bool merge = false;

  // TODO there could be some performance gain by removing merge checking
  //      when axes.size() == 1 and even next_idx could be removed for something
  //      simpler (next_stride always the same) best way to do this would be to
  //      create a function that takes (begin, out, outer_loop_size,
  //      inner_loop_size, next_idx_lambda)
      ...
      std::transform(out, out + inner_loop_size, begin, out,
                     [merge, &init_fct, &reduce_fct](auto&& v1, auto&& v2) {
                       return merge ? reduce_fct(v1, v2) :
                                    // cast because return type of identity
                                    // function is not upcasted
                                  reduce_fct(
...
struct const_value {
  ...
  constexpr const_value(T t) : m_value(t) {}
...
template <class REDUCE_FUNC, class INIT_FUNC = const_value<long int>,
...
struct xreducer_functors
  ...
  xreducer_functors(RF&& reduce_func, IF&& init_func)
      : base_type(std::forward<RF>(reduce_func), std::forward<IF>(init_func),
                  reduce_func) {}
  ...
  reduce_functor_type get_reduce() const { return std::get<0>(upcast()); }

  init_functor_type get_init() const { return std::get<1>(upcast()); }

  merge_functor_type get_merge() const { return std::get<2>(upcast()); }
  ...
  template <class NT>
  rebind_t<NT> rebind() {
    return make_xreducer_functor(get_reduce(), get_init().template rebind<NT>(),
                                 get_merge());
  }
...
struct xcontainer_inner_types<xreducer<F, CT, X, O>> {
  ...
  using value_type = typename detail::evaluated_value_type_t<
      raw_value_type, is_xexpression<raw_value_type>::value>;
 ...
 * <tt>F::reduce_functor_type</tt> when called with elements of the expression
 * @tparam CT.
 *
 * @sa reduce
 */
...
class xreducer : public xsharable_expression<xreducer<F, CT, X, O>>,
 ...
 public:
  ...
  using xreducer_functors_type =
      xreducer_functors<reduce_functor_type, init_functor_type,
                        merge_functor_type>;
  ...
  xreducer_functors_type functors() const {
    return xreducer_functors_type(
        m_reduce, m_init,
        m_merge);  // TODO: understand why make_xreducer_functor is throwing an
                   // error
  }

  const O& options() const { return m_options; }
...
namespace detail {
template <class F, class E, class X, class O>
inline auto reduce_impl(F&& f, E&& e, X&& axes, evaluation_strategy::lazy_type,
                        O&& options) {
  ...
  using evaluated_value_type =
      evaluated_value_type_t<value_type, is_xexpression<value_type>::value>;

  using reducer_type =
      xreducer<F, const_xclosure_t<E>,
...
template <class F, class E, class X, class O>
inline auto reduce_impl(F&& f, E&& e, X&& axes,
                        evaluation_strategy::immediate_type, O&& options) {
  decltype(auto) normalized_axes = normalize_axis(e, std::forward<X>(axes));
  return reduce_immediate(
      std::forward<F>(f), eval(std::forward<E>(e)),
      std::forward<decltype(normalized_axes)>(normalized_axes),
      std::forward<O>(options));
}
}  // namespace detail
...
inline auto reduce(F&& f, E&& e, X&& axes, EVS&& options = EVS()) {
  return detail::reduce_impl(
      std::forward<F>(f), std::forward<E>(e), std::forward<X>(axes),
...
inline auto reduce(F&& f, E&& e, EVS&& options = EVS()) {
  ...
  return detail::reduce_impl(
      std::forward<F>(f), std::forward<E>(e), std::move(ar),
...
inline auto reduce(F&& f, E&& e, const I (&axes)[N], EVS options = EVS()) {
  ...
  return detail::reduce_impl(
      std::forward<F>(f), std::forward<E>(e), std::move(ax),
      typename reducer_options<int, EVS>::evaluation_strategy{}, options);
}
template <class F, class E, class I, std::size_t N,
          class EVS = DEFAULT_STRATEGY_REDUCERS,
...
class xreducer_stepper {
 public:
  ...
  xreducer_stepper(const xreducer_type& red, size_type offset, bool end = false,
                   layout_type l = default_assignable_layout(
...
struct fixed_xreducer_shape_type_impl<Z, fixed_shape<I...>, fixed_shape<J...>,
                                      fixed_shape<R...>> {
  using type = std::conditional_t<
      in<Z, J...>::value,
      typename fixed_xreducer_shape_type_impl<
          Z - 1, fixed_shape<I...>, fixed_shape<J...>, fixed_shape<R...>>::type,
      typename fixed_xreducer_shape_type_impl<
          Z - 1, fixed_shape<I...>, fixed_shape<J...>,
          fixed_shape<detail::at<Z, I...>::value, R...>>::type>;
};

template <std::size_t... I, std::size_t... J, std::size_t... R>
struct fixed_xreducer_shape_type_impl<0, fixed_shape<I...>, fixed_shape<J...>,
                                      fixed_shape<R...>> {
  using type =
      std::conditional_t<in<0, J...>::value, fixed_shape<R...>,
...
struct fixed_xreducer_shape_type<fixed_shape<I...>, fixed_shape<J...>> {
  using type = typename detail::fixed_xreducer_shape_type_impl<
      sizeof...(I) - 1, fixed_shape<I...>, fixed_shape<J...>,
      fixed_shape<>>::type;
};
...
struct xreducer_shape_type<fixed_shape<I...>, std::array<I2, N2>,
                           std::false_type> {
  using type = std::conditional_t<sizeof...(I) == N2, fixed_shape<>,
...
struct ixconcat<std::integer_sequence<T, I1...>,
...
struct xreducer_shape_type<fixed_shape<I...>, std::array<I2, N2>,
  ...
  // if all axes reduced
  using type =
      std::conditional_t<sizeof...(I) == N2,
                         decltype(get_type(
                             typename detail::repeat_integer_sequence<
                                 std::size_t, std::size_t(1), N2>::type{})),
...
struct xreducer_shape_type<fixed_shape<I...>, fixed_shape<J...>, O> {
  using type = typename fixed_xreducer_shape_type<fixed_shape<I...>,
                                                  fixed_shape<J...>>::type;
};

namespace detail {
template <class S, class E, class X, class M>
inline void shape_and_mapping_computation(S& shape, E& e, const X& axes,
                                          M& mapping, std::false_type) {
...
template <class S, class E, class X, class M>
inline void shape_and_mapping_computation_keep_dim(S& shape, E& e,
                                                   const X& axes, M& mapping,
                                                   std::false_type) {
...
template <class S, class E, class X, class M>
inline void shape_and_mapping_computation(S&, E&, const X&, M&,
                                          std::true_type) {}

template <class S, class E, class X, class M>
inline void shape_and_mapping_computation_keep_dim(S&, E&, const X&, M&,
                                                   std::true_type) {}
}  // namespace detail
...
template <class F, class CT, class X, class O>
template <class Func, class CTA, class AX, class OX>
inline xreducer<F, CT, X, O>::xreducer(Func&& func, CTA&& e, AX&& axes,
                                       OX&& options)
    : m_e(std::forward<CTA>(e)),
      ...
      m_shape(xtl::make_sequence<inner_shape_type>(
          typename O::keep_dims() ? m_e.dimension()
                                  : m_e.dimension() - m_axes.size(),
          0)),
  ...
  if (!typename O::keep_dims()) {
    detail::shape_and_mapping_computation(m_shape, m_e, m_axes, m_dim_mapping,
 ...
 */
template <class F, class CT, class X, class O>
inline auto xreducer<F, CT, X, O>::shape() const noexcept
...
template <class F, class CT, class X, class O>
template <class... Args>
inline auto xreducer<F, CT, X, O>::operator()(Args... args) const
    -> const_reference {
 ...
 * reducer.
 * @param args a list of indices specifying the position in the reducer. Indices
 * must be unsigned integers, the number of indices must be equal to the number
 * of dimensions of the reducer, else the behavior is undefined.
 *
 * @warning This method is meant for performance, for expressions with a dynamic
 * number of dimensions (i.e. not known at compile time). Since it may have
 * undefined behavior (see parameters), operator() should be prefered whenever
 * it is possible.
 * @warning This method is NOT compatible with broadcasting, meaning the
 * following code has undefined behavior: \code{.cpp} xt::xarray<double> a =
 * {{0, 1}, {2, 3}}; xt::xarray<double> b = {0, 1}; auto fd = a + b; double res
 * = fd.uncheked(0, 1); \endcode
 */
template <class F, class CT, class X, class O>
template <class... Args>
inline auto xreducer<F, CT, X, O>::unchecked(Args... args) const
    -> const_reference {
  std::array<std::size_t, sizeof...(Args)> arg_array = {
      {static_cast<std::size_t>(args)...}};
  return element(arg_array.cbegin(), arg_array.cend());
...
template <class F, class CT, class X, class O>
template <class It>
inline auto xreducer<F, CT, X, O>::element(It first, It last) const
    -> const_reference {
 ...
 */
template <class F, class CT, class X, class O>
inline auto xreducer<F, CT, X, O>::expression() const noexcept
...
template <class F, class CT, class X, class O>
template <class S>
inline bool xreducer<F, CT, X, O>::has_linear_assign(
    const S& /*strides*/) const noexcept {
  return false;
}
//@}
...
template <class F, class CT, class X, class O>
template <class S>
inline auto xreducer<F, CT, X, O>::stepper_begin(const S& shape) const noexcept
    -> const_stepper {
...
template <class F, class CT, class X, class O>
template <class S>
inline auto xreducer<F, CT, X, O>::stepper_end(const S& shape,
...
inline auto xreducer<F, CT, X, O>::build_reducer(E&& e) const -> rebind_t<E> {
  return rebind_t<E>(std::make_tuple(m_reduce, m_init, m_merge),
                     std::forward<E>(e), axes_type(m_axes), m_options);
}

template <class F, class CT, class X, class O>
template <class E, class Func, class Opts>
inline auto xreducer<F, CT, X, O>::build_reducer(E&& e, Func&& func,
                                                 Opts&& opts) const
    -> rebind_t<E, Func, Opts> {
  return rebind_t<E, Func, Opts>(std::forward<Func>(func), std::forward<E>(e),
                                 axes_type(m_axes), std::forward<Opts>(opts));
}
...
inline void xreducer_stepper<F, CT, X, O>::reset(size_type dim) {
  if (dim >= m_offset) {
    ...
    if (typename O::keep_dims() &&
        std::binary_search(m_reducer->m_axes.begin(), m_reducer->m_axes.end(),
                           dim - m_offset)) {
...
template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::aggregate(size_type dim) const
    -> reference {
  reference res;
...
template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::aggregate_impl(size_type dim,
...
inline auto xreducer_stepper<F, CT, X, O>::aggregate_impl(size_type dim,
  ...
  reference res = {};
  auto ax_it =
      std::find(m_reducer->m_axes.begin(), m_reducer->m_axes.end(), dim);
...
template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::get_substepper_begin() const
    -> substepper_type {
  return m_reducer->m_e.stepper_begin(m_reducer->m_e.shape());
}

template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::get_dim(size_type dim) const noexcept
    -> size_type {
  return m_reducer->m_dim_mapping[dim];
}

template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::shape(size_type i) const noexcept
    -> size_type {
  return m_reducer->m_e.shape()[i];
}

template <class F, class CT, class X, class O>
inline auto xreducer_stepper<F, CT, X, O>::axis(size_type i) const noexcept
    -> size_type {
