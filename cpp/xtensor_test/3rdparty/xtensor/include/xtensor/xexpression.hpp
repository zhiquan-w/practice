 ...
 * expression that can be evaluated to a multidimensional container with tensor
 * semantic. Functions that can apply to any xexpression regardless of its
 * specific type should take a xexpression argument.
 *
...
class xsharable_expression : public xexpression<D> {
 ...
 private:
  ...
  friend xshared_expression<D> detail::make_xshared_impl<D>(
 ...
 */
template <class D>
inline auto xexpression<D>::derived_cast() const& noexcept
    -> const derived_type& {
...
/**
 * is_crtp_base_of<B, E>
 * Resembles std::is_base_of, but adresses the problem of whether _some_
 * instantiation of a CRTP templated class B is a base of class E. A CRTP
 * templated class is correctly templated with the most derived type in the CRTP
 * hierarchy. Using this assumption, this implementation deals with either CRTP
 * final classes (checks for inheritance with E as the CRTP parameter of B) or
 * CRTP base classes (which are singly templated by the most derived class, and
 * that's pulled out to use as a templete parameter for B).
 */
...
struct is_crtp_base_of_impl<B, F<E>>
    : xtl::disjunction<std::is_base_of<B<E>, F<E>>,
                       std::is_base_of<B<F<E>>, F<E>>> {};
}  // namespace detail
...
template <class E, class R = void>
using enable_xsharable_expression =
    typename std::enable_if<is_xsharable_expression<E>::value, R>::type;

template <class E, class R = void>
using disable_xsharable_expression =
    typename std::enable_if<!is_xsharable_expression<E>::value, R>::type;
...
using enable_assignable_expression =
    typename std::enable_if<can_assign<LHS, RHS>::value, R>::type;

template <class LHS, class RHS, class R = void>
using enable_not_assignable_expression =
    typename std::enable_if<!can_assign<LHS, RHS>::value, R>::type;
...
struct is_evaluation_strategy
    : std::is_base_of<detail::option_base, std::decay_t<T>> {};
...
template <class E>
struct get_expression_tag_impl<
    E, void_t<typename std::decay_t<E>::expression_tag>> {
...
struct is_xtensor_expression
    : std::is_same<xexpression_tag_t<E>, xtensor_expression_tag> {};

template <class E>
struct is_xoptional_expression
    : std::is_same<xexpression_tag_t<E>, xoptional_expression_tag> {};
...
struct xoptional_comparable
    : xtl::conjunction<xtl::disjunction<is_xtensor_expression<E>,
...
#define XTENSOR_FORWARD_METHOD(name) \
  auto name()->decltype(std::declval<E>().name()) { return m_ptr->name(); }
 ...
 *     // return xt::sin(std::forward<E>(expr)) +
 * xt::cos(std::forward<E>(expr));
 *     // THIS IS WRONG TOO: because second `expr` is taken as reference (which
 * will be invalid)
 ...
 *     std::cout << shared_expr.use_count() << std::endl; // Will print 3
 * because used twice in expression return result; // all valid because expr
 * lifetime managed by xshared_expression / shared_ptr.
 * }
 * \endcode
 */
template <class E>
class xshared_expression : public xexpression<xshared_expression<E>> {
 public:
  ...
  using strides_type =
      xtl::mpl::eval_if_t<has_strides<E>, detail::expr_strides_type<E>,
                          get_strides_type<shape_type>>;
  using backstrides_type =
      xtl::mpl::eval_if_t<has_strides<E>, detail::expr_backstrides_type<E>,
                          get_strides_type<shape_type>>;
  using inner_strides_type =
      xtl::mpl::eval_if_t<has_strides<E>, detail::expr_inner_strides_type<E>,
                          get_strides_type<shape_type>>;
  using inner_backstrides_type =
      xtl::mpl::eval_if_t<has_strides<E>,
                          detail::expr_inner_backstrides_type<E>,
                          get_strides_type<shape_type>>;
  using storage_type =
      xtl::mpl::eval_if_t<has_data_interface<E>, detail::expr_storage_type<E>,
                          make_invalid_type<>>;
  ...
  template <class T = E>
  std::enable_if_t<has_strides<T>::value, const inner_strides_type&> strides()
      const {
  ...
  template <class T = E>
  std::enable_if_t<has_data_interface<T>::value, pointer> data()
      const noexcept {
    return m_ptr->data();
  }

  template <class T = E>
  std::enable_if_t<has_data_interface<T>::value, size_type> data_offset()
      const noexcept {
  ...
  template <class T = E>
  std::enable_if_t<has_data_interface<T>::value,
                   const typename T::storage_type&>
...
inline xshared_expression<E> make_xshared(xexpression<E>&& expr) {
  static_assert(is_xsharable_expression<E>::value,
