...
template <class EC, std::size_t N, layout_type L, class Tag>
using xtensor_container_base_t =
    typename xtensor_container_base<EC, N, L, Tag>::type;
}  // namespace extension
...
class xtensor_container
 ...
 public:
  ...
  explicit xtensor_container(storage_type&& storage, inner_shape_type&& shape,
...
template <class EC, std::size_t N, layout_type L, class Tag>
using xtensor_adaptor_base_t =
    typename xtensor_adaptor_base<EC, N, L, Tag>::type;
}  // namespace extension
...
struct xcontainer_inner_types<xtensor_adaptor<EC, N, L, Tag>> {
  ...
  using temporary_type =
      xtensor_container<temporary_container_t<storage_type>, N, L, Tag>;
  static constexpr layout_type layout = L;
};
...
class xtensor_adaptor
 ...
 public:
  ...
  template <class D>
  xtensor_adaptor(D&& storage, const shape_type& shape,
                  const strides_type& strides);
...
class xtensor_view : public xstrided_container<xtensor_view<EC, N, L, Tag>>,
 ...
 public:
  ...
  template <class D>
  xtensor_view(D&& storage, const shape_type& shape,
               const strides_type& strides);
...
struct tensor_view_simd_helper {
  using valid_return_type =
      detail::has_simd_interface_impl<V, typename V::value_type>;
  using valid_reference = std::is_lvalue_reference<typename V::reference>;
  static constexpr bool value =
      valid_return_type::value && valid_reference::value;
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline xtensor_container<EC, N, L, Tag>::xtensor_container(
    const shape_type& shape, layout_type l)
    : base_type() {
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline xtensor_container<EC, N, L, Tag>::xtensor_container(
    const shape_type& shape, const_reference value, layout_type l)
    : base_type() {
  base_type::resize(shape, l);
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline xtensor_container<EC, N, L, Tag>::xtensor_container(
    storage_type&& storage, inner_shape_type&& shape,
    inner_strides_type&& strides)
...
inline xtensor_container<EC, N, L, Tag>::xtensor_container(
    ...
    : base_type(
          xtl::forward_sequence<inner_shape_type, decltype(rhs.shape())>(
              rhs.shape()),
          xtl::forward_sequence<inner_strides_type, decltype(rhs.strides())>(
              rhs.strides()),
          xtl::forward_sequence<inner_backstrides_type,
                                decltype(rhs.backstrides())>(rhs.backstrides()),
          std::move(rhs.layout())),
...
xtensor_container<EC, N, L, Tag>::operator=(
    xarray_container<EC, L, SC, Tag>&& rhs) {
  XTENSOR_ASSERT_MSG(N == rhs.dimension(),
                     "Cannot change dimension of xtensor.");
  std::copy(rhs.shape().begin(), rhs.shape().end(), this->shape_impl().begin());
  std::copy(rhs.strides().cbegin(), rhs.strides().cend(),
            this->strides_impl().begin());
  std::copy(rhs.backstrides().cbegin(), rhs.backstrides().cend(),
            this->backstrides_impl().begin());
...
template <class EC, std::size_t N, layout_type L, class Tag>
template <class E>
inline auto xtensor_container<EC, N, L, Tag>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_container<EC, N, L, Tag>::storage_impl() noexcept
    -> storage_type& {
  return m_storage;
}
...
template <class D>
inline xtensor_adaptor<EC, N, L, Tag>::xtensor_adaptor(
    D&& storage, const shape_type& shape, const strides_type& strides)
    : base_type(), m_storage(std::forward<D>(storage)) {
  base_type::resize(shape, strides);
}
//@}
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_adaptor<EC, N, L, Tag>::operator=(
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_adaptor<EC, N, L, Tag>::operator=(xtensor_adaptor&& rhs)
    -> self_type& {
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_adaptor<EC, N, L, Tag>::operator=(temporary_type&& rhs)
    -> self_type& {
  base_type::shape_impl() = std::move(const_cast<shape_type&>(rhs.shape()));
  base_type::strides_impl() =
      std::move(const_cast<strides_type&>(rhs.strides()));
  base_type::backstrides_impl() =
      std::move(const_cast<backstrides_type&>(rhs.backstrides()));
  m_storage = std::move(rhs.storage());
  return *this;
}
...
template <class EC, std::size_t N, layout_type L, class Tag>
template <class E>
inline auto xtensor_adaptor<EC, N, L, Tag>::operator=(const xexpression<E>& e)
    -> self_type& {
  return semantic_base::operator=(e);
}
//@}
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_adaptor<EC, N, L, Tag>::storage_impl() noexcept
    -> storage_type& {
  return m_storage;
}

template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_adaptor<EC, N, L, Tag>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}
...
template <class EC, std::size_t N, layout_type L, class Tag>
template <class D>
inline xtensor_view<EC, N, L, Tag>::xtensor_view(D&& storage,
                                                 const shape_type& shape,
                                                 layout_type l)
    : base_type(), m_storage(std::forward<D>(storage)) {
  base_type::resize(shape, l);
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_view<EC, N, L, Tag>::operator=(const xtensor_view& rhs)
    -> self_type& {
  base_type::operator=(rhs);
  m_storage = rhs.m_storage;
  return *this;
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_view<EC, N, L, Tag>::operator=(xtensor_view&& rhs)
    -> self_type& {
  base_type::operator=(std::move(rhs));
  m_storage = rhs.m_storage;
  return *this;
...
template <class EC, std::size_t N, layout_type L, class Tag>
template <class E>
inline auto xtensor_view<EC, N, L, Tag>::operator=(const xexpression<E>& e)
    -> self_type& {
  return semantic_base::operator=(e);
}
//@}
...
template <class EC, std::size_t N, layout_type L, class Tag>
template <class E>
inline auto xtensor_view<EC, N, L, Tag>::operator=(const E& e)
...
template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_view<EC, N, L, Tag>::storage_impl() noexcept
    -> storage_type& {
  return m_storage;
}

template <class EC, std::size_t N, layout_type L, class Tag>
inline auto xtensor_view<EC, N, L, Tag>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}

template <class EC, std::size_t N, layout_type L, class Tag>
inline void xtensor_view<EC, N, L, Tag>::assign_temporary_impl(
    temporary_type&& tmp) {
  std::copy(tmp.cbegin(), tmp.cend(), m_storage.begin());
}

/**
 * Converts ``std::vector<index_type>`` (returned e.g. from ``xt::argwhere``) to
 * ``xtensor``.
 *
 * @param idx vector of indices
 *
 * @return ``xt::xtensor<typename index_type::value_type, 2>`` (e.g.
 * ``xt::xtensor<size_t, 2>``)
 */
template <class T>
 ...
 *
 * @return ``xt::xtensor<typename index_type::value_type, 1>`` (e.g.
 * ``xt::xtensor<size_t, 1>``)
 */
template <class T>
...
/**
 * Converts ``std::vector<index_type>`` (returned e.g. from ``xt::argwhere``) to
 * ``xtensor`` whereby the indices are ravelled. For 1-d input there is no
 * conversion.
 *
 * @param idx vector of indices
...
ravel_return_type_t<C, Tag> ravel_indices(
    const C& idx, const S& shape, layout_type l = layout_type::row_major) {
  ...
  for (; out_iter != out.end(); ++out_iter, ++idx_iter) {
    *out_iter = element_offset<value_type>(strides, (*idx_iter).cbegin(),
                                           (*idx_iter).cend());
  }
  return out;
}
