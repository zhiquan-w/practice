...
template <class ACCUMULATE_FUNC,
          class INIT_FUNC = detail::accumulator_identity<void>>
struct xaccumulator_functor : public std::tuple<ACCUMULATE_FUNC, INIT_FUNC> {
  ...
  xaccumulator_functor() : base_type() {}
  ...
  xaccumulator_functor(RF&& accumulate_func, IF&& init_func)
      : base_type(std::forward<RF>(accumulate_func),
...
xarray<typename std::decay_t<E>::value_type> accumulator_impl(F&&, E&&,
                                                              std::size_t,
                                                              EVS) {
...
template <class T, class R>
using xaccumulator_return_type_t =
...
struct xaccumulator_linear_return_type<xtensor_fixed<T, xshape<I...>, L>, R> {
  using type =
      xtensor_fixed<R, xshape<fixed_compute_size<xshape<I...>>::value>, L>;
};
...
inline auto accumulator_init_with_f(F&& f, E& e, std::size_t axis) {
  ...
  auto set_loop_sizes = [&outer_loop_size, &inner_loop_size](
                            auto first, auto last, std::ptrdiff_t ax) {
    outer_loop_size = std::accumulate(first, first + ax, std::size_t(1),
                                      std::multiplies<std::size_t>());
    inner_loop_size = std::accumulate(first + ax + 1, last, std::size_t(1),
                                      std::multiplies<std::size_t>());
  };

  // Note: add check that strides > 0
  auto set_loop_strides = [&outer_stride, &inner_stride](auto first, auto last,
                                                         std::ptrdiff_t ax) {
    outer_stride = static_cast<std::size_t>(
  ...
  set_loop_sizes(e.shape().begin(), e.shape().end(),
                 static_cast<std::ptrdiff_t>(axis));
  set_loop_strides(e.strides().begin(), e.strides().end(),
                   static_cast<std::ptrdiff_t>(axis));
...
template <class F, class E>
inline auto accumulator_impl(F&& f, E&& e, std::size_t axis,
  ...
  // using return_type = std::conditional_t<std::is_same<init_type,
  // void>::value, typename std::decay_t<E>::value_type, init_type>;

  using return_type =
      std::decay_t<decltype(std::declval<accumulate_functor_type>()(
  ...
  if (result.shape(axis) != std::size_t(0)) {
    ...
    std::size_t outer_stride = 1;  // this is either going row- or column-wise
                                   // (strides.back / strides.front)
    ...
    auto set_loop_sizes = [&outer_loop_size, &inner_loop_size, init_size](
                              auto first, auto last, std::ptrdiff_t ax) {
      outer_loop_size = std::accumulate(first, first + ax, init_size,
                                        std::multiplies<std::size_t>());

      inner_loop_size = std::accumulate(first + ax, last, std::size_t(1),
                                        std::multiplies<std::size_t>());
    };

    if (result_type::static_layout == layout_type::row_major) {
      set_loop_sizes(result.shape().cbegin(), result.shape().cend(),
                     static_cast<std::ptrdiff_t>(axis));
    } else {
      set_loop_sizes(result.shape().cbegin(), result.shape().cend(),
...
template <class F, class E>
inline auto accumulator_impl(F&& f, E&& e,
                             evaluation_strategy::immediate_type) {
  using init_type = typename F::init_value_type;
  using expr_value_type = typename std::decay_t<E>::value_type;
  using accumulate_functor_type = typename F::accumulate_functor_type;
  ...
  if (sz != std::size_t(0)) {
    ...
    for (std::size_t idx = 0; it != e.template end<XTENSOR_DEFAULT_TRAVERSAL>();
         ++it) {
...
inline auto accumulate(F&& f, E&& e, EVS evaluation_strategy = EVS()) {
  // Note we need to check is_integral above in order to prohibit EVS = int, and
  // not taking the std::size_t overload below!
  return detail::accumulator_impl(std::forward<F>(f), std::forward<E>(e),
                                  evaluation_strategy);
}
 ...
 */
template <class F, class E, class EVS = DEFAULT_STRATEGY_ACCUMULATORS>
inline auto accumulate(F&& f, E&& e, std::ptrdiff_t axis,
                       EVS evaluation_strategy = EVS()) {
  std::size_t ax = normalize_axis(e.dimension(), axis);
  return detail::accumulator_impl(std::forward<F>(f), std::forward<E>(e), ax,
                                  evaluation_strategy);
}
}  // namespace xt
