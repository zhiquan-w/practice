...
inline xexpression_holder& xexpression_holder::operator=(
...
inline void xexpression_holder::init_pointer_from_json(
  ...
  if (j.is_number()) {
    ...
    p_holder.reset(new detail::xexpression_wrapper<xt::xarray<double>>(
        std::move(empty_arr)));
  }

  if (j.is_boolean()) {
    xt::xarray<bool> empty_arr;
    p_holder.reset(new detail::xexpression_wrapper<xt::xarray<bool>>(
        std::move(empty_arr)));
  }

  if (j.is_string()) {
    xt::xarray<std::string> empty_arr;
    p_holder.reset(new detail::xexpression_wrapper<xt::xarray<std::string>>(
        std::move(empty_arr)));
  }
...
inline xexpression_wrapper<CTE>::xexpression_wrapper(E&& expr)
    : xexpression_holder_impl(), m_expression(std::forward<E>(expr)) {}
...
template <class CTE>
inline xexpression_wrapper<CTE>::xexpression_wrapper(
    const xexpression_wrapper& wrapper)
    : xexpression_holder_impl(), m_expression(wrapper.m_expression) {}
}  // namespace detail
}  // namespace xt
