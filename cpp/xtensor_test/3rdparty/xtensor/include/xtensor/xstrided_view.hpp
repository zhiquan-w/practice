...
struct select_iterable_base {
  using type = std::conditional_t<L1 == L2 && L1 != layout_type::dynamic,
                                  xcontiguous_iterable<T>, xiterable<T>>;
};
...
struct xcontainer_inner_types<xstrided_view<CT, S, L, FST>> {
  ...
  using temporary_type = typename detail::xtype_for_shape<S>::template type<
      typename xexpression_type::value_type, L>;
...
class xstrided_view
    : public xview_semantic<xstrided_view<CT, S, L, FST>>,
      public select_iterable_base_t<L, std::decay_t<CT>::static_layout,
                                    xstrided_view<CT, S, L, FST>>,
 ...
 public:
  ...
  using iterable_base =
      select_iterable_base_t<L, xexpression_type::static_layout, self_type>;
  ...
  xstrided_view(CTA&& e, SA&& shape, strides_type&& strides, std::size_t offset,
  ...
  template <class ST, class STEP = const_stepper>
  disable_indexed_stepper_t<STEP> stepper_end(const ST& shape,
                                              layout_type l) const;

  template <class ST, class STEP = const_stepper>
  enable_indexed_stepper_t<STEP> stepper_begin(const ST& shape) const;
  template <class ST, class STEP = const_stepper>
  enable_indexed_stepper_t<STEP> stepper_end(const ST& shape,
                                             layout_type l) const;
  ...
  using enable_simd_interface = std::enable_if_t<
      has_simd_interface<T>::value && L != layout_type::dynamic, R>;
...
auto strided_view(E&& e, S&& shape, X&& stride, std::size_t offset = 0,
                  layout_type layout = L) noexcept;
...
template <class CT, class S, layout_type L, class FST>
template <class CTA, class SA>
inline xstrided_view<CT, S, L, FST>::xstrided_view(CTA&& e, SA&& shape,
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::operator=(const self_type& rhs)
    -> self_type& {
...
template <class CT, class S, layout_type L, class FST>
template <class E>
inline auto xstrided_view<CT, S, L, FST>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class CT, class S, layout_type L, class FST>
inline void xstrided_view<CT, S, L, FST>::assign_temporary_impl(
    temporary_type&& tmp) {
  constexpr bool fast_assign =
      xassign_traits<xstrided_view<CT, S, L, FST>,
                     temporary_type>::simd_strided_assign();
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::data_element(size_type i)
    -> reference {
  return storage()[i];
}

template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::data_element(size_type i) const
    -> const_reference {
  return storage()[i];
}
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::flat(size_type i) const
    -> const_reference {
  return storage()[i];
}
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::storage_cbegin() const
    -> const_storage_iterator {
  return this->storage().cbegin() + static_cast<std::ptrdiff_t>(data_offset());
}

template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::storage_cend() const
    -> const_storage_iterator {
...
template <class CT, class S, layout_type L, class FST>
template <class ST, class STEP>
inline auto xstrided_view<CT, S, L, FST>::stepper_end(const ST& shape,
                                                      layout_type l)
    -> disable_indexed_stepper_t<STEP> {
  size_type offset = shape.size() - dimension();
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline It xstrided_view<CT, S, L, FST>::data_xbegin_impl(
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline It xstrided_view<CT, S, L, FST>::data_xend_impl(
    It begin, layout_type l, size_type offset) const noexcept {
  return strided_data_end(*this, begin + std::ptrdiff_t(this->data_offset()), l,
                          offset);
}

template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::data_xbegin() noexcept
    -> container_iterator {
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::data_xend(layout_type l,
...
template <class CT, class S, layout_type L, class FST>
inline auto xstrided_view<CT, S, L, FST>::data_xend(
...
template <class CT, class S, layout_type L, class FST>
template <class E>
inline auto xstrided_view<CT, S, L, FST>::build_view(E&& e) const
    -> rebind_t<E> {
  ...
  return rebind_t<E>(std::forward<E>(e), std::move(sh), std::move(str),
                     base_type::data_offset(), this->layout());
}
...
inline auto strided_view(E&& e, S&& shape, X&& strides, std::size_t offset,
                         layout_type layout) noexcept {
  using view_type = xstrided_view<xclosure_t<E>, S, L>;
  return view_type(std::forward<E>(e), std::forward<S>(shape),
                   std::forward<X>(strides), offset, layout);
}
...
struct no_adj_strides_policy {
 protected:
  ...
  bool fill_args(const xstrided_slice_vector& /*slices*/,
                 std::size_t /*sl_idx*/, std::size_t /*i*/,
                 std::size_t /*old_shape*/, const ST& /*old_stride*/,
...
inline auto reshape_view(E&& e, S&& shape) {
  ...
  constexpr auto computed_layout =
      std::decay_t<E>::static_layout == L ? L : layout_type::dynamic;
  using view_type =
      xstrided_view<xclosure_t<E>, shape_type, computed_layout,
                    detail::flat_adaptor_getter<xclosure_t<E>, L>>;
  return view_type(std::forward<E>(e), std::forward<S>(shape),
                   std::move(strides), 0, e.layout());
}
...
inline auto reshape_view(E&& e, const I (&shape)[N], layout_type order) {
  ...
  return reshape_view<L>(
      std::forward<E>(e),
      xtl::forward_sequence<shape_type, decltype(shape)>(shape), order);
}
