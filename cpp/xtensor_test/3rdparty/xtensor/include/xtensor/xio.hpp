...
std::ostream& xoutput(std::ostream& out, const E& e,
                      xstrided_slice_vector& slices, F& printer,
                      std::size_t blanks, std::streamsize element_width,
                      std::size_t edgeitems, std::size_t line_width) {
  ...
  if (view.dimension() == 0) {
    ...
    for (; i != size_type(view.shape()[0] - 1); ++i) {
      if (edgeitems && size_type(view.shape()[0]) > (edgeitems * 2) &&
          i == edgeitems) {
        out << "..., ";
        if (view.dimension() > 1) {
          elems_on_line = 0;
      ...
      xoutput(out, e, slices, printer, blanks + 1, element_width, edgeitems,
              line_width)
          << ',';
      slices.pop_back();
      elems_on_line++;
...
static void recurser_run(F& fn, const E& e, xstrided_slice_vector& slices,
                         std::size_t lim = 0) {
  using size_type = typename E::size_type;
...
template <class T>
struct printer<
    T, std::enable_if_t<xtl::is_integral<typename T::value_type>::value &&
...
template <class T>
struct printer<
    T, std::enable_if_t<std::is_same<typename T::value_type, bool>::value>> {
...
template <class T>
struct printer<
    T, std::enable_if_t<xtl::is_complex<typename T::value_type>::value>> {
  using value_type = std::decay_t<typename T::value_type>;
  using cache_type = std::vector<bool>;
  using cache_iterator = typename cache_type::const_iterator;
...
template <class T>
struct printer<
    T, std::enable_if_t<!xtl::is_fundamental<typename T::value_type>::value &&
...
template <class E, class F>
std::ostream& pretty_print(const xexpression<E>& e, F&& func,
...
std::ostream& pretty_print(const xexpression<E>& e,
  ...
  p.init();
  sv.clear();
  xoutput(out, d, sv, p, 1, p.width(), lim,
