...
namespace detail {
/**************************************************************************************
   The following is something we can currently only dream about -- for when we
drop support for a lot of the old compilers (e.g. GCC 4.9, MSVC 2017 ;)
...
constexpr R get_strides_impl(const xt::fixed_shape<X...>& shape,
  ...
  return R(
      {workaround::get_computed_strides<L, I, temp_type>(shape[I] == 1)...});
#else
...
constexpr T get_backstrides_impl(const S& shape, const T& strides,
                                 std::index_sequence<I...>) {
...
constexpr R get_strides(const fixed_shape<X...>& shape) noexcept {
  return detail::get_strides_impl<L, R>(
      shape, std::make_index_sequence<sizeof...(X)>{});
}
...
struct xcontainer_inner_types<xfixed_container<ET, S, L, SH, Tag>> {
  ...
  // WORKAROUND FOR MSVC 2015 32 bit, fallback to unaligned container for 0D
  // scalar case
...
class xfixed_container
 ...
 public:
  ...
  explicit xfixed_container(const inner_shape_type& shape, value_type v,
                            layout_type l = L);
 ...
 private:
  ...
  XTENSOR_CONSTEXPR_RETURN const inner_strides_type& strides_impl()
      const noexcept;
  XTENSOR_CONSTEXPR_RETURN const inner_backstrides_type& backstrides_impl()
      const noexcept;
...
template <class ET, class S, layout_type L, bool SH, class Tag>
constexpr typename xfixed_container<ET, S, L, SH, Tag>::inner_shape_type
    xfixed_container<ET, S, L, SH, Tag>::m_shape;

template <class ET, class S, layout_type L, bool SH, class Tag>
constexpr typename xfixed_container<ET, S, L, SH, Tag>::inner_strides_type
    xfixed_container<ET, S, L, SH, Tag>::m_strides;
...
struct xcontainer_inner_types<xfixed_adaptor<EC, S, L, SH, Tag>> {
  ...
  using temporary_type =
      xfixed_container<typename storage_type::value_type, S, L, SH, Tag>;
  static constexpr layout_type layout = L;
};
...
template <class EC, class S, layout_type L, bool SH, class Tag>
class xfixed_adaptor
    : public xcontainer<xfixed_adaptor<EC, S, L, SH, Tag>>,
...
// Out of line definitions to prevent linker errors prior to C++17
template <class EC, class S, layout_type L, bool SH, class Tag>
constexpr typename xfixed_adaptor<EC, S, L, SH, Tag>::inner_shape_type
    xfixed_adaptor<EC, S, L, SH, Tag>::m_shape;

template <class EC, class S, layout_type L, bool SH, class Tag>
constexpr typename xfixed_adaptor<EC, S, L, SH, Tag>::inner_strides_type
    xfixed_adaptor<EC, S, L, SH, Tag>::m_strides;
 ...
 * Note this function is only provided for homogenity, and the shape & layout
 * argument is disregarded (the template shape is always used).
 *
...
inline xfixed_container<ET, S, L, SH, Tag>::xfixed_container(
    const inner_shape_type& shape, layout_type l) {
  (void)(shape);
  (void)(l);
...
template <class ET, class S, layout_type L, bool SH, class Tag>
inline xfixed_container<ET, S, L, SH, Tag>::xfixed_container(
    const inner_shape_type& shape, value_type v, layout_type l) {
  (void)(shape);
  (void)(l);
  XTENSOR_ASSERT(shape.size() == N &&
                 std::equal(shape.begin(), shape.end(), m_shape.begin()));
...
struct check_initializer_list_shape {
  ...
  static bool run(const T& t, const S& shape) {
    ...
    for (std::size_t i = 0; i < shape[IX]; ++i) {
      result = result &&
               check_initializer_list_shape<X - 1>::run(t.begin()[i], shape);
    }
    return result;
  }
};
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline xfixed_container<ET, S, L, SH, Tag>
 ...
 * The type returned by get_init_type_t is raw C array ``value_type[X][Y][Z]``
 * for ``xt::xshape<X, Y, Z>``. C arrays can be initialized with the initializer
 * list syntax, but the size is checked at compile time to prevent errors. Note:
 * for clang < 3.8 this is an initializer_list and the size is not checked at
 * compile-or runtime.
 */
template <class ET, class S, layout_type L, bool SH, class Tag>
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class E>
inline auto xfixed_container<ET, S, L, SH, Tag>::operator=(
...
/**
 * Note that the xfixed_container **cannot** be resized. Attempting to resize
 * with a different size throws an assert in debug mode.
 */
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline void xfixed_container<ET, S, L, SH, Tag>::resize(ST&& shape,
                                                        bool) const {
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline void xfixed_container<ET, S, L, SH, Tag>::resize(
    ST&& shape, const strides_type& strides) const {
  (void)(shape);  // remove unused parameter warning if XTENSOR_ASSERT undefined
  (void)(strides);
  XTENSOR_ASSERT(std::equal(shape.begin(), shape.end(), m_shape.begin()) &&
                 shape.size() == m_shape.size());
  XTENSOR_ASSERT(
      std::equal(strides.begin(), strides.end(), m_strides.begin()) &&
      strides.size() == m_strides.size());
}

/**
 * Note that the xfixed_container **cannot** be reshaped to a shape different
 * from ``S``.
 */
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline auto const& xfixed_container<ET, S, L, SH, Tag>::reshape(
    ST&& shape, layout_type layout) const {
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline bool xfixed_container<ET, S, L, SH, Tag>::broadcast_shape(ST& shape,
                                                                 bool) const {
  return xt::broadcast_shape(m_shape, shape);
}

template <class ET, class S, layout_type L, bool SH, class Tag>
constexpr layout_type xfixed_container<ET, S, L, SH, Tag>::layout()
    const noexcept {
  return base_type::static_layout;
}

template <class ET, class S, layout_type L, bool SH, class Tag>
inline bool xfixed_container<ET, S, L, SH, Tag>::is_contiguous()
    const noexcept {
  using str_type = typename inner_strides_type::value_type;
  return m_strides.empty() ||
         (layout() == layout_type::row_major &&
          m_strides.back() == str_type(1)) ||
         (layout() == layout_type::column_major &&
          m_strides.front() == str_type(1));
}

template <class ET, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_container<ET, S, L, SH, Tag>::storage_impl() noexcept
    -> storage_type& {
  return m_storage;
}

template <class ET, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_container<ET, S, L, SH, Tag>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}

template <class ET, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto xfixed_container<ET, S, L, SH, Tag>::shape_impl()
    const noexcept -> const inner_shape_type& {
  return m_shape;
}

template <class ET, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto
xfixed_container<ET, S, L, SH, Tag>::strides_impl() const noexcept
...
template <class ET, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto
xfixed_container<ET, S, L, SH, Tag>::backstrides_impl() const noexcept
 ...
 */
template <class EC, class S, layout_type L, bool SH, class Tag>
inline xfixed_adaptor<EC, S, L, SH, Tag>::xfixed_adaptor(
...
template <class EC, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_adaptor<EC, S, L, SH, Tag>::operator=(
...
template <class EC, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_adaptor<EC, S, L, SH, Tag>::operator=(xfixed_adaptor&& rhs)
    -> self_type& {
...
template <class EC, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_adaptor<EC, S, L, SH, Tag>::operator=(temporary_type&& rhs)
    -> self_type& {
...
/**
 * Note that the xfixed_adaptor **cannot** be resized. Attempting to resize with
 * a different size throws an assert in debug mode.
 */
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline void xfixed_adaptor<ET, S, L, SH, Tag>::resize(ST&& shape,
                                                      layout_type l) const {
  (void)(shape);  // remove unused parameter warning if XTENSOR_ASSERT undefined
  (void)(l);
  XTENSOR_ASSERT(std::equal(shape.begin(), shape.end(), m_shape.begin()) &&
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline void xfixed_adaptor<ET, S, L, SH, Tag>::resize(
    ST&& shape, const strides_type& strides) const {
  (void)(shape);  // remove unused parameter warning if XTENSOR_ASSERT undefined
  (void)(strides);
  XTENSOR_ASSERT(std::equal(shape.begin(), shape.end(), m_shape.begin()) &&
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline auto const& xfixed_adaptor<ET, S, L, SH, Tag>::reshape(
    ST&& shape, layout_type layout) const {
  if (!(std::equal(shape.begin(), shape.end(), m_shape.begin()) &&
        shape.size() == m_shape.size() && layout == L)) {
    XTENSOR_THROW(
...
template <class ET, class S, layout_type L, bool SH, class Tag>
template <class ST>
inline bool xfixed_adaptor<ET, S, L, SH, Tag>::broadcast_shape(ST& shape,
                                                               bool) const {
  return xt::broadcast_shape(m_shape, shape);
}

template <class EC, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_adaptor<EC, S, L, SH, Tag>::storage_impl() noexcept
    -> storage_type& {
  return m_storage;
}

template <class EC, class S, layout_type L, bool SH, class Tag>
inline auto xfixed_adaptor<EC, S, L, SH, Tag>::storage_impl() const noexcept
    -> const storage_type& {
  return m_storage;
}

template <class EC, class S, layout_type L, bool SH, class Tag>
constexpr layout_type xfixed_adaptor<EC, S, L, SH, Tag>::layout()
    const noexcept {
  return base_type::static_layout;
}
...
template <class EC, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto xfixed_adaptor<EC, S, L, SH, Tag>::shape_impl()
    const noexcept -> const inner_shape_type& {
  return m_shape;
}

template <class EC, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto xfixed_adaptor<EC, S, L, SH, Tag>::strides_impl()
    const noexcept -> const inner_strides_type& {
  return m_strides;
}

template <class EC, class S, layout_type L, bool SH, class Tag>
XTENSOR_CONSTEXPR_RETURN auto
xfixed_adaptor<EC, S, L, SH, Tag>::backstrides_impl() const noexcept
    -> const inner_backstrides_type& {
  return m_backstrides;
}
}  // namespace xt
