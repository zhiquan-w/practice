...
void xchunked_view<E>::init() {
  ...
  std::transform(m_shape.cbegin(), m_shape.cend(), m_chunk_shape.cbegin(),
                 m_grid_shape.begin(), [](auto s, auto cs) {
                   std::size_t cn = s / cs;
                   if (s % cs > 0) {
                     cn++;  // edge_chunk
                   }
  ...
  m_chunk_nb = std::accumulate(std::begin(m_grid_shape), std::end(m_grid_shape),
...
xchunked_view<E>::operator=(const OE& e) {
  ...
  for (auto it1 = chunk_begin(); it1 != end1; ++it1, ++it2) {
    ...
    if (lhs_shape != el2.shape()) {
      xstrided_slice_vector esv(
          el2.dimension());  // element slice in edge chunk
...
template <class E>
inline auto xchunked_view<E>::chunk_shape() const noexcept
    -> const shape_type& {
...
template <class E>
inline auto xchunked_view<E>::expression() const noexcept
