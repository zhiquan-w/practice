...
class uvector {
 public:
  ...
  uvector(size_type count, const_reference value,
          const allocator_type& alloc = allocator_type());

  template <class InputIt, class = detail::require_input_iter<InputIt>>
  uvector(InputIt first, InputIt last,
          const allocator_type& alloc = allocator_type());
...
inline void safe_destroy_deallocate(
    A& alloc, typename std::allocator_traits<A>::pointer ptr,
...
template <class T, class A>
inline uvector<T, A>::uvector(size_type count, const_reference value,
...
inline uvector<T, A>::uvector(InputIt first, InputIt last,
                              const allocator_type& alloc)
    : m_allocator(alloc), p_begin(nullptr), p_end(nullptr) {
...
template <class T, std::size_t N = 4, class A = std::allocator<T>,
          bool Init = true>
class svector {
 public:
  ...
  svector(size_type n, const value_type& v,
          const allocator_type& alloc = allocator_type());
  svector(std::initializer_list<T> il,
          const allocator_type& alloc = allocator_type());
  ...
  svector& operator=(svector&& rhs) noexcept(
...
inline svector<T, N, A, Init>::svector(IT begin, IT end,
                                       const allocator_type& alloc)
    : m_allocator(alloc) {
  assign(begin, end);
}
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(size_type n, const value_type& v,
                                       const allocator_type& alloc)
    : m_allocator(alloc) {
  assign(n, v);
}
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(std::initializer_list<T> il,
                                       const allocator_type& alloc)
    : m_allocator(alloc) {
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>&
svector<T, N, A, Init>::operator=(svector&& rhs) noexcept(
    std::is_nothrow_move_assignable<value_type>::value) {
  assign(rhs.begin(), rhs.end());
  return *this;
}
...
template <class T, std::size_t N, class A, bool Init>
inline svector<T, N, A, Init>::svector(svector&& rhs) noexcept(
    std::is_nothrow_move_constructible<value_type>::value) {
  this->swap(rhs);
}
...
template <class T, std::size_t N, class A, bool Init>
inline auto svector<T, N, A, Init>::operator[](size_type idx) const
    -> const_reference {
  return m_begin[idx];
}
...
template <class T, std::size_t N, class A, bool Init>
inline auto svector<T, N, A, Init>::get_allocator() const noexcept
    -> allocator_type {
  return m_allocator;
}
...
template <class T, std::size_t N, class A, bool Init>
inline auto svector<T, N, A, Init>::erase(const_iterator cfirst,
                                          const_iterator clast) -> iterator {
...
template <class T, std::size_t N, class A, bool Init>
inline auto svector<T, N, A, Init>::insert(const_iterator cit, const T& elt)
    -> iterator {
  auto it = const_cast<pointer>(cit);
  if (it == m_end) {
    push_back(elt);
...
template <class T, std::size_t N, class A, bool Init>
template <class It>
inline auto svector<T, N, A, Init>::insert(const_iterator pos, It first,
                                           It last) -> iterator {
...
template <class T, std::size_t N, class A, bool Init>
inline auto svector<T, N, A, Init>::insert(const_iterator pos,
...
template <class T, std::size_t N, class A, bool Init>
inline bool operator==(const std::vector<T>& lhs,
...
template <class T, std::size_t N, class A, bool Init>
inline bool operator==(const svector<T, N, A, Init>& lhs,
                       const std::vector<T>& rhs) {
  return lhs.size() == rhs.size() &&
         std::equal(lhs.begin(), lhs.end(), rhs.begin());
}
...
template <class T, std::size_t N, class A, bool Init>
inline void swap(svector<T, N, A, Init>& lhs,
                 svector<T, N, A, Init>& rhs) noexcept {
  lhs.swap(rhs);
}
...
/**
 * This array class is modeled after ``std::array`` but adds optional alignment
...
class alignas(Align) aligned_array : public std::array<T, N> {
 public:
  // Note: this is for alignment detection. The allocator serves no other
  // purpose than
  //       that of a trait here.
  using allocator_type =
      std::conditional_t<Align != 0, xt_simd::aligned_allocator<T, Align>,
                         std::allocator<T>>;
};
...
struct array_traits<T, 0> {
  ...
  static constexpr T& ref(const storage_type& /*t*/,
...
template <class T, std::size_t N>
inline bool operator==(const const_array<T, N>& lhs,
...
template <class E, std::ptrdiff_t Start, std::ptrdiff_t End>
template <std::ptrdiff_t OS, std::ptrdiff_t OE>
sequence_view<E, Start, End>::sequence_view(
...
template <class E, std::ptrdiff_t Start, std::ptrdiff_t End>
auto sequence_view<E, Start, End>::operator[](std::size_t idx) const
    -> const_reference {
...
inline bool operator==(const sequence_view<T, TB, TE>& lhs,
