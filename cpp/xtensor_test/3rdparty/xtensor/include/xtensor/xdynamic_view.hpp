...
struct xcontainer_inner_types<xdynamic_view<CT, S, L, FST>> {
  ...
  using temporary_type =
      xarray<std::decay_t<typename xexpression_type::value_type>,
...
struct xiterable_inner_types<xdynamic_view<CT, S, L, FST>> {
  ...
  using const_stepper =
      xindexed_stepper<const xdynamic_view<CT, S, L, FST>, true>;
...
template <class CT, class S, layout_type L = layout_type::dynamic,
          class FST =
              detail::flat_storage_getter<CT, XTENSOR_DEFAULT_TRAVERSAL>>
class xdynamic_view : public xview_semantic<xdynamic_view<CT, S, L, FST>>,
 ...
 public:
  ...
  using slice_type =
      xtl::variant<detail::xfake_slice<strides_vt>, xkeep_slice<strides_vt>,
                   xdrop_slice<strides_vt>>;
  using slice_vector_type = std::vector<slice_type>;

  template <class CTA, class SA>
  xdynamic_view(CTA&& e, SA&& shape, get_strides_t<S>&& strides,
  ...
  using container_iterator =
      std::conditional_t<is_const, typename storage_type::const_iterator,
 ...
 private:
  ...
  const_container_iterator data_xend(layout_type l,
  ...
  offset_type adjust_offset(offset_type offset, T idx,
                            Args... args) const noexcept;
  offset_type adjust_offset(offset_type offset) const noexcept;

  template <class T, class... Args>
  offset_type adjust_offset_impl(offset_type offset, size_type idx_offset,
                                 T idx, Args... args) const noexcept;
  offset_type adjust_offset_impl(offset_type offset,
                                 size_type idx_offset) const noexcept;

  template <class It>
  offset_type adjust_element_offset(offset_type offset, It first,
...
using xdynamic_slice =
    xtl::variant<T,
                 ...
                 xrange_adaptor<placeholders::xtuph, placeholders::xtuph,
                                placeholders::xtuph>,

                 xrange<T>, xstepped_range<T>,

                 xkeep_slice<T>, xdrop_slice<T>,

                 xall_tag, xellipsis_tag, xnewaxis_tag>;
...
class xfake_slice : public xslice<xfake_slice<T>> {
 public:
  ...
  size_type operator()(size_type /*i*/) const noexcept { return size_type(0); }

  size_type size() const noexcept { return size_type(1); }

  size_type step_size() const noexcept { return size_type(0); }

  size_type step_size(std::size_t /*i*/, std::size_t /*n*/ = 1) const noexcept {
    return size_type(0);
  }

  size_type revert_index(std::size_t i) const noexcept { return i; }

  bool contains(size_type /*i*/) const noexcept { return true; }

  bool operator==(const self_type& /*rhs*/) const noexcept { return true; }

  bool operator!=(const self_type& /*rhs*/) const noexcept { return false; }
};
}  // namespace detail
...
inline xdynamic_view<CT, S, L, FST>::xdynamic_view(
    CTA&& e, SA&& shape, get_strides_t<S>&& strides, std::size_t offset,
    layout_type layout, slice_vector_type&& slices,
    get_strides_t<S>&& adj_strides) noexcept
    : base_type(std::forward<CTA>(e), std::forward<SA>(shape),
                std::move(strides), offset, layout),
      m_slices(std::move(slices)),
      m_adj_strides(std::move(adj_strides)) {}

template <class CT, class S, layout_type L, class FST>
template <class E>
inline auto xdynamic_view<CT, S, L, FST>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::operator()() const
    -> const_reference {
  return base_type::storage()[data_offset()];
}
...
template <class CT, class S, layout_type L, class FST>
template <class... Args>
inline auto xdynamic_view<CT, S, L, FST>::operator()(Args... args)
    -> reference {
...
template <class CT, class S, layout_type L, class FST>
template <class... Args>
inline auto xdynamic_view<CT, S, L, FST>::operator()(Args... args) const
    -> const_reference {
  XTENSOR_TRY(check_index(base_type::shape(), args...));
  XTENSOR_CHECK_DIMENSION(base_type::shape(), args...);
  offset_type offset = base_type::compute_index(args...);
...
template <class CT, class S, layout_type L, class FST>
template <class O>
inline bool xdynamic_view<CT, S, L, FST>::has_linear_assign(
    const O&) const noexcept {
  return false;
}
...
template <class CT, class S, layout_type L, class FST>
template <class... Args>
inline auto xdynamic_view<CT, S, L, FST>::unchecked(Args... args) const
    -> const_reference {
  offset_type offset = base_type::compute_unchecked_index(args...);
  offset = adjust_offset(args...);
  return base_type::storage()[static_cast<size_type>(offset)];
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::flat(size_type i) const
    -> const_reference {
  return base_type::storage()[data_offset() + i];
}
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline auto xdynamic_view<CT, S, L, FST>::element(It first, It last)
    -> reference {
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline auto xdynamic_view<CT, S, L, FST>::element(It first, It last) const
    -> const_reference {
  XTENSOR_TRY(check_element_index(base_type::shape(), first, last));
  offset_type offset = base_type::compute_element_index(first, last);
  offset = adjust_element_offset(offset, first, last);
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::data_offset() const noexcept
    -> size_type {
...
template <class CT, class S, layout_type L, class FST>
template <class ST>
inline auto xdynamic_view<CT, S, L, FST>::stepper_begin(const ST& shape)
    -> stepper {
...
template <class CT, class S, layout_type L, class FST>
template <class ST>
inline auto xdynamic_view<CT, S, L, FST>::stepper_end(const ST& shape,
                                                      layout_type /*l*/)
    -> stepper {
  size_type offset = shape.size() - dimension();
...
template <class CT, class S, layout_type L, class FST>
template <class ST>
inline auto xdynamic_view<CT, S, L, FST>::stepper_begin(const ST& shape) const
    -> const_stepper {
  size_type offset = shape.size() - dimension();
...
template <class CT, class S, layout_type L, class FST>
template <class E>
inline auto xdynamic_view<CT, S, L, FST>::build_view(E&& e) const
    -> rebind_t<E> {
  ...
  return rebind_t<E>(std::forward<E>(e), std::move(sh), std::move(str),
                     base_type::data_offset(), this->layout(), std::move(svt),
                     std::move(adj_str));
}

template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::data_xbegin() noexcept
    -> container_iterator {
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::data_xend(layout_type l,
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::data_xend(
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline It xdynamic_view<CT, S, L, FST>::data_xbegin_impl(
...
template <class CT, class S, layout_type L, class FST>
template <class It>
inline It xdynamic_view<CT, S, L, FST>::data_xend_impl(
    It begin, layout_type l, size_type offset) const noexcept {
  return strided_data_end(*this, begin + std::ptrdiff_t(data_offset()), l,
                          offset);
}

template <class CT, class S, layout_type L, class FST>
inline void xdynamic_view<CT, S, L, FST>::assign_temporary_impl(
    temporary_type&& tmp) {
...
template <class CT, class S, layout_type L, class FST>
template <class T, class... Args>
inline auto xdynamic_view<CT, S, L, FST>::adjust_offset(
    offset_type offset, T idx, Args... args) const noexcept -> offset_type {
  constexpr size_type nb_args = sizeof...(Args) + 1;
  size_type dim = base_type::dimension();
  offset_type res =
      nb_args > dim ? adjust_offset(offset, args...)
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::adjust_offset(
    offset_type offset) const noexcept -> offset_type {
  return offset;
}
...
inline auto xdynamic_view<CT, S, L, FST>::adjust_offset_impl(
    offset_type offset, size_type idx_offset, T idx,
    Args... args) const noexcept -> offset_type {
...
template <class CT, class S, layout_type L, class FST>
inline auto xdynamic_view<CT, S, L, FST>::adjust_offset_impl(
    offset_type offset, size_type) const noexcept -> offset_type {
  return offset;
}

template <class CT, class S, layout_type L, class FST>
template <class It>
inline auto xdynamic_view<CT, S, L, FST>::adjust_element_offset(
    offset_type offset, It first, It last) const noexcept -> offset_type {
  ...
  for (offset_type i = loop_offset; i < dim; ++i, ++first) {
    ...
    offset_type sl_offset = xtl::visit(
        [j](const auto& sl) { return static_cast<offset_type>(sl(j)); },
...
struct adj_strides_policy {
 ...
 protected:
  ...
  bool fill_args(const xdynamic_slice_vector& slices, std::size_t sl_idx,
                 std::size_t i, std::size_t old_shape, const ST& old_stride,
...
inline auto dynamic_view(E&& e, const xdynamic_slice_vector& slices) {
  ...
  args.fill_args(e.shape(), detail::get_strides<XTENSOR_DEFAULT_TRAVERSAL>(e),
                 detail::get_offset<XTENSOR_DEFAULT_TRAVERSAL>(e), e.layout(),
                 slices);
  return view_type(std::forward<E>(e), std::move(args.new_shape),
                   std::move(args.new_strides), args.new_offset,
                   args.new_layout, std::move(args.new_slices),
