...
template <class T, class S, class E = random::default_engine_type>
auto randint(const S& shape, T lower = 0,
...
template <class T, class S, class D = double,
...
template <class T, class I, std::size_t L,
          class E = random::default_engine_type>
auto randint(const I (&shape)[L], T lower = 0,
             T upper = (std::numeric_limits<T>::max)(),
             E& engine = random::get_default_random_engine());
...
template <class T, class I, std::size_t L, class D = double,
          class E = random::default_engine_type>
...
void shuffle(xexpression<T>& e,
             E& engine = random::get_default_random_engine());

template <class T, class E = random::default_engine_type>
std::enable_if_t<xtl::is_integral<T>::value, xtensor<T, 1>> permutation(
...
xtensor<typename T::value_type, 1> choice(
    const xexpression<T>& e, std::size_t n, bool replace = true,
    E& engine = random::get_default_random_engine());

template <class T, class W, class E = random::default_engine_type>
xtensor<typename T::value_type, 1> choice(
    const xexpression<T>& e, std::size_t n, const xexpression<W>& weights,
    bool replace = true, E& engine = random::get_default_random_engine());
}  // namespace random
 ...
 * numbers in the interval from @p lower to @p upper, excluding upper.
 *
...
/**
 * xexpression with specified @p shape containing numbers sampled from
 * a negative binomial random number distribution (also known as Pascal
 * distribution) that returns the number of successes before @p k trials with
 * probability of success equal to @p prob for each of the Bernoulli trials.
 *
...
xtensor<typename T::value_type, 1> choice(const xexpression<T>& e,
  ...
  if (replace) {
    ...
    size_type i = n;
    for (auto it = de.storage().begin() + n; it != de.storage().end();
         ++it, ++i) {
 ...
 *
 * Randomly sample n unique elements from xexpression ``e`` using the discrete
 * distribution parametrized by the weights ``w``. When sampling with
 * replacement, this means that the probability to sample element ``e[i]`` is
 * defined as
 * ``w[i] / sum(w)``.
 * Without replacement, this only describes the probability of the first sample
 * element. In successive samples, the weight of items already sampled is
 * assumed to be zero.
 *
 * For weighted random sampling with replacement, binary search with cumulative
 * weights alogrithm is used. For weighted random sampling without replacement,
 * the algorithm used is the exponential sort from [Efraimidis and
 * Spirakis](https://doi.org/10.1016/j.ipl.2005.11.003) (2006) with the ``weight
 * / randexp(1)``
 * [trick](https://web.archive.org/web/20201021162211/https://krlmlr.github.io/wrswoR/)
 * from Kirill Müller.
 *
 * Note: this function makes a copy of your data, and only 1D data is accepted.
 *
...
xtensor<typename T::value_type, 1> choice(const xexpression<T>& e,
  ...
  if (replace) {
    ...
    // The index idx of the sampled element is such that weight_cumul[idx - 1]
    // <= u < weight_cumul[idx]. Where weight_cumul[-1] is implicitly 0, as the
    // empty sum.
    const auto wc = eval(cumsum(dweights));
    std::uniform_real_distribution<weight_type> weight_dist{0,
                                                            wc[wc.size() - 1]};
    for (auto& x : result) {
