...
struct is_strided_view
    : std::integral_constant<
          bool,
          xtl::conjunction<has_data_interface<E>,
...
struct is_contiguous_view_impl<layout_type::row_major, valid, all_seen,
                               range_seen, V> {
  ...
  static constexpr bool value =
                            is_contiguous_view_impl < layout_type::row_major,
...
struct is_contiguous_view_impl<layout_type::column_major, valid, int_seen,
                               range_seen, V> {
  using slice = xtl::mpl::front_t<V>;
  static constexpr bool is_range_slice = is_xrange<slice>::value;
  static constexpr bool is_int_slice = xtl::is_integral<slice>::value;
  ...
  static constexpr bool value =
                            is_contiguous_view_impl < layout_type::column_major,
...
struct unwrap_offset_container<layout_type::row_major,
                               sequence_view<T, start, end>, offset> {
...
struct get_contigous_shape_type {
  ...
  using type = std::conditional_t<
      ...
      // In the false branch we know that we have only integers at the front OR
      // end, and NO range
...
struct xcontainer_inner_types<xview<CT, S...>> {
  ...
  using extract_storage_type =
      xtl::mpl::eval_if_t<has_data_interface<xexpression_type>,
                          ...
                          make_invalid_type<>>;
  using storage_type = std::conditional_t<is_const, const extract_storage_type,
...
struct xiterable_inner_types<xview<CT, S...>> {
  ...
  using stepper = std::conditional_t<
      ...
      xview_stepper<std::is_const<std::remove_reference_t<CT>>::value, CT,
                    S...>>;

  using const_stepper =
      std::conditional_t<is_strided_view, xstepper<const xview<CT, S...>>,
...
class xview
    : public xview_semantic<xview<CT, S...>>,
      public std::conditional_t<
          ...
          xcontiguous_iterable<xview<CT, S...>>, xiterable<xview<CT, S...>>>,
 ...
 public:
  ...
  using pointer =
      std::conditional_t<is_const, typename xexpression_type::const_pointer,
  ...
  using inner_backstrides_type = std::conditional_t<
      ...
      typename detail::unwrap_offset_container<
          xexpression_type::static_layout, xexpression_inner_backstrides_type,
          integral_count<S...>()>::type,
      get_strides_t<shape_type>>;
  ...
  using const_storage_iterator =
      std::conditional_t<has_data_interface<xexpression_type>::value &&
                             is_strided_view,
                         typename xexpression_type::const_storage_iterator,
  ...
  template <class ST, bool Enable = is_strided_view>
  std::enable_if_t<!Enable, stepper> stepper_end(const ST& shape,
                                                 layout_type l);

  template <class ST, bool Enable = is_strided_view>
  std::enable_if_t<!Enable, const_stepper> stepper_begin(const ST& shape) const;
  template <class ST, bool Enable = is_strided_view>
  std::enable_if_t<!Enable, const_stepper> stepper_end(const ST& shape,
                                                       layout_type l) const;
  ...
  template <class T = xexpression_type>
  std::enable_if_t<has_data_interface<T>::value, const storage_type&> storage()
      const;
  ...
  template <class It>
  inline It data_xend_impl(It begin, layout_type l,
  ...
  inline const_container_iterator data_xend(layout_type l,
                                            size_type offset) const noexcept;
  ...
  template <class E, class T = xexpression_type,
            class = std::enable_if_t<
                has_data_interface<T>::value && is_contiguous_view, int>>
  ...
  enable_simd_interface<T, simd_return_type<requested_type>> load_simd(
      size_type i) const;
 ...
 private:
  ...
  template <class CTA, class FSL, class... SL>
  explicit xview(std::true_type, CTA&& e, FSL&& first_slice,
                 SL&&... slices) noexcept;

  template <class CTA, class FSL, class... SL>
  explicit xview(std::false_type, CTA&& e, FSL&& first_slice,
                 SL&&... slices) noexcept;
  ...
  template <typename std::decay_t<CT>::size_type I, class T, class Arg,
            class... Args>
...
class xview_stepper {
 public:
  ...
  xview_stepper(view_type* view, substepper_type it, size_type offset,
                bool end = false, layout_type l = XTENSOR_DEFAULT_TRAVERSAL);
...
struct xview_shape_type<fixed_shape<I...>, S...> {
  using type = typename xview_shape_type<std::array<std::size_t, sizeof...(I)>,
                                         S...>::type;
};
...
xview<CT, S...>::xview(CTA&& e, FSL&& first_slice, SL&&... slices) noexcept
    : xview(std::integral_constant<bool, has_trivial_strides>{},
            std::forward<CTA>(e), std::forward<FSL>(first_slice),
            std::forward<SL>(slices)...) {}

// trivial strides initializer
template <class CT, class... S>
template <class CTA, class FSL, class... SL>
xview<CT, S...>::xview(std::true_type, CTA&& e, FSL&& first_slice,
                       SL&&... slices) noexcept
...
template <class CT, class... S>
template <class CTA, class FSL, class... SL>
xview<CT, S...>::xview(std::false_type, CTA&& e, FSL&& first_slice,
                       SL&&... slices) noexcept
    : m_e(std::forward<CTA>(e)),
      m_slices(std::forward<FSL>(first_slice), std::forward<SL>(slices)...),
  ...
  return xtl::mpl::static_if<is_strided_view>(
      [&](auto self) {
        if (static_layout != layout_type::dynamic) {
          ...
          bool strides_match =
              do_strides_match(self(this)->shape(), self(this)->strides(),
                               self(this)->m_e.layout(), true);
          return strides_match ? self(this)->m_e.layout()
                               : layout_type::dynamic;
        }
      },
...
inline void xview<CT, S...>::fill(const T& value) {
  xtl::mpl::static_if<static_layout != layout_type::dynamic>(
      [&](auto self) {
        std::fill(self(this)->storage_begin(), self(this)->storage_end(),
                  value);
      },
      /*else*/
      [&](auto self) {
...
inline auto xview<CT, S...>::operator()(Args... args) -> reference {
  ...
  // methods with signed integers, leading to warning about signed/unsigned
  // conversions in the deeper layers of the access methods
 ...
 * of dimensions of the view, else the behavior is undefined.
 *
...
inline auto xview<CT, S...>::unchecked(Args... args) -> reference {
  return unchecked_impl(make_index_sequence(args...),
 ...
 */
template <class CT, class... S>
inline auto xview<CT, S...>::expression() const noexcept
 ...
 * realized container). ``xt::eval`` will make sure that the underlying
 * xexpression is on a realized container.
 */
template <class CT, class... S>
template <class T>
...
template <class CT, class... S>
inline auto
xview<CT, S...>::operator&() & -> xtl::xclosure_pointer<self_type&> {
  return xtl::closure_pointer(*this);
}

template <class CT, class... S>
inline auto xview<CT, S...>::operator&()
    const& -> xtl::xclosure_pointer<const self_type&> {
  return xtl::closure_pointer(*this);
}
  ...
  return xtl::mpl::static_if<is_strided_view>(
      [&](auto self) {
        return str.size() == self(this)->strides().size() &&
               std::equal(str.cbegin(), str.cend(),
                          self(this)->strides().begin());
      },
...
template <class CT, class... S>
inline auto xview<CT, S...>::data_xbegin() const noexcept
...
template <class CT, class... S>
inline auto xview<CT, S...>::data_xend(layout_type l,
                                       size_type offset) const noexcept
    -> const_container_iterator {
  return data_xend_impl(data() + data_offset(), l, offset);
}
...
template <class CT, class... S>
template <class E, std::size_t... I>
inline auto xview<CT, S...>::build_view_impl(E&& e,
...
inline auto xview<CT, S...>::build_view(E&& e) const -> rebind_t<E> {
  return build_view_impl(std::forward<E>(e),
...
inline auto xview<CT, S...>::load_simd(size_type i) const
  ...
  return m_e.template load_simd<xt_simd::unaligned_mode, requested_type>(
      data_offset() + i);
}

template <class CT, class... S>
template <class T>
inline auto xview<CT, S...>::data_element(size_type i)
...
template <class CT, class... S>
template <class T>
inline auto xview<CT, S...>::flat(size_type i)
    -> enable_simd_interface<T, reference> {
...
template <class CT, class... S>
template <std::size_t... I>
inline auto xview<CT, S...>::compute_strides_impl(
    std::index_sequence<I...>) const noexcept {
  ...
  return std::array<std::ptrdiff_t, sizeof...(I)>(
      {(static_cast<std::ptrdiff_t>(
        ...
        ((integral_skip<S...>(I) -
          newaxis_count_before<S...>(integral_skip<S...>(I))) < original_dim
...
inline void xview<CT, S...>::compute_strides(std::false_type) const {
  ...
  auto slice_strides =
      compute_strides_impl(std::make_index_sequence<n_strides>());
  ...
  for (std::size_t i = n_strides; i < this->dimension(); ++i) {
    m_strides[i] =
        m_e.strides()[i + integral_count<S...>() - newaxis_count<S...>()];
    detail::adapt_strides(shape(), m_strides, &m_backstrides, i);
  }
...
template <class CT, class... S>
template <class Arg, class... Args>
inline auto xview<CT, S...>::access(Arg arg, Args... args) const
    -> const_reference {
  if (sizeof...(Args) >= this->dimension()) {
    return access(args...);
  }
...
template <class CT, class... S>
template <typename std::decay_t<CT>::size_type... I, class... Args>
inline auto xview<CT, S...>::unchecked_impl(std::index_sequence<I...>,
                                            Args... args) -> reference {
...
template <typename std::decay_t<CT>::size_type I, class T, class Arg,
          class... Args>
inline auto xview<CT, S...>::sliced_access(const xslice<T>& slice, Arg arg,
...
inline auto xview<CT, S...>::make_index(It first, It last) const
  ...
  for (size_type i = 0; i != m_e.dimension(); ++i) {
    ...
    first = first_copy;
    std::advance(
        first, static_cast<diff_type>(k - xt::integral_count_before<S...>(i)));

    if (first < last) {
      index[i] = k < sizeof...(S) ? apply<size_type>(k, func1, m_slices)
                                  : static_cast<ivalue_type>(*first);
    } else {
      index[i] = k < sizeof...(S) ? apply<size_type>(k, func2, m_slices)
                                  : ivalue_type(0);
    }
  }
  return index;
...
template <class CT, class... S>
inline auto xview<CT, S...>::compute_shape(std::true_type) const
    -> inner_shape_type {
  return inner_shape_type(m_e.shape());
}

template <class CT, class... S>
inline auto xview<CT, S...>::compute_shape(std::false_type) const
    -> inner_shape_type {
  std::size_t dim =
      m_e.dimension() - integral_count<S...>() + newaxis_count<S...>();
...
inline void run_assign_temporary_impl(
    V& v, const T& t, std::true_type /* enable strided assign */) {
  strided_loop_assigner<true>::run(v, t);
}

template <class V, class T>
inline void run_assign_temporary_impl(
    V& v, const T& t, std::false_type /* fallback to iterator assign */) {
...
inline auto make_view_impl(E&& e, std::index_sequence<I...>, S&&... slices) {
  ...
  using view_type =
      xview<xtl::closure_type_t<E>, get_slice_type<std::decay_t<E>, S>...>;
  return view_type(
      std::forward<E>(e),
...
class row_impl {
 ...
 private:
  ...
  static inline void check_dimension(const S& shape) {
    if (shape.size() != 2) {
      XTENSOR_THROW(std::invalid_argument,
                    "A row can only be accessed on an expression with exact "
                    "two dimensions");
    }
  }
  ...
  static inline void check_dimension(const std::array<T, N>&) {
    static_assert(N == 2,
                  "A row can only be accessed on an expression with exact two "
                  "dimensions");
  }
};
...
class column_impl {
 ...
 private:
  ...
  static inline void check_dimension(const S& shape) {
    if (shape.size() != 2) {
      XTENSOR_THROW(std::invalid_argument,
                    "A column can only be accessed on an expression with exact "
                    "two dimensions");
    }
  }
...
template <bool is_const, class CT, class... S>
template <class F>
void xview_stepper<is_const, CT, S...>::common_step_forward(size_type dim,
                                                            F f) {
  if (dim >= m_offset) {
    ...
    if (!is_newaxis_slice(index)) {
      size_type step_size =
          index < sizeof...(S) ? apply<size_type>(index, func, p_view->slices())
                               : 1;
...
template <bool is_const, class CT, class... S>
template <class F>
void xview_stepper<is_const, CT, S...>::common_step_backward(size_type dim,
                                                             F f) {
  if (dim >= m_offset) {
    auto func = [&dim, this](const auto& s) noexcept {
...
void xview_stepper<is_const, CT, S...>::common_reset(size_type dim, F f,
  ...
  if (!is_newaxis_slice(index)) {
    if (dim < m_index_keeper.size()) {
      size_type size = index < sizeof...(S) ? apply<size_type>(index, size_func,
                                                               p_view->slices())
                                            : p_view->shape()[dim];
