...
class xrange : public xslice<xrange<T>> {
 public:
  ...
  template <class S,
            typename = std::enable_if_t<std::is_convertible<S, T>::value, void>>
...
class xstepped_range : public xslice<xstepped_range<T>> {
 public:
  ...
  xstepped_range(size_type start_val, size_type stop_val,
 ...
 * @sa strided_view
 */
inline auto ellipsis() noexcept { return xellipsis_tag(); }
...
using disable_integral_keep =
    std::enable_if_t<!xtl::is_integral<std::decay_t<T>>::value,
                     xkeep_slice<typename std::decay_t<T>::value_type>>;

template <class T, class R>
using enable_integral_keep =
    std::enable_if_t<xtl::is_integral<T>::value, xkeep_slice<R>>;
}  // namespace detail
...
inline detail::disable_integral_keep<T> keep(T&& indices) {
  return xkeep_slice<typename std::decay_t<T>::value_type>(
...
inline xkeep_slice<R> keep(Arg0 i0, Arg1 i1, Args... args) {
  using slice_type = xkeep_slice<R>;
  using container_type = typename slice_type::container_type;
  container_type tmp = {static_cast<R>(i0), static_cast<R>(i1),
                        static_cast<R>(args)...};
  return slice_type(std::move(tmp));
}
...
template <class T, class R>
using enable_integral_drop =
    std::enable_if_t<xtl::is_integral<T>::value, xdrop_slice<R>>;
}  // namespace detail
...
inline detail::disable_integral_drop<T> drop(T&& indices) {
  return xdrop_slice<typename std::decay_t<T>::value_type>(
      std::forward<T>(indices));
}
...
struct xrange_adaptor {
  ...
  get(std::size_t size) const {
    return get_stepped_range(
        m_step > 0 ? 0 : static_cast<std::ptrdiff_t>(size) - 1, m_stop, m_step,
        size);
  }
  ...
  get(std::size_t size) const {
    ...
    return get_stepped_range(m_start, m_step > 0 ? sz : -(sz + 1), m_step,
                             size);
  }
 ...
 private:
  ...
  static auto get_stepped_range(std::ptrdiff_t start, std::ptrdiff_t stop,
                                std::ptrdiff_t step, std::size_t ssize) {
    std::ptrdiff_t size = static_cast<std::ptrdiff_t>(ssize);
...
XTENSOR_CONSTEXPR xtuph get_tuph_or_val(std::ptrdiff_t /*val*/,
                                        std::true_type) {
  return xtuph();
}

XTENSOR_CONSTEXPR std::ptrdiff_t get_tuph_or_val(std::ptrdiff_t val,
                                                 std::false_type) {
  return val;
}
...
struct rangemaker<A, B> {
  XTENSOR_CONSTEXPR operator xrange_adaptor<A, B, xt::placeholders::xtuph>() {
    return xrange_adaptor<A, B, xt::placeholders::xtuph>(
        {get_tuph_or_val(rng[0], std::is_same<A, xtuph>()),
         get_tuph_or_val(rng[1], std::is_same<B, xtuph>()), xtuph()});
  }
...
struct cast_if_integer<T, std::enable_if_t<xtl::is_integral<T>::value>> {
  ...
  type operator()(T t) { return static_cast<type>(t); }
};
 ...
 * You can use the shorthand `_` syntax to select from the start or until the
 * end.
 *
 * \code{.cpp}
...
inline auto range(A start_val, B stop_val) {
  return xrange_adaptor<detail::cast_if_integer_t<A>,
                        detail::cast_if_integer_t<B>, placeholders::xtuph>(
      detail::cast_if_integer<A>{}(start_val),
      detail::cast_if_integer<B>{}(stop_val), placeholders::xtuph());
}
...
template <class S>
inline auto step_size(const xslice<S>& slice, std::size_t idx,
...
struct slice_implementation_getter {
  ...
  inline decltype(auto) operator()(E& e, SL&& slice, std::size_t index) const {
    return get_slice(e, std::forward<SL>(slice), index,
                     xtl::is_signed<std::decay_t<SL>>());
  }

 private:
  template <class E, class SL>
  inline decltype(auto) get_slice(E&, SL&& slice, std::size_t,
                                  std::false_type) const {
    return std::forward<SL>(slice);
  }

  template <class E, class SL>
  inline decltype(auto) get_slice(E& e, SL&& slice, std::size_t index,
                                  std::true_type) const {
    using int_type = std::decay_t<SL>;
    return slice < int_type(0)
               ? slice + static_cast<std::ptrdiff_t>(e.shape(index))
               : std::ptrdiff_t(slice);
  }
};
...
template <class T>
inline auto xrange<T>::step_size(std::size_t /*i*/,
                                 std::size_t n) const noexcept -> size_type {
...
template <class T>
inline auto xstepped_range<T>::step_size(std::size_t /*i*/,
...
template <class T>
inline auto xstepped_range<T>::revert_index(std::size_t i) const noexcept
    -> size_type {
...
inline bool xstepped_range<T>::contains(size_type i) const noexcept {
  return i >= m_start && i < m_start + m_size * m_step &&
         ((i - m_start) % m_step == 0);
}
...
template <class T>
inline auto xall<T>::step_size(std::size_t /*i*/, std::size_t n) const noexcept
    -> size_type {
  return static_cast<size_type>(n);
}
...
template <class T>
inline auto xnewaxis<T>::step_size(std::size_t /*i*/,
                                   std::size_t /*n*/) const noexcept
    -> size_type {
  return 0;
}
...
template <class T>
inline auto xnewaxis<T>::revert_index(std::size_t i) const noexcept
    -> size_type {
  return i;
}
...
inline xkeep_slice<T>::operator xkeep_slice<S>() const noexcept {
  ...
  std::transform(m_raw_indices.cbegin(), m_raw_indices.cend(),
...
inline void xkeep_slice<T>::normalize(std::size_t shape) {
  ...
  for (std::size_t i = 0; i < sz; ++i) {
    m_indices[i] = m_raw_indices[i] < 0
                       ? static_cast<size_type>(shape) + m_raw_indices[i]
                       : m_raw_indices[i];
  }
}

template <class T>
inline auto xkeep_slice<T>::operator()(size_type i) const noexcept
    -> size_type {
...
template <class T>
inline auto xkeep_slice<T>::step_size(std::size_t i,
                                      std::size_t n) const noexcept
    -> size_type {
  if (m_indices.size() == 1) {
    return 0;
...
inline auto xkeep_slice<T>::revert_index(std::size_t i) const -> size_type {
  ...
  if (it != m_indices.end()) {
    ...
    XTENSOR_THROW(std::runtime_error, "Index i (" + std::to_string(i) +
...
inline xdrop_slice<T>::operator xdrop_slice<S>() const noexcept {
  ...
  std::transform(m_indices.cbegin(), m_indices.cend(), ret.m_indices.begin(),
                 [](const T& val) { return static_cast<S>(val); });
  std::transform(m_inc.cbegin(), m_inc.cend(),
...
template <class T>
inline auto xdrop_slice<T>::operator()(size_type i) const noexcept
    -> size_type {
...
template <class T>
inline auto xdrop_slice<T>::step_size(std::size_t i,
                                      std::size_t n) const noexcept
    -> size_type {
  if (i + n >= static_cast<std::size_t>(m_size)) {
    return (*this)(static_cast<size_type>(m_size - 1)) -
           (*this)(static_cast<size_type>(i)) + 1;
  } else {
    return (*this)(static_cast<size_type>(i + n)) -
           (*this)(static_cast<size_type>(i));
  }
}
