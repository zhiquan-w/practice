...
/**
 * Create a xcontainer (xarray, xtensor or xtensor_fixed) with uninitialized
 * values of with value_type T and shape. Selects the best container match
 * automatically from the supplied shape.
 *
 * - ``std::vector`` → ``xarray<T>``
...
inline xtensor_fixed<T, fixed_shape<N...>, L> empty(
    const fixed_shape<N...>& /*shape*/) {
  return xtensor_fixed<T, fixed_shape<N...>, L>();
}

/**
 * Create a xcontainer (xarray, xtensor or xtensor_fixed) with uninitialized
 * values of the same shape, value type and layout as the input xexpression *e*.
 *
...
/**
 * Create a xcontainer (xarray, xtensor or xtensor_fixed), filled with
 * *fill_value* and of the same shape, value type and layout as the input
 * xexpression *e*.
 *
 * @param e the xexpression from which to extract shape, value type and layout.
 * @param fill_value the value used to set each element of the returned
 * xcontainer.
 */
template <class E>
inline auto full_like(const xexpression<E>& e,
...
/**
 * Create a xcontainer (xarray, xtensor or xtensor_fixed), filled with zeros and
 * of the same shape, value type and layout as the input xexpression *e*.
 *
 * Note: contrary to zeros(shape), this function returns a non-lazy, allocated
 * container! Use ``xt::zeros<double>(e.shape());` for a lazy version.
 *
 * @param e the xexpression from which to extract shape, value type and layout.
 */
template <class E>
...
/**
 * Create a xcontainer (xarray, xtensor or xtensor_fixed), filled with ones and
 * of the same shape, value type and layout as the input xexpression *e*.
 *
 * Note: contrary to ones(shape), this function returns a non-lazy, evaluated
 * container! Use ``xt::ones<double>(e.shape());`` for a lazy version.
 *
 * @param e the xexpression from which to extract shape, value type and layout.
 */
template <class E>
...
// These methods should be private methods of arange_generator, however thi
// leads to ICE on VS2015
template <class R, class E, class U, class X, XTL_REQUIRES(xtl::is_integral<X>)>
inline void arange_assign_to(xexpression<E>& e, U start, U, X step,
                             bool) noexcept {
...
template <class R, class E, class U, class X,
          XTL_REQUIRES(xtl::negation<xtl::is_integral<X>>)>
inline void arange_assign_to(xexpression<E>& e, U start, U stop, X step,
...
class arange_generator {
 public:
  ...
  arange_generator(T start, T stop, S step, size_t num_steps,
                   bool endpoint = false)
      : m_start(start),
        m_stop(stop),
        m_step(step),
 ...
 private:
  ...
  inline R access_impl() const { return static_cast<R>(m_start); }
};
...
template <class T, class S>
using integer_with_signed_integer =
    xtl::conjunction<both_integer<T, S>, xtl::is_signed<S>>;

template <class T, class S>
using integer_with_unsigned_integer =
    xtl::conjunction<both_integer<T, S>, std::is_unsigned<S>>;
...
inline auto arange_impl(T start, T stop, S step = 1) noexcept {
  ...
  return detail::make_xgenerator(
      detail::arange_generator<T, T, S>(start, stop, step, shape), {shape});
}
...
inline auto arange_impl(T start, T stop, S step = 1) noexcept {
  ...
  if (!empty_cond) {
    shape =
        stop > start
            ? static_cast<std::size_t>((stop - start + step - S(1)) / step)
...
template <class T, class S = T,
          XTL_REQUIRES(integer_with_unsigned_integer<T, S>)>
inline auto arange_impl(T start, T stop, S step = 1) noexcept {
...
class eye_fn {
 public:
  ...
  inline T operator()(const It& /*begin*/, const It& end) const {
    ...
    return *(end - 1) == *(end - 2) + static_cast<lvalue_type>(m_k) ? T(1)
                                                                    : T(0);
  }
 ...
 */
template <class T>
inline auto linspace(T start, T stop, std::size_t num_samples = 50,
 ...
 */
template <class T>
inline auto logspace(T start, T stop, std::size_t num_samples, T base = 10,
                     bool endpoint = true) noexcept {
...
class concatenate_access {
 public:
  ...
  using size_type = std::size_t;
  using value_type =
      xtl::promote_type_t<typename std::decay_t<CT>::value_type...>;
  ...
  inline value_type access(const tuple_type& t, size_type axis, S index) const {
    ...
    auto get = [&index](auto& arr) { return arr[index]; };
...
class stack_access {
 public:
  ...
  template <class S>
  inline value_type access(const tuple_type& t, size_type axis, S index) const {
    auto get_item = [&index](auto& arr) { return arr[index]; };
...
inline auto xtuple(Types&&... args) {
  return std::tuple<xtl::const_closure_type_t<Types>...>(
...
struct concat_fixed_shape_impl<X, Y, axis, std::index_sequence<Is...>> {
  ...
  static_assert(all_true<(axis == Is || X::template get<Is>() ==
                                            Y::template get<Is>())...>::value,
                "Concatenation requires compatible shapes and axis");

  using type =
      fixed_shape<(axis == Is ? X::template get<Is>() + Y::template get<Is>()
                              : X::template get<Is>())...>;
};
...
struct concat_fixed_shape<axis, X, Y> {
  using type = typename concat_fixed_shape_impl<
      X, Y, axis, std::make_index_sequence<X::size()>>::type;
};

template <std::size_t axis, class X, class Y, class... Rest>
struct concat_fixed_shape {
  using type = typename concat_fixed_shape<
      axis, X, typename concat_fixed_shape<axis, Y, Rest...>::type>::type;
};
...
template <class... CT>
using all_fixed_shapes =
    detail::all_fixed<typename std::decay_t<CT>::shape_type...>;
...
struct concat_shape_builder_t {
  ...
  struct concat_shape<Shape, true> {
    // Convert `fixed_shape` to `static_shape` to allow runtime dimension
    // calculation.
  ...
  static auto build(const std::tuple<Args...>& t, std::size_t axis) {
    using shape_type = promote_shape_t<typename concat_shape<
        typename std::decay_t<Args>::shape_type>::type...>;
...
inline auto concatenate(std::tuple<CT...>&& t, std::size_t axis = 0) {
  ...
  return detail::make_xgenerator(
      detail::concatenate_impl<CT...>(std::move(t), axis), shape);
}

template <std::size_t axis, class... CT,
          typename = std::enable_if_t<detail::all_fixed_shapes<CT...>::value>>
inline auto concatenate(std::tuple<CT...>&& t) {
  using shape_type =
      detail::concat_fixed_shape_t<axis,
                                   typename std::decay_t<CT>::shape_type...>;
  return detail::make_xgenerator(
      detail::concatenate_impl<CT...>(std::move(t), axis), shape_type{});
}

namespace detail {
template <class T, std::size_t N>
inline std::array<T, N + 1> add_axis(std::array<T, N> arr, std::size_t axis,
                                     std::size_t value) {
...
inline auto stack(std::tuple<CT...>&& t, std::size_t axis = 0) {
  ...
  using source_shape_type = decltype(std::get<0>(t).shape());
  auto new_shape =
      detail::add_axis(xtl::forward_sequence<shape_type, source_shape_type>(
                           std::get<0>(t).shape()),
                       axis, sizeof...(CT));
  return detail::make_xgenerator(detail::stack_impl<CT...>(std::move(t), axis),
                                 new_shape);
}
...
inline auto vstack(std::tuple<CT...>&& t) {
  ...
  auto new_shape = detail::vstack_shape(
      t, xtl::forward_sequence<shape_type, source_shape_type>(
             std::get<0>(t).shape()));
  return detail::make_xgenerator(
      detail::vstack_impl<CT...>(std::move(t), size_t(0)), new_shape);
}
 ...
 * N-D scalar/vector fields over N-D grids, given one-dimensional coordinate
 * arrays x1, x2,..., xn.
 *
 ...
 */
template <class E>
inline auto diagonal(E&& arr, int offset = 0, std::size_t axis_1 = 0,
                     std::size_t axis_2 = 1) {
  ...
  return detail::make_xgenerator(
      detail::fn_impl<detail::diagonal_fn<CT>>(detail::diagonal_fn<CT>(
          std::forward<E>(arr), offset, axis_1, axis_2)),
      ret_shape);
}
 ...
 * selects the offset of the diagonal.
 *
 * @param arr the input array
 * @param k the diagonal above which to zero elements. 0 (default) selects the
 * main diagonal, k < 0 is below the main diagonal, k > 0 above.
