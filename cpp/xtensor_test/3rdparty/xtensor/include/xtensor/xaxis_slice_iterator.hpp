...
class xaxis_slice_iterator {
 public:
  ...
  using pointer = xtl::xclosure_pointer<
      std::remove_reference_t<apply_cv_t<CT, value_type>>>;
  ...
  template <class CTA>
  xaxis_slice_iterator(CTA&& e, size_type axis, size_type index,
                       size_type offset);
...
template <class CT>
bool operator==(const xaxis_slice_iterator<CT>& lhs,
...
inline xaxis_slice_iterator<CT>::xaxis_slice_iterator(CTA&& e, size_type axis,
    ...
    : p_expression(get_storage_init<storing_type>(std::forward<CTA>(e))),
      m_index(index),
      m_offset(offset),
      m_axis_stride(static_cast<size_type>(e.strides()[axis]) *
                    (e.shape()[axis] - 1u)),
      m_lower_shape(0),
      m_upper_shape(0),
      m_iter_size(0),
      m_is_target_axis(false),
      m_sv(strided_view(std::forward<CT>(e),
                        std::forward<shape_type>({e.shape()[axis]}),
                        std::forward<strides_type>({e.strides()[axis]}), offset,
                        e.layout())) {
  if (e.layout() == layout_type::row_major) {
    m_is_target_axis = axis == e.dimension() - 1;
    m_lower_shape =
        std::accumulate(e.shape().begin() + axis + 1, e.shape().end(),
                        size_t(1), std::multiplies<>());
    m_iter_size = std::accumulate(e.shape().begin() + 1, e.shape().end(),
                                  size_t(1), std::multiplies<>());

  } else {
    m_is_target_axis = axis == 0;
    m_lower_shape = std::accumulate(e.shape().begin(), e.shape().begin() + axis,
                                    size_t(1), std::multiplies<>());
    m_iter_size = std::accumulate(e.shape().begin(), e.shape().end() - 1,
                                  size_t(1), std::multiplies<>());
  }
...
inline auto xaxis_slice_iterator<CT>::operator++() -> self_type& {
  ...
  if (m_is_target_axis ||
      (m_upper_shape >= index_compare && index_compare >= m_lower_shape)) {
 ...
 */
template <class CT>
inline bool operator==(const xaxis_slice_iterator<CT>& lhs,
                       const xaxis_slice_iterator<CT>& rhs) {
...
/**
 * Returns an iterator to the first element of the expression for the specified
 * axis
 *
 * @param e the expession to iterate over
...
inline auto axis_slice_end(E&& e) {
  using return_type = xaxis_slice_iterator<xtl::closure_type_t<E>>;
  return return_type(std::forward<E>(e), 0,
                     std::accumulate(e.shape().begin() + 1, e.shape().end(),
                                     size_t(1), std::multiplies<>()),
                     e.size());
}
...
inline auto axis_slice_end(E&& e, typename std::decay_t<E>::size_type axis) {
  ...
  return return_type(
      ...
      std::accumulate(e.shape().begin() + axis + 1, e.shape().end(), index_sum,
                      std::multiplies<>()),
      e.size() + axis);
}
//@}
