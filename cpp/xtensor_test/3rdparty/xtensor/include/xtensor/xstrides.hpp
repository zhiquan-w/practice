...
template <class offset_type, layout_type L = layout_type::dynamic, class S,
          class... Args>
...
template <layout_type L = layout_type::dynamic, class shape_type,
          class strides_type>
std::size_t compute_strides(const shape_type& shape, layout_type l,
                            strides_type& strides);

template <layout_type L = layout_type::dynamic, class shape_type,
          class strides_type, class backstrides_type>
std::size_t compute_strides(const shape_type& shape, layout_type l,
                            strides_type& strides,
...
template <class S>
S unravel_from_strides(typename S::value_type index, const S& strides,
                       layout_type l = layout_type::row_major);

template <class S>
get_strides_t<S> unravel_index(typename S::value_type index, const S& shape,
                               layout_type l = layout_type::row_major);

template <class S, class T>
std::vector<get_strides_t<S>> unravel_indices(
    const T& indices, const S& shape, layout_type l = layout_type::row_major);
...
inline std::size_t compute_size_impl(const shape_type& shape,
  ...
  return static_cast<std::size_t>(
      std::abs(std::accumulate(shape.cbegin(), shape.cend(), size_type(1),
...
inline std::size_t compute_size_impl(const shape_type& shape,
  ...
  using size_type = std::decay_t<typename shape_type::value_type>;
  return static_cast<std::size_t>(
      std::accumulate(shape.cbegin(), shape.cend(), size_type(1),
                      std::multiplies<size_type>()));
}
}  // namespace detail
...
inline std::size_t compute_size(const shape_type& shape) noexcept {
  return detail::compute_size_impl(
      shape,
      xtl::is_signed<
          std::decay_t<typename std::decay_t<shape_type>::value_type>>());
}
...
struct layout_data_offset<layout_type::row_major, static_dim> {
  ...
  static inline auto run(const S& strides, Arg arg, Args... args) noexcept {
    return arg * strides[dim] +
           self_type::template run<dim + 1>(strides, args...);
  }
};
...
template <class offset_type, class S, class Arg, class... Args>
inline offset_type data_offset(const S& strides, Arg arg,
                               Args... args) noexcept {
  ...
  if (nargs == strides.size()) {
    ...
    return static_cast<offset_type>(
        detail::raw_data_offset<0>(strides, arg, args...));
    ...
    return static_cast<offset_type>(
        detail::raw_data_offset<0>(view, arg, args...));
  }
}

template <class offset_type, layout_type L, class S, class... Args>
inline offset_type unchecked_data_offset(const S& strides,
                                         Args... args) noexcept {
  return static_cast<offset_type>(
      detail::layout_data_offset<L, static_dimension<S>::value>::template run<
          0>(strides.cbegin(), args...));
}
...
inline bool stride_match_condition(const T1& stride, const T2& shape,
                                   const T1& data_size, bool zero_strides) {
  return (shape == T2(1) && stride == T1(0) && zero_strides) ||
         (stride == data_size);
}

// zero_strides should be true when strides are set to 0 if the corresponding
// dimensions are 1
template <class shape_type, class strides_type>
inline bool do_strides_match(const shape_type& shape,
                             const strides_type& strides, layout_type l,
                             bool zero_strides) {
  using value_type = typename strides_type::value_type;
  value_type data_size = 1;
  if (l == layout_type::row_major) {
    for (std::size_t i = strides.size(); i != 0; --i) {
      if (!stride_match_condition(strides[i - 1], shape[i - 1], data_size,
                                  zero_strides)) {
        return false;
      }
    ...
    for (std::size_t i = 0; i < strides.size(); ++i) {
      if (!stride_match_condition(strides[i], shape[i], data_size,
                                  zero_strides)) {
        return false;
      }
...
namespace detail {
template <class S>
inline S unravel_noexcept(typename S::value_type idx, const S& strides,
...
template <class S>
inline S unravel_from_strides(typename S::value_type index, const S& strides,
                              layout_type l) {
...
template <class S, class T>
inline get_value_type_t<T> ravel_from_strides(const T& index,
                                              const S& strides) {
  return element_offset<get_value_type_t<T>>(strides, index.begin(),
                                             index.end());
}
...
inline get_strides_t<S> unravel_index(typename S::value_type index,
  ...
  return unravel_from_strides(static_cast<strides_value_type>(index), strides,
                              l);
}
...
inline std::vector<get_strides_t<S>> unravel_indices(const T& idx,
  ...
  for (; out_iter != out.end(); ++out_iter, ++idx_iter) {
    *out_iter = unravel_from_strides(static_cast<strides_value_type>(*idx_iter),
                                     strides, l);
  }
  return out;
}
...
template <class S, class T>
inline get_value_type_t<T> ravel_index(const T& index, const S& shape,
                                       layout_type l) {
  using strides_type = get_strides_t<S>;
  strides_type strides = xtl::make_sequence<strides_type>(shape.size(), 0);
  compute_strides(shape, l, strides);
...
inline bool broadcastable(const S1& src_shape, const S2& dst_shape) {
  ...
  for (; src_iter != src_shape.crend() && res; ++src_iter, ++dst_iter) {
    res = (static_cast<std::size_t>(*src_iter) ==
           static_cast<std::size_t>(*dst_iter)) ||
          (*src_iter == 1);
  }
  return res;
}
...
struct check_strides_overlap<layout_type::column_major> {
  ...
  static std::size_t get(const S1& s1, const S2& s2) {
    ...
    for (; index < size; ++index) {
      if (static_cast<value_type>(s1[index]) !=
          static_cast<value_type>(s2[index])) {
        break;
      }
    }
    return index;
