 ...
 * - ``symmetric``: Pads with the reflection of the vector mirrored along the
 * edge of the array.
 ...
 * - ``BORDER_WRAP == wrap``
 */
enum class pad_mode { constant, symmetric, reflect, wrap, periodic };

namespace detail {
template <class S, class T>
inline bool check_pad_width(const std::vector<std::vector<S>>& pad_width,
                            const T& shape) {
 ...
 * @param mode The type of algorithm to use. [default:
...
inline auto pad(E&& e, const std::vector<std::vector<S>>& pad_width,
    ...
    if (ne > static_cast<size_type>(0)) {
      ...
      if (mode == pad_mode::wrap || mode == pad_mode::periodic) {
        ...
        if (ne == nb + e.shape(axis)) {
          ...
          svs[axis] = xt::range(nb + e.shape(axis) - 1,
                                nb + e.shape(axis) - ne - 1, -1);
        }
      } else if (mode == pad_mode::reflect) {
        ...
        if (ne == nb + e.shape(axis) - 1) {
          ...
          svs[axis] = xt::range(nb + e.shape(axis) - 2,
                                nb + e.shape(axis) - ne - 2, -1);
        }
      }
...
template <class E, class S = typename std::decay_t<E>::size_type,
          class V = typename std::decay_t<E>::value_type>
inline auto pad(E&& e, const std::vector<S>& pad_width,
                pad_mode mode = pad_mode::constant, V constant_value = 0) {
...
template <class E, class S = typename std::decay_t<E>::size_type,
          class V = typename std::decay_t<E>::value_type>
inline auto pad(E&& e, S pad_width, pad_mode mode = pad_mode::constant,
                V constant_value = 0) {
 ...
 */
template <class E, class S = typename std::decay_t<E>::size_type,
          XTL_REQUIRES(xtl::is_integral<S>)>
