...
template <class T>
inline std::size_t get_alignment_offset(const T* /*p*/, std::size_t size,
...
struct driven_align_mode_impl {
  using type = std::conditional_t<std::is_same<A1, A2>::value, A1,
                                  ::xt_simd::unaligned_mode>;
};
...
struct has_load_simd<
    E, T,
    void_t<decltype(std::declval<E>().template load_simd<aligned_mode, T>(
        typename E::size_type(0)))>> : std::true_type {};

template <class E, class T,
          bool B = xt_simd::simd_condition<typename E::value_type, T>::value>
...
struct has_simd_type
    : std::integral_constant<bool,
                             !std::is_same<T, xt_simd::simd_type<T>>::value> {};
...
using bool_load_type =
    std::conditional_t<std::is_same<T, bool>::value, uint8_t, T>;
...
struct container_simd_return_type
    : std::enable_if<!forbid_simd<C>::value,
                     xt_simd::simd_return_type<T1, bool_load_type<T2>>> {};

template <class C, class T1, class T2>
using container_simd_return_type_t =
    typename container_simd_return_type<C, T1, T2>::type;
}  // namespace xt
