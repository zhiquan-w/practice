...
template <class CT>
struct xscalar_base
    : xscalar_base_impl<get_expression_tag_t<std::decay_t<CT>>, CT> {};
...
class xscalar : public xsharable_expression<xscalar<CT>>,
 ...
 public:
  ...
  xt_simd::simd_return_type<value_type, requested_type> load_simd(
      size_type i) const;
...
struct all_xscalar {
  static constexpr bool value =
      xtl::conjunction<is_xscalar<std::decay_t<E>>...>::value;
};
}  // namespace detail
...
class xscalar_stepper {
 public:
  ...
  using reference =
      std::conditional_t<is_const, typename storage_type::const_reference,
                         typename storage_type::reference>;
  using pointer =
      std::conditional_t<is_const, typename storage_type::const_pointer,
...
namespace detail {
template <bool is_const, class CT>
using dummy_reference_t =
    std::conditional_t<is_const, typename xscalar<CT>::const_reference,
                       typename xscalar<CT>::reference>;

template <bool is_const, class CT>
using dummy_pointer_t =
    std::conditional_t<is_const, typename xscalar<CT>::const_pointer,
...
class xdummy_iterator
    : public xtl::xrandom_access_iterator_base<
          xdummy_iterator<is_const, CT>, typename xscalar<CT>::value_type,
...
template <class CT>
XTENSOR_CONSTEXPR_RETURN auto linear_begin(xscalar<CT>& c) noexcept
    -> decltype(c.dummy_begin()) {
  return c.dummy_begin();
}

template <class CT>
XTENSOR_CONSTEXPR_RETURN auto linear_end(xscalar<CT>& c) noexcept
...
template <class CT>
inline xscalar<CT>::xscalar() noexcept : m_value() {}
...
template <class CT>
inline auto xscalar<CT>::expression() const noexcept
...
template <bool is_const, class CT>
inline auto xdummy_iterator<is_const, CT>::operator+=(difference_type) noexcept
    -> self_type& {
  return *this;
}
...
template <bool is_const, class CT>
inline auto xdummy_iterator<is_const, CT>::operator-(
    const self_type&) const noexcept -> difference_type {
  return 0;
}
...
template <bool is_const, class CT>
inline bool xdummy_iterator<is_const, CT>::equal(
    const self_type& rhs) const noexcept {
  return p_c == rhs.p_c;
}

template <bool is_const, class CT>
inline bool xdummy_iterator<is_const, CT>::less_than(
    const self_type& rhs) const noexcept {
  return p_c < rhs.p_c;
}
