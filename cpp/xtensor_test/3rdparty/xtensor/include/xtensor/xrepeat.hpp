...
struct xiterable_inner_types<xrepeat<CT, R>> {
  ...
  using const_stepper =
      xrepeat_stepper<typename xexpression_type::const_stepper, repeats_type>;
...
class xrepeat_stepper {
 public:
  ...
  xrepeat_stepper(S&& s, const shape_type& shape, const repeats_type& repeats,
                  size_type axis);
 ...
 private:
  ...
  std::vector<size_type> get_next_positions(size_type dim,
                                            size_type steps_to_go) const;
  std::vector<size_type> get_next_positions_back(size_type dim,
                                                 size_type steps_to_go) const;
};
...
xrepeat<CT, R>::xrepeat(CTA&& e, R&& repeats, size_type axis)
  ...
  m_shape[axis] = static_cast<shape_value_type>(
      std::accumulate(m_repeats.begin(), m_repeats.end(), 0));
}
...
template <class CT, class R>
template <class It>
inline auto xrepeat<CT, R>::element(It first, It last) const
    -> const_reference {
...
/**
 * Returns a constant reference to the underlying expression of the broadcast
 * expression.
 */
template <class CT, class R>
inline auto xrepeat<CT, R>::expression() const noexcept
...
template <class CT, class R>
template <class Arg, class... Args>
inline auto xrepeat<CT, R>::access(Arg arg, Args... args) const
    -> const_reference {
...
template <class CT, class R>
inline auto xrepeat<CT, R>::stepper_begin(const shape_type& s) const
    -> const_stepper {
  return const_stepper(m_e.stepper_begin(s), m_shape, m_repeats,
                       m_repeating_axis);
}
...
template <class CT, class R>
inline auto xrepeat<CT, R>::stepper_end(const shape_type& s,
                                        layout_type l) const -> const_stepper {
  auto st =
      const_stepper(m_e.stepper_begin(s), m_shape, m_repeats, m_repeating_axis);
  st.to_end(l);
  return st;
}
...
template <class CT, class R>
template <std::size_t I, class Arg, class... Args>
inline auto xrepeat<CT, R>::access_impl(stepper&& s, Arg arg,
...
template <class S, class R>
xrepeat_stepper<S, R>::xrepeat_stepper(S&& s, const shape_type& shape,
...
template <class S, class R>
inline void xrepeat_stepper<S, R>::step_back(size_type dim,
                                             size_type steps_to_go) {
...
inline void xrepeat_stepper<S, R>::to_end(layout_type l) {
  ...
  std::transform(m_shape.begin(), m_shape.end(), m_positions.begin(),
...
template <class S, class R>
inline void xrepeat_stepper<S, R>::make_step(size_type dim,
                                             size_type steps_to_go) {
  if (steps_to_go > 0) {
    if (dim == m_repeating_axis) {
      size_type subposition = m_subposition;
      m_repeating_steps += static_cast<std::ptrdiff_t>(steps_to_go);
      while (m_repeating_steps >=
             static_cast<ptrdiff_t>(m_repeats[subposition])) {
...
template <class S, class R>
inline void xrepeat_stepper<S, R>::make_step_back(size_type dim,
                                                  size_type steps_to_go) {
  if (steps_to_go > 0) {
    if (dim == m_repeating_axis) {
      size_type subposition = m_subposition;
...
template <class S, class R>
inline auto xrepeat_stepper<S, R>::get_next_positions(
    size_type dim, size_type steps_to_go) const -> std::vector<size_type> {
...
template <class S, class R>
inline auto xrepeat_stepper<S, R>::get_next_positions_back(
    size_type dim, size_type steps_to_go) const -> std::vector<size_type> {
  auto next_position_for_dim =
      static_cast<std::ptrdiff_t>(m_positions[dim] - steps_to_go);
  if (dim > 0) {
    size_type steps_in_previous_dim = 0;
    ...
    if (steps_in_previous_dim > 0) {
      auto next_positions =
          get_next_positions_back(dim - 1, steps_in_previous_dim);
