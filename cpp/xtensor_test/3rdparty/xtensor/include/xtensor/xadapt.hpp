...
inline xarray_adaptor<xtl::closure_type_t<C>, L, std::decay_t<SC>> adapt(
...
/**
 * Constructs an non-owning xarray_adaptor from a pointer with the specified
 * shape and layout.
...
inline auto adapt(C&& pointer, const SC& shape, layout_type l = L) {
  static_assert(!xtl::is_integral<SC>::value, "shape cannot be a integer");
  using buffer_type = xbuffer_adaptor<C, xt::no_ownership,
                                      detail::default_allocator_for_ptr_t<C>>;
...
inline xarray_adaptor<xtl::closure_type_t<C>, layout_type::dynamic,
                      std::decay_t<SC>>
adapt(C&& container, SC&& shape, SS&& strides) {
  static_assert(!xtl::is_integral<std::decay_t<SC>>::value,
                "shape cannot be a integer");
  using return_type = xarray_adaptor<xtl::closure_type_t<C>,
                                     layout_type::dynamic, std::decay_t<SC>>;
  return return_type(
      std::forward<C>(container),
      xtl::forward_sequence<typename return_type::inner_shape_type, SC>(shape),
      xtl::forward_sequence<typename return_type::inner_strides_type, SS>(
          strides));
}
 ...
 * array
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class P, class O, class SC,
          class A = detail::default_allocator_for_ptr_t<P>,
          XTL_REQUIRES(detail::not_an_array<std::decay_t<SC>>)>
inline xarray_adaptor<xbuffer_adaptor<xtl::closure_type_t<P>, O, A>, L, SC>
adapt(P&& pointer, typename A::size_type size, O ownership, const SC& shape,
      layout_type l = L, const A& alloc = A()) {
  static_assert(!xtl::is_integral<SC>::value, "shape cannot be a integer");
  (void)ownership;
 ...
 * array
 */
template <class P, class O, class SC, class SS,
          class A = detail::default_allocator_for_ptr_t<P>,
          XTL_REQUIRES(detail::not_an_array<std::decay_t<SC>>,
                       detail::not_a_layout<std::decay_t<SS>>)>
inline xarray_adaptor<xbuffer_adaptor<xtl::closure_type_t<P>, O, A>,
                      layout_type::dynamic, std::decay_t<SC>>
adapt(P&& pointer, typename A::size_type size, O ownership, SC&& shape,
      SS&& strides, const A& alloc = A()) {
  static_assert(!xtl::is_integral<std::decay_t<SC>>::value,
                "shape cannot be a integer");
  (void)ownership;
  using buffer_type = xbuffer_adaptor<xtl::closure_type_t<P>, O, A>;
  using return_type =
      xarray_adaptor<buffer_type, layout_type::dynamic, std::decay_t<SC>>;
  buffer_type buf(std::forward<P>(pointer), size, alloc);
  return return_type(
      std::move(buf),
      xtl::forward_sequence<typename return_type::inner_shape_type, SC>(shape),
...
/**
 * Contructs an xarray_adaptor of the given C array allocated on the stack, with
 * the specified shape and layout.
 * @param c_array the C array allocated on the stack
 * @param shape the shape of the xarray_adaptor
 * @param l the layout_type of the xarray_adaptor
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class T, std::size_t N,
...
/**
 * Contructs an xarray_adaptor of the given C array allocated on the stack, with
 * the specified shape and stirdes.
 * @param c_array the C array allocated on the stack
 * @param shape the shape of the xarray_adaptor
 * @param strides the strides of the xarray_adaptor
 */
...
inline auto adapt(T (&c_array)[N], SC&& shape, SS&& strides) {
  return adapt(&c_array[0], N, xt::no_ownership(), std::forward<SC>(shape),
...
inline xtensor_adaptor<C, 1, L> adapt(C&& container, layout_type l = L) {
  const std::array<typename std::decay_t<C>::size_type, 1> shape{
      container.size()};
...
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class C, class SC,
          XTL_REQUIRES(detail::is_array<std::decay_t<SC>>,
                       detail::not_a_pointer<C>)>
inline xtensor_adaptor<C, detail::array_size<SC>::value, L> adapt(
    C&& container, const SC& shape, layout_type l = L) {
  static_assert(!xtl::is_integral<SC>::value, "shape cannot be a integer");
...
/**
 * Constructs an non-owning xtensor_adaptor from a pointer with the specified
 * shape and layout.
...
adapt(C&& container, SC&& shape, SS&& strides) {
  ...
  constexpr std::size_t N = detail::array_size<SC>::value;
  using return_type =
      xtensor_adaptor<xtl::closure_type_t<C>, N, layout_type::dynamic>;
  return return_type(
      std::forward<C>(container),
      xtl::forward_sequence<typename return_type::inner_shape_type, SC>(shape),
      xtl::forward_sequence<typename return_type::inner_strides_type, SS>(
 ...
 * array
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class P, class O,
          class A = detail::default_allocator_for_ptr_t<P>>
inline xtensor_adaptor<xbuffer_adaptor<xtl::closure_type_t<P>, O, A>, 1, L>
adapt(P&& pointer, typename A::size_type size, O ownership, layout_type l = L,
      const A& alloc = A()) {
  (void)ownership;
  using buffer_type = xbuffer_adaptor<xtl::closure_type_t<P>, O, A>;
...
template <class P, class O, class SC, class SS,
          ...
          XTL_REQUIRES(detail::is_array<std::decay_t<SC>>,
                       detail::not_a_layout<std::decay_t<SS>>)>
inline xtensor_adaptor<xbuffer_adaptor<xtl::closure_type_t<P>, O, A>,
                       detail::array_size<SC>::value, layout_type::dynamic>
adapt(P&& pointer, typename A::size_type size, O ownership, SC&& shape,
      SS&& strides, const A& alloc = A()) {
  static_assert(!xtl::is_integral<std::decay_t<SC>>::value,
                "shape cannot be a integer");
...
/**
 * Contructs an xtensor_adaptor of the given C array allocated on the stack,
 * with the specified shape and layout.
 * @param c_array the C array allocated on the stack
 * @param shape the shape of the xarray_adaptor
 * @param l the layout_type of the xarray_adaptor
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class T, std::size_t N,
          class SC, XTL_REQUIRES(detail::is_array<std::decay_t<SC>>)>
inline auto adapt(T (&c_array)[N], const SC& shape, layout_type l = L) {
  return adapt(&c_array[0], N, xt::no_ownership(), shape, l);
}
...
/**
 * Contructs an xtensor_adaptor of the given C array allocated on the stack,
 * with the specified shape and stirdes.
 * @param c_array the C array allocated on the stack
 * @param shape the shape of the xarray_adaptor
 * @param strides the strides of the xarray_adaptor
 */
...
inline auto adapt(C&& pointer, const fixed_shape<X...>& /*shape*/) {
  ...
  return return_type(buffer_type(
      pointer, detail::fixed_compute_size<fixed_shape<X...>>::value));
}

template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class C, class T,
          std::size_t N>
...
auto adapt_smart_ptr(P&& smart_ptr, const SC& shape, layout_type l = L) {
  using buffer_adaptor = xbuffer_adaptor<decltype(smart_ptr.get()),
                                         smart_ownership, std::decay_t<P>>;
  return xarray_adaptor<buffer_adaptor, L, std::decay_t<SC>>(
      buffer_adaptor(smart_ptr.get(), compute_size(shape),
...
auto adapt_smart_ptr(P&& data_ptr, const SC& shape, D&& smart_ptr,
                     layout_type l = L) {
 ...
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class P, class I,
          std::size_t N>
...
auto adapt_smart_ptr(P&& data_ptr, const I (&shape)[N], D&& smart_ptr,
  ...
  return xtensor_adaptor<buffer_adaptor, N, L>(
      buffer_adaptor(data_ptr, compute_size(fshape),
                     std::forward<D>(smart_ptr)),
      std::move(fshape), l);
}
}  // namespace xt
