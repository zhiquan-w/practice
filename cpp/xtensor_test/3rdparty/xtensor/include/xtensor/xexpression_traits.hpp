...
template <template <class, std::size_t, class, bool> class S, class X,
          std::size_t N, class A, bool Init>
...
struct temporary_type_from_tag<xtensor_expression_tag, T> {
  ...
  using type = typename detail::xtype_for_shape<shape_type>::template type<
      value_type, static_layout>;
};
...
struct common_tensor_type_impl {
  ...
  using type =
      typename xtype_for_shape<shape_type>::template type<value_type,
                                                          static_layout>;
};
}  // namespace detail
...
struct common_tensor_type
    : detail::common_tensor_type_impl<std::decay_t<C>...> {};
