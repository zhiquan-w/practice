 ...
 */
template <class E, class S,
          class = typename std::enable_if_t<has_iterator_interface<S>::value>>
...
struct static_dimension {
  static constexpr std::ptrdiff_t value =
      detail::static_dimension_impl<S>::value;
};
...
template <layout_type L, class S>
struct select_layout {
  constexpr static std::ptrdiff_t static_dimension =
      xt::static_dimension<S>::value;
  constexpr static bool is_any = static_dimension != -1 &&
                                 static_dimension <= 1 &&
...
struct max_array_size<T, Ts...>
    : std::integral_constant<std::size_t, imax(std::tuple_size<T>::value,
...
struct broadcast_fixed_shape_cmp_impl<JX, fixed_shape<I...>,
                                      fixed_shape<J...>> {
  ...
  // we're statically checking if the broadcast shapes are either one on either
  // of them or equal
...
template <std::size_t... JX, std::size_t... I, std::size_t... J>
struct broadcast_fixed_shape_impl<std::index_sequence<JX...>, fixed_shape<I...>,
                                  fixed_shape<J...>> {
  static_assert(sizeof...(J) >= sizeof...(I), "broadcast shapes do not match.");

  using type = xt::fixed_shape<broadcast_fixed_shape_cmp_impl<
      JX, fixed_shape<I...>, fixed_shape<J...>>::ordinate...>;
  static constexpr bool value = xtl::conjunction<broadcast_fixed_shape_cmp_impl<
      JX, fixed_shape<I...>, fixed_shape<J...>>...>::value;
};
...
template <class... S>
using only_array =
    xtl::conjunction<xtl::disjunction<is_array<S>, is_fixed<S>>...>;

// test that at least one argument is a fixed shape. If yes, then either
// argument has to be fixed or scalar
template <class... S>
using only_fixed = std::integral_constant<
    bool, xtl::disjunction<is_fixed<S>...>::value &&
              xtl::conjunction<
                  xtl::disjunction<is_fixed<S>, is_scalar_shape<S>>...>::value>;

template <class... S>
using all_fixed = xtl::conjunction<is_fixed<S>...>;

// The promote_index meta-function returns std::vector<promoted_value_type> in
// the general case and an array of the promoted value type and maximal size if
// all arguments are of type std::array
...
struct select_promote_index<false, false, S...> {
  using type =
      dynamic_shape<typename std::common_type<typename S::value_type...>::type>;
};

template <class... S>
struct promote_index : select_promote_index<only_fixed<S...>::value,
                                            only_array<S...>::value, S...> {};
