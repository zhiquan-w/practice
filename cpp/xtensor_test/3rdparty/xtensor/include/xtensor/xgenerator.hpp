...
class xgenerator : public xsharable_expression<xgenerator<F, R, S>>,
 ...
 public:
  ...
  template <class E, class FE = F,
            class = std::enable_if_t<has_assign_to<E, FE>::value>>
...
template <class F, class R, class S>
template <class... Args>
inline auto xgenerator<F, R, S>::unchecked(Args... args) const
    -> const_reference {
  return m_f(args...);
}
...
inline auto xgenerator<F, R, S>::element(It first, It last) const
    -> const_reference {
  using bounded_iterator =
      xbounded_iterator<It, typename shape_type::const_iterator>;
...
template <class F, class R, class S>
template <class O>
inline bool xgenerator<F, R, S>::has_linear_assign(
...
template <class F, class R, class S>
template <class O>
inline auto xgenerator<F, R, S>::stepper_end(const O& shape,
...
template <class F, class R, class S>
inline auto xgenerator<F, R, S>::functor() const noexcept
    -> const functor_type& {
  return m_f;
}
...
template <class F, class R, class S>
template <class OR, class OF>
inline auto xgenerator<F, R, S>::build_generator(OF&& func) const
    -> rebind_t<OR, OF> {
  return rebind_t<OR, OF>(std::move(func), shape_type(m_shape));
}

/**
 * Reshapes the generator and keeps old elements. The `shape` argument can have
 * one of its value equal to `-1`, in this case the value is inferred from the
 * number of elements in the generator and the remaining values in the `shape`.
 * \code{.cpp}
...
inline auto xgenerator<F, R, S>::reshape(O&& shape) const& {
  return reshape_view(
      *this,
      compute_shape(shape,
                    xtl::is_signed<typename std::decay_t<O>::value_type>()));
}
...
template <class F, class R, class S>
template <class T>
inline auto xgenerator<F, R, S>::reshape(
    std::initializer_list<T> shape) const& {
...
template <class F, class R, class S>
template <class O>
inline decltype(auto) xgenerator<F, R, S>::compute_shape(
    O&& shape, std::false_type) const {
  return xtl::forward_sequence<
      xt::dynamic_shape<typename shape_type::value_type>, O>(shape);
}

template <class F, class R, class S>
template <class O>
inline auto xgenerator<F, R, S>::compute_shape(O&& shape,
                                               std::true_type) const {
...
template <class F, class R, class S>
template <class T>
inline auto xgenerator<F, R, S>::compute_shape(
    std::initializer_list<T> shape) const {
