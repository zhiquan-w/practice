 ...
 *
 * The xoptional_assembly_base class defines the interface for dense
 * multidimensional optional assembly classes. Optional assembly classes hold
 * optional values and are optimized for tensor operations.
 * xoptional_assembly_base does not embed any data container, this
 * responsibility is delegated to the inheriting classes.
 *
 * @tparam D The derived type, i.e. the inheriting class for which
 * xoptional_assembly_base provides the interface.
 */
template <class D>
class xoptional_assembly_base : private xiterable<D> {
 public:
  ...
  static constexpr layout_type static_layout =
      raw_value_expression::static_layout;
  static constexpr bool contiguous_layout =
      raw_value_expression::contiguous_layout;
...
class xoptional_assembly_stepper {
 public:
  ...
  using reference =
      std::conditional_t<is_const, typename assembly_type::const_reference,
                         typename assembly_type::reference>;
  using pointer =
      std::conditional_t<is_const, typename assembly_type::const_pointer,
  ...
  using value_stepper =
      std::conditional_t<is_const, typename raw_value_expression::const_stepper,
                         typename raw_value_expression::stepper>;
  using flag_stepper =
      std::conditional_t<is_const, typename raw_flag_expression::const_stepper,
 ...
 */
template <class D>
inline auto constexpr xoptional_assembly_base<D>::dimension() const noexcept
    -> size_type {
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::shape() const noexcept
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::strides() const noexcept
...
template <class D>
template <class S>
inline void xoptional_assembly_base<D>::resize(const S& shape,
...
template <class D>
template <class S>
inline auto& xoptional_assembly_base<D>::reshape(const S& shape,
                                                 layout_type layout) & {
...
template <class D>
template <class T>
inline auto& xoptional_assembly_base<D>::reshape(std::initializer_list<T> shape,
                                                 layout_type layout) & {
  value().reshape(shape, layout);
  has_value().reshape(shape, layout);
  return *this;
...
//@{
/**
 * Returns a reference to the element at the specified position in the optional
 * assembly.
 * @param args a list of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices should be
 * equal or greater than the number of dimensions of the optional assembly.
 */
template <class D>
template <class... Args>
...
template <class D>
template <class... Args>
inline auto xoptional_assembly_base<D>::operator()(Args... args) const
    -> const_reference {
 ...
 * @param args a list of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices should be
 * equal to the number of dimensions of the optional assembly.
 * @exception std::out_of_range if the number of argument is greater than the
 * number of dimensions or if indices are out of bounds.
 */
template <class D>
template <class... Args>
...
/**
 * Returns a constant reference to the element at the specified position in the
 * optional assembly, after dimension and bounds checking.
 * @param args a list of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices should be
 * equal to the number of dimensions of the optional assembly.
 * @exception std::out_of_range if the number of argument is greater than the
...
template <class D>
template <class... Args>
inline auto xoptional_assembly_base<D>::at(Args... args) const
    -> const_reference {
 ...
 * assembly.
 * @param args a list of indices specifying the position in the  optional
 * assembly. Indices must be unsigned integers, the number of indices must be
 * equal to the number of dimensions of the  optional assembly, else the
...
template <class D>
template <class... Args>
inline auto xoptional_assembly_base<D>::unchecked(Args... args) const
    -> const_reference {
  return const_reference(value().unchecked(args...),
                         has_value().unchecked(args...));
}

/**
 * Returns a reference to the element at the specified position in the optional
 * assembly.
 * @param index a sequence of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices in the
 * list should be equal or greater than the number of dimensions of the optional
 * assembly.
 */
template <class D>
template <class S>
...
template <class D>
template <class I>
inline auto xoptional_assembly_base<D>::operator[](
...
template <class I>
inline auto xoptional_assembly_base<D>::operator[](
    std::initializer_list<I> index) const -> const_reference {
  return const_reference(value()[index], has_value()[index]);
}

template <class D>
inline auto xoptional_assembly_base<D>::operator[](size_type i) const
    -> const_reference {
...
/**
 * Returns a reference to the element at the specified position in the optional
 * assembly, after applying periodicity to the indices (negative and
 * 'overflowing' indices are changed).
 * @param args a list of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices should be
 * equal to the number of dimensions of the optional assembly.
 */
...
/**
 * Returns a constant reference to the element at the specified position in the
 * optional assembly, after applying periodicity to the indices (negative and
 * 'overflowing' indices are changed).
 * @param args a list of indices specifying the position in the optional
 * assembly. Indices must be unsigned integers, the number of indices should be
 * equal to the number of dimensions of the optional assembly.
...
template <class D>
template <class... Args>
inline auto xoptional_assembly_base<D>::periodic(Args... args) const
    -> const_reference {
  return const_reference(value().periodic(args...),
                         has_value().periodic(args...));
}
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::flat(size_type i) const
    -> const_reference {
...
template <class D>
template <class It>
inline auto xoptional_assembly_base<D>::element(It first, It last)
    -> reference {
  return reference(value().element(first, last),
                   has_value().element(first, last));
}
...
template <class D>
template <class It>
inline auto xoptional_assembly_base<D>::element(It first, It last) const
    -> const_reference {
  return const_reference(value().element(first, last),
                         has_value().element(first, last));
}

/**
 * Returns ``true`` only if the the specified position is a valid entry in the
 * expression.
...
template <class D>
inline auto xoptional_assembly_base<D>::storage() const noexcept
    -> const storage_type& {
  return derived_cast().storage_impl();
}
...
template <class D>
inline auto xoptional_assembly_base<D>::data() const noexcept
    -> const value_type* {
  return storage().data();
}

template <class D>
inline auto xoptional_assembly_base<D>::data_offset() const noexcept
    -> const size_type {
  return size_type(0);
}
...
template <class D>
template <class S>
inline bool xoptional_assembly_base<D>::broadcast_shape(
    S& shape, bool reuse_cache) const {
  bool res = value().broadcast_shape(shape, reuse_cache);
  return res && has_value().broadcast_shape(shape, reuse_cache);
}

/**
 * Checks whether the xoptional_assembly_base can be linearly assigned to an
 * expression with the specified strides.
 * @return a boolean indicating whether a linear assign is possible
 */
template <class D>
template <class S>
inline bool xoptional_assembly_base<D>::has_linear_assign(
    const S& strides) const noexcept {
  return value().has_linear_assign(strides) &&
         has_value().has_linear_assign(strides);
}
//@}

template <class D>
inline auto xoptional_assembly_base<D>::storage_begin() noexcept
    -> storage_iterator {
  return storage_iterator(value().storage_begin(), has_value().storage_begin());
}

template <class D>
inline auto xoptional_assembly_base<D>::storage_end() noexcept
    -> storage_iterator {
  return storage_iterator(value().storage_end(), has_value().storage_end());
}

template <class D>
inline auto xoptional_assembly_base<D>::storage_begin() const noexcept
    -> const_storage_iterator {
  return storage_cbegin();
}

template <class D>
inline auto xoptional_assembly_base<D>::storage_end() const noexcept
    -> const_storage_iterator {
  return storage_cend();
}
...
template <class D>
inline auto xoptional_assembly_base<D>::storage_cend() const noexcept
    -> const_storage_iterator {
...
template <class D>
inline auto xoptional_assembly_base<D>::storage_rbegin() noexcept
    -> reverse_storage_iterator {
  return reverse_storage_iterator(storage_end());
}

template <class D>
inline auto xoptional_assembly_base<D>::storage_rend() noexcept
    -> reverse_storage_iterator {
...
template <class D>
template <class S>
inline auto xoptional_assembly_base<D>::stepper_begin(const S& shape) noexcept
    -> stepper {
  return stepper(value().stepper_begin(shape),
                 has_value().stepper_begin(shape));
}

template <class D>
template <class S>
inline auto xoptional_assembly_base<D>::stepper_end(const S& shape,
                                                    layout_type l) noexcept
    -> stepper {
  return stepper(value().stepper_end(shape, l),
                 has_value().stepper_end(shape, l));
}

template <class D>
template <class S>
inline auto xoptional_assembly_base<D>::stepper_begin(
    const S& shape) const noexcept -> const_stepper {
  return const_stepper(value().stepper_begin(shape),
                       has_value().stepper_begin(shape));
}

template <class D>
template <class S>
inline auto xoptional_assembly_base<D>::stepper_end(
    const S& shape, layout_type l) const noexcept -> const_stepper {
  return const_stepper(value().stepper_end(shape, l),
                       has_value().stepper_end(shape, l));
}
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::value() const noexcept
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::has_value() noexcept
    -> flag_expression {
 ...
 */
template <class D>
inline auto xoptional_assembly_base<D>::has_value() const noexcept
    -> const_flag_expression {
  return derived_cast().has_value_impl();
}

template <class D>
inline auto xoptional_assembly_base<D>::derived_cast() noexcept
    -> derived_type& {
  return *static_cast<derived_type*>(this);
}

template <class D>
inline auto xoptional_assembly_base<D>::derived_cast() const noexcept
    -> const derived_type& {
...
inline xoptional_assembly_stepper<D, C>::xoptional_assembly_stepper(
    value_stepper vs, flag_stepper fs) noexcept
    : m_vs(vs), m_fs(fs) {}
