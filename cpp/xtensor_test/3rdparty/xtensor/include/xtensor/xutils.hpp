...
noexcept(implementation_dependent);*/
...
template <std::size_t I, class F, class... T>
    ...
    I<sizeof...(T), void>::type for_each_impl(
        F&& f, std::tuple<T...>& t) noexcept(noexcept(f(std::get<I>(t)))) {
  f(std::get<I>(t));
...
template <std::size_t I, class F, class... T>
    ...
    I<sizeof...(T), void>::type for_each_impl(
        F&& f,
        const std::tuple<T...>& t) noexcept(noexcept(f(std::get<I>(t)))) {
  f(std::get<I>(t));
  for_each_impl<I + 1, F, T...>(std::forward<F>(f), t);
}
}  // namespace detail
...
template <std::size_t I, class F, class R, class... T>
    ...
    I<sizeof...(T), R> accumulate_impl(
        F&& f, R init,
        const std::tuple<T...>& t) noexcept(noexcept(f(init, std::get<I>(t)))) {
...
template <class F, class R, class... T>
inline R accumulate(F&& f, R init, const std::tuple<T...>& t) noexcept(noexcept(
    detail::accumulate_impl<0, F, R, T...>(std::forward<F>(f), init, t))) {
...
R apply_one(F&& func, const std::tuple<S...>& s)
    NOEXCEPT(noexcept(func(std::get<I>(s)))) {
...
inline std::size_t normalize_axis(std::size_t dim, std::ptrdiff_t axis) {
  return axis < 0
             ? static_cast<std::size_t>(static_cast<std::ptrdiff_t>(dim) + axis)
             : static_cast<std::size_t>(axis);
}

template <class E, class C>
inline std::enable_if_t<
    !xtl::is_integral<std::decay_t<C>>::value &&
...
normalize_axis(E& expr, C&& axes) {
  ...
  XTENSOR_ASSERT(std::all_of(res.begin(), res.end(), [&expr](auto ax_el) {
...
normalize_axis(E& expr, C&& axes) {
  ...
  XTENSOR_ASSERT(std::all_of(axes.begin(), axes.end(), [&expr](auto ax_el) {
    return ax_el < expr.dimension();
  }));
...
inline auto forward_normalize(E& expr, C&& axes) -> std::enable_if_t<
    xtl::is_signed<std::decay_t<decltype(*std::begin(axes))>>::value, R> {
  R res;
  xt::resize_container(res, xtl::sequence_size(axes));
  auto dim = expr.dimension();
  std::transform(std::begin(axes), std::end(axes), std::begin(res),
                 [&dim](auto ax_el) { return normalize_axis(dim, ax_el); });
...
template <class R, class E, class C>
inline auto forward_normalize(E& expr, C&& axes) -> std::enable_if_t<
    !xtl::is_signed<std::decay_t<decltype(*std::begin(axes))>>::value &&
...
inline auto forward_normalize(E& expr, C&& axes) -> std::enable_if_t<
  ...
  static_cast<void>(expr);
  XTENSOR_ASSERT(
      std::all_of(std::begin(axes), std::end(axes),
                  [&expr](auto ax_el) { return ax_el < expr.dimension(); }));
...
// When subclassing from std::tuple not all compilers are able to correctly
// instantiate get See here: https://stackoverflow.com/a/37188019/2528668
...
template <class E>
struct is_iterator<
    E, void_t<decltype(*std::declval<const E>(),
                       std::declval<const E>() == std::declval<const E>(),
                       std::declval<const E>() != std::declval<const E>(),
                       ++(*std::declval<E*>()), (*std::declval<E*>())++,
...
#define XTENSOR_GLIBCXX_USE_CXX11_ABI 1
    ...
    (defined(_GLIBCXX_USE_DUAL_ABI) && !_GLIBCXX_USE_DUAL_ABI)
#define XTENSOR_GLIBCXX_USE_CXX11_ABI 1
#endif
#endif
#endif
    ...
    defined(XTENSOR_GLIBCXX_USE_CXX11_ABI)
...
struct tracking_allocator : private A {
  ...
  T* allocate(std::size_t n) {
    if (alloc_tracking::enabled()) {
      if (P == alloc_tracking::print) {
        ...
        XTENSOR_THROW(std::runtime_error, "xtensor allocation of " +
                                              std::to_string(n) +
                                              " elements detected");
      }
    }
  ...
  struct rebind {
    ...
    using other =
        tracking_allocator<U, typename traits::template rebind_alloc<U>, P>;
  };
};

template <class T, class AT, alloc_tracking::policy PT, class U, class AU,
...
#else
template <class X, template <class, std::size_t> class C, class T,
          std::size_t N>
...
struct inner_reference {
  ...
  using type =
      std::conditional_t<std::is_const<std::remove_reference_t<ST>>::value,
...
struct has_fixed_rank {
  using type = std::integral_constant<bool, get_rank<std::decay_t<E>>::value !=
                                                SIZE_MAX>;
};
...
struct has_rank {
  using type =
      std::integral_constant<bool, get_rank<std::decay_t<E>>::value == N>;
};
