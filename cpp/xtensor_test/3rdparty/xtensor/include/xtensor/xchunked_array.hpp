...
class xchunked_array : public xaccessible<xchunked_array<chunk_storage>>,
 ...
 private:
  template <class... Idxs>
  using indexes_type = std::pair<std::array<std::size_t, sizeof...(Idxs)>,
                                 std::array<std::size_t, sizeof...(Idxs)>>;

  template <class... Idxs>
  using chunk_indexes_type =
      std::array<std::pair<std::size_t, std::size_t>, sizeof...(Idxs)>;

  template <std::size_t N>
  using static_indexes_type =
      std::pair<std::array<std::size_t, N>, std::array<std::size_t, N>>;

  using dynamic_indexes_type =
  ...
  template <class Idx>
  std::pair<std::size_t, std::size_t> get_chunk_indexes_in_dimension(
      std::size_t dim, Idx idx) const;

  template <std::size_t... dims, class... Idxs>
  chunk_indexes_type<Idxs...> get_chunk_indexes(std::index_sequence<dims...>,
                                                Idxs... idxs) const;
 ...
 * @param chunk_memory_layout The layout of each chunk (default:
 * XTENSOR_DEFAULT_LAYOUT)
 *
 * @return returns a ``xchunked_array<xarray<T>>`` with the given shape, chunk
...
xchunked_array<xarray<xarray<T>>> chunked_array(
    S&& shape, S&& chunk_shape,
    layout_type chunk_memory_layout = XTENSOR_DEFAULT_LAYOUT);

template <class T, layout_type L = XTENSOR_DEFAULT_LAYOUT, class S>
xchunked_array<xarray<xarray<T>>> chunked_array(
    std::initializer_list<S> shape, std::initializer_list<S> chunk_shape,
    layout_type chunk_memory_layout = XTENSOR_DEFAULT_LAYOUT);

/**
 * Creates an in-memory chunked array.
 * This function returns a ``xchunked_array<xarray<T>>`` initialized from an
 * expression.
 *
 * @tparam L The layout_type of the array
 *
 ...
 * XTENSOR_DEFAULT_LAYOUT)
 *
 * @return returns a ``xchunked_array<xarray<T>>`` from the given expression,
 * with the given chunk shape and memory layout.
 */
template <layout_type L = XTENSOR_DEFAULT_LAYOUT, class E, class S>
xchunked_array<xarray<xarray<typename E::value_type>>> chunked_array(
    const xexpression<E>& e, S&& chunk_shape,
    layout_type chunk_memory_layout = XTENSOR_DEFAULT_LAYOUT);
 ...
 *
 * @return returns a ``xchunked_array<xarray<T>>`` from the given expression,
 * with the expression's chunk shape and the given memory layout.
 */
...
inline xchunked_array<xarray<xarray<T>>> chunked_array(
    S&& shape, S&& chunk_shape, layout_type chunk_memory_layout) {
  using chunk_storage = xarray<xarray<T, L>>;
  return xchunked_array<chunk_storage>(chunk_storage(), std::forward<S>(shape),
...
xchunked_array<xarray<xarray<T>>> chunked_array(
  ...
  auto ch_sh =
      xtl::forward_sequence<sh_type, std::initializer_list<S>>(chunk_shape);
  return chunked_array<T, L, sh_type>(std::move(sh), std::move(ch_sh),
                                      chunk_memory_layout);
}

template <layout_type L, class E, class S>
inline xchunked_array<xarray<xarray<typename E::value_type>>> chunked_array(
    const xexpression<E>& e, S&& chunk_shape, layout_type chunk_memory_layout) {
  using chunk_storage = xarray<xarray<typename E::value_type, L>>;
  return xchunked_array<chunk_storage>(
      e, chunk_storage(), std::forward<S>(chunk_shape), chunk_memory_layout);
}

template <layout_type L, class E>
inline xchunked_array<xarray<xarray<typename E::value_type>>> chunked_array(
    const xexpression<E>& e, layout_type chunk_memory_layout) {
  using chunk_storage = xarray<xarray<typename E::value_type, L>>;
...
template <class CS>
template <class S>
inline xchunked_array<CS>::xchunked_array(CS&& chunks, S&& shape,
                                          S&& chunk_shape,
                                          layout_type chunk_memory_layout)
...
template <class CS>
template <class E>
inline xchunked_array<CS>::xchunked_array(const xexpression<E>& e, CS&& chunks,
                                          layout_type chunk_memory_layout)
    : xchunked_array(e, std::move(chunks),
                     detail::chunk_helper<E>::chunk_shape(e),
                     chunk_memory_layout) {}
...
template <class CS>
template <class E>
inline auto xchunked_array<CS>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class CS>
template <class... Idxs>
inline auto xchunked_array<CS>::operator()(Idxs... idxs) const
    -> const_reference {
  auto ii = get_indexes(idxs...);
  auto& chunk = m_chunks.element(ii.first.cbegin(), ii.first.cend());
  return chunk.element(ii.second.cbegin(), ii.second.cend());
...
template <class CS>
template <class It>
inline auto xchunked_array<CS>::element(It first, It last) const
    -> const_reference {
  auto ii = get_indexes_dynamic(first, last);
  auto& chunk = m_chunks.element(ii.first.begin(), ii.first.end());
  return chunk.element(ii.second.begin(), ii.second.end());
...
template <class CS>
template <class S>
inline auto xchunked_array<CS>::stepper_begin(const S& shape) noexcept
    -> stepper {
...
template <class CS>
template <class S>
inline auto xchunked_array<CS>::stepper_end(const S& shape,
                                            layout_type) noexcept -> stepper {
  size_type offset = shape.size() - this->dimension();
...
template <class CS>
template <class S>
inline auto xchunked_array<CS>::stepper_begin(const S& shape) const noexcept
    -> const_stepper {
  size_type offset = shape.size() - this->dimension();
...
template <class CS>
inline auto xchunked_array<CS>::chunk_shape() const noexcept
    -> const shape_type& {
...
template <class CS>
inline auto xchunked_array<CS>::grid_shape() const noexcept
    -> const grid_shape_type& {
...
inline auto xchunked_array<CS>::chunk_end() -> chunk_iterator {
  shape_type sh =
      xtl::forward_sequence<shape_type, const grid_shape_type>(grid_shape());
...
inline void xchunked_array<CS>::resize(S1&& shape, S2&& chunk_shape,
  ...
  std::transform(shape.cbegin(), shape.cend(), chunk_shape.cbegin(),
                 shape_of_chunks.begin(), [](auto s, auto cs) {
                   std::size_t cn = s / cs;
                   if (s % cs > 0) cn += std::size_t(1);  // edge_chunk
                   return cn;
                 });
...
template <class CS>
template <class... Idxs>
inline auto xchunked_array<CS>::get_indexes(Idxs... idxs) const
    -> indexes_type<Idxs...> {
  auto chunk_indexes_packed =
      get_chunk_indexes(std::make_index_sequence<sizeof...(Idxs)>(), idxs...);
...
xchunked_array<CS>::get_chunk_indexes_in_dimension(std::size_t dim,
                                                   Idx idx) const {
...
template <class CS>
template <std::size_t... dims, class... Idxs>
inline auto xchunked_array<CS>::get_chunk_indexes(std::index_sequence<dims...>,
                                                  Idxs... idxs) const
    -> chunk_indexes_type<Idxs...> {
  chunk_indexes_type<Idxs...> chunk_indexes = {
      {get_chunk_indexes_in_dimension(dims, idxs)...}};
  return chunk_indexes;
}

template <class CS>
template <class T, std::size_t N>
inline auto xchunked_array<CS>::unpack(const std::array<T, N>& arr) const
    -> static_indexes_type<N> {
...
template <class CS>
template <class It>
inline auto xchunked_array<CS>::get_indexes_dynamic(It first, It last) const
    -> dynamic_indexes_type {
