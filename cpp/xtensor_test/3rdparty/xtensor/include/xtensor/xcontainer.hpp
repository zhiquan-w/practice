 ...
 */
template <class D>
class xcontainer : public xcontiguous_iterable<D>, private xaccessible<D> {
 public:
  ...
  using reverse_storage_iterator =
      typename iterable_base::reverse_storage_iterator;
  ...
  XTENSOR_CONSTEXPR_RETURN const inner_backstrides_type& backstrides()
      const noexcept;
 ...
 * container. It does not embed the data container, this responsibility is
 * delegated to the inheriting classes.
 *
 * @tparam D The derived type, i.e. the inheriting class for which
 * xstrided_container provides the partial imlpementation of xcontainer.
 */
template <class D>
class xstrided_container : public xcontainer<D> {
 ...
 protected:
  ...
  explicit xstrided_container(inner_shape_type&&, inner_strides_type&&,
                              inner_backstrides_type&&, layout_type&&) noexcept;
...
template <class D>
template <class It>
inline It xcontainer<D>::data_xend_impl(It begin, layout_type l,
                                        size_type offset) const noexcept {
 ...
 */
template <class D>
XTENSOR_CONSTEXPR_RETURN auto xcontainer<D>::dimension() const noexcept
    -> size_type {
  return shape().size();
}
 ...
 */
template <class D>
XTENSOR_CONSTEXPR_RETURN auto xcontainer<D>::shape() const noexcept
 ...
 * greater than the number of dimensions of the container.
 */
template <class D>
template <class... Args>
inline auto xcontainer<D>::operator()(Args... args) -> reference {
  ...
  size_type index = xt::data_offset<size_type>(
 ...
 * number of dimensions of the container, else the behavior is undefined.
 *
...
/**
 * Returns a pointer to the underlying array serving as element storage. The
 * pointer is such that range [data(); data() + size()] is always a valid range,
 * even if the container is empty (data() is not is not dereferenceable in that
 * case)
 */
template <class D>
...
/**
 * Returns a constant pointer to the underlying array serving as element
 * storage. The pointer is such that range [data(); data() + size()] is always a
 * valid range, even if the container is empty (data() is not is not
...
template <class D>
inline auto xcontainer<D>::derived_cast() const& noexcept
    -> const derived_type& {
...
template <class D>
template <class S>
inline auto xcontainer<D>::stepper_end(const S& shape,
                                       layout_type l) const noexcept
    -> const_stepper {
  size_type offset = shape.size() - dimension();
  return const_stepper(static_cast<const derived_type*>(this),
                       data_xend(l, offset), offset);
}
...
template <class D>
inline auto xcontainer<D>::data_xend(layout_type l,
                                     size_type offset) const noexcept
    -> const_container_iterator {
...
inline auto xcontainer<D>::load_simd(size_type i) const
    -> container_simd_return_type_t<storage_type, value_type, requested_type> {
  using align_mode = driven_align_mode_t<alignment, data_alignment>;
  return xt_simd::load_as<requested_type>(std::addressof(storage()[i]),
                                          align_mode());
}
...
inline xstrided_container<D>::xstrided_container() noexcept : base_type() {
  ...
  m_backstrides =
      xtl::make_sequence<inner_backstrides_type>(base_type::dimension(), 0);
}
...
template <class D>
inline xstrided_container<D>::xstrided_container(
    inner_shape_type&& shape, inner_strides_type&& strides,
    inner_backstrides_type&& backstrides, layout_type&& layout) noexcept
    : base_type(),
...
inline bool xstrided_container<D>::is_contiguous() const noexcept {
  ...
  return is_contiguous_container<storage_type>::value &&
         (m_strides.empty() ||
          (m_layout == layout_type::row_major &&
           m_strides.back() == str_type(1)) ||
          (m_layout == layout_type::column_major &&
...
inline void xstrided_container<D>::resize(S&& shape, bool force) {
  ...
  if (m_shape.size() != dim ||
    ...
    size_type data_size = compute_strides<D::static_layout>(
        m_shape, m_layout, m_strides, m_backstrides);
...
template <class D>
template <class S>
inline void xstrided_container<D>::resize(S&& shape,
                                          const strides_type& strides) {
  XTENSOR_ASSERT_MSG(detail::check_resize_dimension(m_shape, shape),
                     "cannot change the number of dimensions of xtensor")
  if (base_type::static_layout != layout_type::dynamic) {
    XTENSOR_THROW(std::runtime_error,
                  "Cannot resize with custom strides when layout() is != "
                  "layout_type::dynamic.");
  }
  m_shape = xtl::forward_sequence<shape_type, S>(shape);
  m_strides = strides;
...
/**
 * Reshapes the container and keeps old elements. The `shape` argument can have
 * one of its value equal to `-1`, in this case the value is inferred from the
 * number of elements in the container and the remaining values in the `shape`.
 * \code{.cpp}
 * xt::xarray<int> a = { 1, 2, 3, 4, 5, 6, 7, 8 };
 * a.reshape({-1, 4});
 * //a.shape() is {2, 4}
 * \endcode
 * @param shape the new shape (has to have same number of elements as the
 * original container)
 * @param layout the layout to compute the strides (defaults to static layout of
 * the container, or for a container with dynamic layout to
...
template <class D>
template <class T>
inline auto& xstrided_container<D>::reshape(std::initializer_list<T> shape,
                                            layout_type layout) & {
  ...
  reshape_impl(std::move(sh), xtl::is_signed<T>(),
               std::forward<layout_type>(layout));
  return this->derived_cast();
}
...
template <class D>
template <class S>
inline void xstrided_container<D>::reshape_impl(
    S&& shape, std::false_type /* is unsigned */, layout_type layout) {
  if (compute_size(shape) != this->size()) {
    XTENSOR_THROW(std::runtime_error,
                  "Cannot reshape with incorrect number of elements. Do you "
                  "mean to resize?");
  }
  if (D::static_layout == layout_type::dynamic &&
  ...
  compute_strides<D::static_layout>(m_shape, m_layout, m_strides,
                                    m_backstrides);
}
...
inline void xstrided_container<D>::reshape_impl(S&& _shape,
  ...
  if (accumulator < 0) {
    shape[neg_idx] =
        static_cast<tmp_value_type>(this->size()) / std::abs(accumulator);
