...
struct xcontainer_inner_types<xmasked_view<CTD, CTM>> {
  ...
  using const_reference =
      xtl::xmasked_value<typename data_type::const_reference,
...
class xmasked_view : public xview_semantic<xmasked_view<CTD, CTM>>,
 ...
 public:
  ...
  using bool_load_type =
      xtl::xmasked_value<typename data_type::bool_load_type, mask_type>;
...
class xmasked_view_stepper {
 public:
  ...
  using reference =
      std::conditional_t<is_const, typename masked_view_type::const_reference,
                         typename masked_view_type::reference>;
  using pointer =
      std::conditional_t<is_const, typename masked_view_type::const_pointer,
  ...
  using value_stepper =
      std::conditional_t<is_const, typename data_type::const_stepper,
                         typename data_type::stepper>;
  using mask_stepper =
      std::conditional_t<is_const, typename mask_type::const_stepper,
...
inline xmasked_view<CTD, CTM>::xmasked_view(D&& data, M&& mask)
    : m_data(std::forward<D>(data)), m_mask(std::forward<M>(mask)) {}
 ...
 */
template <class CTD, class CTM>
inline auto xmasked_view<CTD, CTM>::shape() const noexcept
 ...
 */
template <class CTD, class CTM>
inline auto xmasked_view<CTD, CTM>::strides() const noexcept
 ...
 * xmasked_view.
 * @param args a list of indices specifying the position in the xmasked_view.
 * Indices must be unsigned integers, the number of indices should be equal or
 * greater than the number of dimensions of the xmasked_view.
 */
template <class CTD, class CTM>
template <class... Args>
...
template <class CTD, class CTM>
template <class... Args>
inline auto xmasked_view<CTD, CTM>::operator()(Args... args) const
    -> const_reference {
 ...
 * number of dimensions of the  xmasked_view, else the behavior is undefined.
 *
...
template <class CTD, class CTM>
template <class... Args>
inline auto xmasked_view<CTD, CTM>::unchecked(Args... args) const
    -> const_reference {
...
template <class CTD, class CTM>
template <class It>
inline auto xmasked_view<CTD, CTM>::element(It first, It last) const
    -> const_reference {
  return const_reference(m_data.element(first, last),
                         m_mask.element(first, last));
}
//@}

template <class CTD, class CTM>
template <class S>
inline bool xmasked_view<CTD, CTM>::has_linear_assign(
 ...
 */
template <class CTD, class CTM>
inline auto xmasked_view<CTD, CTM>::visible() const noexcept
    -> const mask_type& {
  return m_mask;
}
...
template <class CTD, class CTM>
template <class S>
inline auto xmasked_view<CTD, CTM>::stepper_begin(const S& shape) noexcept
    -> stepper {
...
template <class CTD, class CTM>
template <class S>
inline auto xmasked_view<CTD, CTM>::stepper_end(const S& shape,
                                                layout_type l) noexcept
    -> stepper {
  return stepper(value().stepper_end(shape, l),
                 visible().stepper_end(shape, l));
}
...
template <class CTD, class CTM>
inline auto xmasked_view<CTD, CTM>::operator=(const self_type& rhs)
    -> self_type& {
...
template <class CTD, class CTM>
template <class E>
inline auto xmasked_view<CTD, CTM>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class CTD, class CTM>
template <class E>
inline auto xmasked_view<CTD, CTM>::operator=(const E& e)
...
template <class CTD, class CTM>
inline void xmasked_view<CTD, CTM>::assign_temporary_impl(
    temporary_type&& tmp) {
...
inline xmasked_view<CTD, CTM> masked_view(CTD&& data, CTM&& mask) {
  return xmasked_view<CTD, CTM>(std::forward<CTD>(data),
                                std::forward<CTM>(mask));
}
...
inline xmasked_view_stepper<D, C>::xmasked_view_stepper(
    value_stepper vs, mask_stepper ms) noexcept
    : m_vs(vs), m_ms(ms) {}
