...
template <class CP, class O = no_ownership,
          class A = std::allocator<
              std::remove_pointer_t<std::remove_reference_t<CP>>>>
...
class xbuffer_storage {
 public:
  ...
  template <class P>
  xbuffer_storage(P&& data, size_type size,
...
class xbuffer_owner_storage {
 public:
  ...
  template <class P>
  xbuffer_owner_storage(P&& data, size_type size,
                        const allocator_type& alloc = allocator_type());
...
struct buffer_inner_types<xiterator_adaptor<I, CI>> {
  ...
  using difference_type =
      std::common_type_t<typename traits::difference_type,
...
template <class I, class CI>
class xiterator_adaptor
    : public xbuffer_adaptor_base<xiterator_adaptor<I, CI>> {
 public:
...
template <class C, class IG>
class xiterator_owner_adaptor
    : public xbuffer_adaptor_base<xiterator_owner_adaptor<C, IG>> {
 public:
...
namespace detail {
template <class CP, class A>
inline xbuffer_storage<CP, A>::xbuffer_storage() : p_data(nullptr), m_size(0) {}

template <class CP, class A>
template <class P>
inline xbuffer_storage<CP, A>::xbuffer_storage(P&& data, size_type size,
...
inline xbuffer_owner_storage<CP, A>::xbuffer_owner_storage(
    P&& data, size_type size, const allocator_type& alloc)
...
template <class CP, class A>
inline auto xbuffer_owner_storage<CP, A>::operator=(const self_type& rhs)
    -> self_type& {
  using std::swap;
  if (this != &rhs) {
    ...
    if (xtrivially_default_constructible<value_type>::value) {
      std::uninitialized_copy(rhs.m_data.get(), rhs.m_data.get() + rhs.m_size,
                              tmp);
    } else {
...
template <class CP, class A>
inline auto xbuffer_owner_storage<CP, A>::operator=(self_type&& rhs)
    -> self_type& {
  swap(rhs);
  return *this;
}
...
template <class CP, class A>
inline auto xbuffer_owner_storage<CP, A>::data() const noexcept
    -> const_pointer {
  return m_data.get();
}

template <class CP, class A>
inline auto xbuffer_owner_storage<CP, A>::get_allocator() const noexcept
    -> allocator_type {
...
template <class D>
inline auto xbuffer_adaptor_base<D>::operator[](size_type i) const
    -> const_reference {
  return derived_cast().data()[static_cast<index_type>(i)];
}
...
template <class D>
inline auto xbuffer_adaptor_base<D>::rbegin() const noexcept
    -> const_reverse_iterator {
  return const_reverse_iterator(end());
}

template <class D>
inline auto xbuffer_adaptor_base<D>::rend() const noexcept
    -> const_reverse_iterator {
  return const_reverse_iterator(begin());
}

template <class D>
inline auto xbuffer_adaptor_base<D>::crbegin() const noexcept
    -> const_reverse_iterator {
  return rbegin();
}

template <class D>
inline auto xbuffer_adaptor_base<D>::crend() const noexcept
    -> const_reverse_iterator {
  return rend();
}
...
template <class D>
inline auto xbuffer_adaptor_base<D>::derived_cast() const noexcept
    -> const derived_type& {
...
template <class CP, class O, class A>
inline auto xbuffer_adaptor<CP, O, A>::operator=(temporary_type&& tmp)
    -> self_type& {
...
template <class I, class CI>
inline auto xiterator_adaptor<I, CI>::operator=(const temporary_type& rhs)
    -> self_type& {
  resize(rhs.size());
...
template <class I, class CI>
inline auto xiterator_adaptor<I, CI>::operator=(temporary_type&& rhs)
    -> self_type& {
  return (*this = rhs);
}
...
template <class C, class IG>
inline xiterator_owner_adaptor<C, IG>::xiterator_owner_adaptor(
    const self_type& rhs)
...
template <class C, class IG>
inline xiterator_owner_adaptor<C, IG>&
xiterator_owner_adaptor<C, IG>::operator=(const self_type& rhs) {
  m_container = rhs.m_container;
  init_iterators();
}
...
template <class C, class IG>
inline xiterator_owner_adaptor<C, IG>&
xiterator_owner_adaptor<C, IG>::operator=(self_type&& rhs) {
  m_container = std::move(rhs.m_container);
  init_iterators();
}
...
template <class C, class IG>
inline xiterator_owner_adaptor<C, IG>&
xiterator_owner_adaptor<C, IG>::operator=(const temporary_type& rhs) {
  resize(rhs.size());
  std::copy(rhs.cbegin(), rhs.cend(), m_it);
  return *this;
}
...
template <class C, class IG>
inline xiterator_owner_adaptor<C, IG>&
xiterator_owner_adaptor<C, IG>::operator=(temporary_type&& rhs) {
  return (*this = rhs);
}
...
template <class C, class IG>
inline auto xiterator_owner_adaptor<C, IG>::data() const noexcept
    -> const_iterator {
  return m_cit;
}
...
struct xiterator_adaptor_builder<C, IG, false> {
  ...
  inline static type build(C&& c) { return type(std::move(c)); }
};
}  // namespace detail
