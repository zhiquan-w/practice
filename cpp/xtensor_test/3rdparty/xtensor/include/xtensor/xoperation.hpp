...
#define DEFINE_COMPLEX_OVERLOAD(OP)                                          \
  template <class T1, class T2,                                              \
            XTL_REQUIRES(xtl::negation<std::is_same<T1, T2>>)>               \
  constexpr auto operator OP(const std::complex<T1>& arg1,                   \
                             const std::complex<T2>& arg2) {                 \
    using result_type =                                                      \
        typename xtl::promote_type_t<std::complex<T1>, std::complex<T2>>;    \
...
struct conditional_ternary {
  ...
  constexpr auto operator()(const B& cond, const A1& v1,
                            const A2& v2) const noexcept {
    return xtl::select(cond, v1, v2);
  }

  template <class B>
  constexpr B simd_apply(const get_batch_bool<B>& t1, const B& t2,
...
// condition is false. Wrapping the xfunction type in the xfunction_type
// metafunction avoids this evaluation when the condition is false, since it
// leads to a tricky bug preventing from using operator+ and operator- on vector
// and arrays iterators.
template <class F, class... E>
...
inline auto operator+(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::plus>(std::forward<E1>(e1),
                                              std::forward<E2>(e2));
}
...
inline auto operator-(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::minus>(std::forward<E1>(e1),
                                               std::forward<E2>(e2));
}
...
inline auto operator/(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::divides>(std::forward<E1>(e1),
                                                 std::forward<E2>(e2));
}
...
inline auto operator%(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::modulus>(std::forward<E1>(e1),
                                                 std::forward<E2>(e2));
}
...
namespace detail {
// Shift operator is not available for all the types, so the xfunction type
// instantiation has to be delayed, enable_if_t is not sufficient
...
template <class F, class E1, class E2>
using shift_return_type_t =
    eval_enable_if_t<is_xexpression<std::decay_t<E1>>::value,
...
inline auto operator<(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::less>(std::forward<E1>(e1),
                                              std::forward<E2>(e2));
}
...
inline auto operator>(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::greater>(std::forward<E1>(e1),
                                                 std::forward<E2>(e2));
}
...
inline auto equal(E1&& e1, E2&& e2) noexcept
  ...
  return detail::make_xfunction<detail::equal_to>(std::forward<E1>(e1),
                                                  std::forward<E2>(e2));
}
 ...
 * @param arr input array
 * @return vector of index_types where arr is not equal to zero (use
 * `xt::from_indices` to convert)
 *
 * @sa xt::from_indices
 */
...
inline auto cast(E&& e) noexcept
  ...
  return detail::make_xfunction<typename detail::cast<R>::functor>(
      std::forward<E>(e));
}
