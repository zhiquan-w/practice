...
template <class E, class S, class Tag = check_policy::none,
          std::enable_if_t<!xtl::is_integral<S>::value, int> = 0>
...
inline auto transpose_impl(E&& e, S&& permutation, check_policy::none) {
  ...
  if (std::is_sorted(std::begin(permutation), std::end(permutation))) {
    // keep old layout
    new_layout = e.layout();
  } else if (std::is_sorted(std::begin(permutation), std::end(permutation),
                            std::greater<>())) {
    new_layout = transpose_layout_noexcept(e.layout());
  }

  return strided_view(std::forward<E>(e), std::move(temp_shape),
                      std::move(temp_strides),
                      get_offset<XTENSOR_DEFAULT_LAYOUT>(e), new_layout);
}
...
inline void compute_transposed_strides(E&&, const S& shape, X& strides) {
  ...
  // makes use of a flat storage adaptor that has layout
  // XTENSOR_DEFAULT_TRAVERSAL which should be the one inverted.
...
inline auto transpose(E&& e) noexcept {
  ...
  return strided_view(std::forward<E>(e), std::move(shape), std::move(strides),
...
inline auto transpose(E&& e, const I (&permutation)[N],
...
struct expression_iterator_getter {
  ...
  inline static iterator begin(E& e) { return e.template begin<L>(); }

  inline static const_iterator cbegin(E& e) { return e.template cbegin<L>(); }

  inline static auto size(E& e) { return e.size(); }
};
}  // namespace detail
...
inline auto ravel(E&& e) {
  ...
  using iterator_getter =
      detail::expression_iterator_getter<std::remove_reference_t<E>, L>;
  auto size = e.size();
  auto adaptor = make_xiterator_adaptor(std::forward<E>(e), iterator_getter());
  constexpr layout_type layout =
      std::is_pointer<iterator>::value ? L : layout_type::dynamic;
  using type = xtensor_view<decltype(adaptor), 1, layout,
                            extension::get_expression_tag_t<E>>;
 ...
 * @param direction string of either 'f' for trim from beginning, 'b' for trim
 * from end or 'fb' (default) for both.
 * @return returns a view without zeros at the beginning and end
 */
template <class E>
inline auto trim_zeros(E&& e, const std::string& direction) {
  XTENSOR_ASSERT_MSG(e.dimension() == 1,
...
inline auto squeeze_impl(E&& e, S&& axis, check_policy::full) {
  ...
  return squeeze_impl(std::forward<E>(e), std::forward<S>(axis),
                      check_policy::none());
}
}  // namespace detail
 ...
 *
 * @param e input xexpression
 * @param axis integer or container of integers, select a subset of
 * single-dimensional entries of the shape.
 * @param check_policy select check_policy. With check_policy::full(), selecting
 * an axis which is greater than one will throw a runtime_error.
 */
template <class E, class S, class Tag,
          std::enable_if_t<!xtl::is_integral<S>::value, int>>
inline auto squeeze(E&& e, S&& axis, Tag check_policy) {
  return detail::squeeze_impl(std::forward<E>(e), std::forward<S>(axis),
                              check_policy);
}
...
inline auto squeeze(E&& e, const I (&axis)[N], Tag check_policy = Tag()) {
  ...
  return detail::squeeze_impl(
      std::forward<E>(e), xtl::forward_sequence<arr_t, decltype(axis)>(axis),
      check_policy);
}

template <class E, class Tag = check_policy::none>
inline auto squeeze(E&& e, std::size_t axis, Tag check_policy = Tag()) {
  return squeeze(std::forward<E>(e), std::array<std::size_t, 1>{axis},
                 check_policy);
}
/// @endcond
 ...
 * shape. This will return a ``strided_view`` with a ``xt::newaxis()`` at the
 * indicated axis.
 *
 * @param e input xexpression
...
inline auto atleast_Nd(E&& e) {
  ...
  if (e.dimension() < N) {
    ...
    std::size_t end = static_cast<std::size_t>(
        std::round(double(N - e.dimension()) / double(N)));
    for (; i < end; ++i) {
      sv[i] = newaxis();
    }
    i += e.dimension();
 ...
 *
 * @param e the input xexpression
 * @param axes the array is rotated in the plane defined by the axes. Axes must
 * be different.
...
inline auto roll(E&& e, std::ptrdiff_t shift) {
  ...
  auto flat_size = std::accumulate(cpy.shape().begin(), cpy.shape().end(), 1L,
...
To roll(To to, From from, std::ptrdiff_t shift, std::size_t axis,
        S const& shape, std::size_t M) {
  std::ptrdiff_t dim = std::ptrdiff_t(shape[M]);
  std::ptrdiff_t offset =
      std::accumulate(shape.begin() + M + 1, shape.end(), std::ptrdiff_t(1),
                      std::multiplies<std::ptrdiff_t>());
  if (shape.size() == M + 1) {
    if (axis == M) {
      const auto split = from + (dim - shift) * offset;
      for (auto iter = split, end = from + dim * offset; iter != end;
           iter += offset, ++to) {
        *to = *iter;
      }
      ...
      for (auto iter = from, end = from + dim * offset; iter != end;
           iter += offset, ++to) {
        *to = *iter;
      }
    }
...
inline auto make_xrepeat(E&& e, R&& r,
                         typename std::decay_t<E>::size_type axis) {
  const auto casted_axis =
      static_cast<typename std::decay_t<E>::size_type>(axis);
  if (r.size() != e.shape(casted_axis)) {
    XTENSOR_THROW(std::invalid_argument,
                  "repeats must have the same size as the specified axis");
  }
  return xrepeat<const_xclosure_t<E>, R>(std::forward<E>(e), std::forward<R>(r),
                                         axis);
}
}  // namespace detail
 ...
 *
 * @param e the input xexpression
 * @param repeats The number of repetition of each elements. \ref repeats is
 * broadcasted to fit the shape of the given \ref axis.
 * @param axis the axis along which to repeat the value
 *
 * @return an expression which as the same shape as \ref e, except along the
 * given \ref axis
 */
template <class E>
 ...
 *
 * @param e the input xexpression
 * @param repeats The number of repetition of each elements. The size of \ref
 * repeats must match the shape of the given \ref axis.
 * @param axis the axis along which to repeat the value
 *
 * @return an expression which as the same shape as \ref e, except along the
 * given \ref axis
 */
template <class E>
inline auto repeat(E&& e, const std::vector<std::size_t>& repeats,
                   std::size_t axis) {
 ...
 */
template <class E>
inline auto repeat(E&& e, std::vector<std::size_t>&& repeats,
                   std::size_t axis) {
