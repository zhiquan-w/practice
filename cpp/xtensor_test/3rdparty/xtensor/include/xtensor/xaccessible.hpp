 ...
 * @brief Base class for implementation of common expression constant access
 * methods.
 *
 * The xaccessible class implements constant access methods common to all
 * expressions.
 *
 * @tparam D The derived type, i.e. the inheriting class for which
 * xconst_accessible provides the interface.
 */
template <class D>
class xconst_accessible {
 public:
 ...
 * expression, after dimension and bounds checking.
 * @param args a list of indices specifying the position in the expression.
 * Indices must be unsigned integers, the number of indices should be equal to
 ...
 * Returns a constant reference to the element at the specified position in the
 * expression.
 * @param index a sequence of indices specifying the position in the expression.
 * Indices must be unsigned integers, the number of indices in the list should
 * be equal or greater than the number of dimensions of the expression.
 */
template <class D>
template <class S>
...
/**
 * Returns a constant reference to the element at the specified position in the
 * expression, after applying periodicity to the indices (negative and
 * 'overflowing' indices are changed).
 * @param args a list of indices specifying the position in the expression.
 * Indices must be integers, the number of indices should be equal to the number
 * of dimensions of the expression.
 */
template <class D>
template <class... Args>
inline auto xconst_accessible<D>::periodic(Args... args) const
    -> const_reference {
...
template <class D>
inline auto xconst_accessible<D>::derived_cast() const noexcept
    -> const derived_type& {
...
template <class D>
template <class I>
inline auto xaccessible<D>::operator[](std::initializer_list<I> index)
    -> reference {
  return derived_cast().element(index.begin(), index.end());
}
