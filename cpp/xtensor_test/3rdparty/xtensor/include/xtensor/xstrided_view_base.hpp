...
class flat_expression_adaptor {
 public:
  ...
  using reference =
      std::conditional_t<std::is_const<std::remove_reference_t<CT>>::value,
  ...
  using reverse_iterator = decltype(std::declval<std::remove_reference_t<CT>>()
                                        .template rbegin<L>());
...
template <class CT, layout_type L>
struct is_flat_expression_adaptor<flat_expression_adaptor<CT, L>>
    : std::true_type {};
...
class xstrided_view_base : public xaccessible<D> {
 public:
  ...
  xstrided_view_base(CTA&& e, SA&& shape, strides_type&& strides,
...
struct inner_storage_getter {
  ...
  static decltype(auto) get_flat_storage(reference e) { return e.storage(); }

  static auto get_offset(reference e) { return e.data_offset(); }

  static decltype(auto) get_strides(reference e) { return e.strides(); }
};
...
template <class CT, layout_type L>
using flat_storage_getter =
    std::conditional_t<has_data_interface<std::decay_t<CT>>::value,
                       inner_storage_getter<CT>, flat_adaptor_getter<CT, L>>;
...
template <class D>
template <class CTA, class SA>
inline xstrided_view_base<D>::xstrided_view_base(CTA&& e, SA&& shape,
...
template <class D>
template <class... Args>
inline auto xstrided_view_base<D>::operator()(Args... args) const
    -> const_reference {
  XTENSOR_TRY(check_index(shape(), args...));
  XTENSOR_CHECK_DIMENSION(shape(), args...);
  offset_type index = compute_index(args...);
...
template <class D>
template <class... Args>
inline auto xstrided_view_base<D>::unchecked(Args... args) const
    -> const_reference {
  offset_type index = compute_unchecked_index(args...);
  return m_storage[static_cast<size_type>(index)];
}
...
template <class D>
template <class It>
inline auto xstrided_view_base<D>::element(It first, It last) const
    -> const_reference {
  XTENSOR_TRY(check_element_index(shape(), first, last));
  return m_storage[static_cast<size_type>(compute_element_index(first, last))];
}
 ...
 */
template <class D>
inline auto xstrided_view_base<D>::storage() const noexcept
    -> const storage_type& {
  return m_storage;
}
...
inline auto xstrided_view_base<D>::data() noexcept
    -> std::enable_if_t<detail::provides_data_interface<E, ST>::value,
                        pointer> {
  return m_e.data();
}
 ...
 */
template <class D>
inline auto xstrided_view_base<D>::expression() const noexcept
...
/**
 * Checks whether the xstrided_view_base can be linearly assigned to an
 * expression with the specified strides.
 * @return a boolean indicating whether a linear assign is possible
 */
template <class D>
template <class O>
inline bool xstrided_view_base<D>::has_linear_assign(
    const O& str) const noexcept {
  return has_data_interface<xexpression_type>::value &&
         str.size() == strides().size() &&
...
template <class D>
template <class... Args>
inline auto xstrided_view_base<D>::compute_index(Args... args) const
    -> offset_type {
  return static_cast<offset_type>(m_offset) +
         xt::data_offset<offset_type>(strides(),
                                      static_cast<offset_type>(args)...);
}

template <class D>
template <class... Args>
inline auto xstrided_view_base<D>::compute_unchecked_index(Args... args) const
    -> offset_type {
  return static_cast<offset_type>(m_offset) +
         xt::unchecked_data_offset<offset_type>(
             strides(), static_cast<offset_type>(args)...);
}

template <class D>
template <class It>
inline auto xstrided_view_base<D>::compute_element_index(It first,
                                                         It last) const
    -> offset_type {
  return static_cast<offset_type>(m_offset) +
...
namespace detail {
template <class CT, layout_type L>
inline flat_expression_adaptor<CT, L>::flat_expression_adaptor(CT* e) : m_e(e) {
...
template <class CT, layout_type L>
inline auto flat_expression_adaptor<CT, L>::operator[](size_type idx)
    -> reference {
...
struct slice_getter_impl {
  ...
  explicit slice_getter_impl(const S& shape) : m_shape(shape), idx(0) {}
...
struct strided_view_args : adj_strides_policy {
  ...
  void fill_args(const S& shape, ST&& old_strides, std::size_t base_offset,
                 layout_type layout, const V& slices) {
    // Compute dimension
    ...
    for (; i < slices.size(); ++i) {
      i_ax =
          static_cast<std::size_t>(static_cast<std::ptrdiff_t>(i) - axis_skip);
      ...
      if (ptr != nullptr) {
        ...
        base_type::set_fake_slice(idx);
        ++idx;
      } else if (base_type::fill_args(slices, i, idx, old_shape[i_ax],
                                      old_strides[i_ax], new_shape,
                                      new_strides)) {
        ++idx;
      } else {
