...
#define XTENSOR_UNSIGNED_ABS_FUNC(T) \
  constexpr inline T abs(const T& x) { return x; }
...
template <class T>
inline std::enable_if_t<xtl::is_arithmetic<T>::value, bool> isfinite(
    const T& t) {
...
#endif
// The following specializations are needed to avoid 'ambiguous overload'
// errors, whereas 'unsigned char' and 'unsigned short' are automatically
// converted to 'int'. we're still adding those functions to silence warnings
...
#define XTENSOR_REDUCER_FUNCTION(NAME, FUNCTOR, INIT_VALUE_TYPE, INIT)        \
  template <class T = void, class E, class X,                                 \
            ...
            XTL_REQUIRES(xtl::negation<is_reducer_options<X>>,                \
                         xtl::negation<xtl::is_integral<std::decay_t<X>>>)>   \
  inline auto NAME(E&& e, X&& axes, EVS es = EVS()) {                         \
    using init_value_type =                                                   \
        std::conditional_t<std::is_same<T, void>::value, INIT_VALUE_TYPE, T>; \
    using functor_type = FUNCTOR;                                             \
    using init_value_fct = xt::const_value<init_value_type>;                  \
    return xt::reduce(                                                        \
        make_xreducer_functor(                                                \
            functor_type(),                                                   \
...
struct deg2rad {
  ...
  template <class A,
            std::enable_if_t<std::is_floating_point<A>::value, int> = 0>
...
XTENSOR_REDUCER_FUNCTION(
    amax, math::maximum<void>, typename std::decay_t<E>::value_type,
 ...
 */
XTENSOR_REDUCER_FUNCTION(
    amin, math::minimum<void>, typename std::decay_t<E>::value_type,
...
template <class F>
struct lambda_adapt {
  explicit lambda_adapt(F&& lmbd) : m_lambda(std::move(lmbd)) {}
 ...
 *
 * This function can be used to easily create performant xfunctions from
 * lambdas:
 *
 * \code{cpp}
 * template <class E1>
 ...
 * \endcode
 *
 * Lambda function allow the reusal of a single arguments in multiple places
 * (otherwise only correctly possible when using xshared_expressions). ``auto``
 * lambda functions are automatically vectorized with ``xsimd`` if possible
...
#define XTENSOR_GCC_VERSION \
  (__GNUC__ * 10000 + __GNUC_MINOR__ * 100 + __GNUC_PATCHLEVEL__)

// Workaround for MSVC 2015 & GCC 4.9
#if (defined(_MSC_VER) && _MSC_VER < 1910) || \
    (defined(__GNUC__) && GCC_VERSION < 49999)
 ...
 *
 * Instead of computing the power by using the (expensive) logarithm, this
 * function computes the power in a number of straight-forward multiplication
 * steps. This function is therefore much faster (even for high N) than the
 * generic pow-function.
 *
 * For example, `e1^20` can be expressed as `(((e1^2)^2)^2)^2*(e1^2)^2`, which
 * is just 5 multiplications.
 *
 * @param e an \ref xexpression
...
inline auto make_xfunction(std::tuple<A...>&& f_args, E&&... e) noexcept {
  ...
  using type = select_xfunction_expression_t<expression_tag, functor_type,
                                             const_xclosure_t<E>...>;
  auto functor =
...
struct isclose {
  ...
  bool operator()(const A1& a, const A2& b) const {
    ...
    return d <= m_atol ||
           d <= m_rtol * double((std::max)(math::abs(internal_type(a)),
 ...
 */
template <class E1, class E2>
inline auto isclose(E1&& e1, E2&& e2, double rtol = 1e-05, double atol = 1e-08,
 ...
 */
template <class E1, class E2>
inline auto allclose(E1&& e1, E2&& e2, double rtol = 1e-05,
 ...
 *           You can pass `big_promote_value_type_t<E>` to avoid overflow in
 * computation.
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(sum, detail::plus,
 ...
 * @tparam T the value type used for internal computation. The default is
 `E::value_type`.
 *           `T` is also used for determining the value type of the result,
 which is the type
 *           of `T() * E::value_type()`.
 *           You can pass `big_promote_value_type_t<E>` to avoid overflow in
 computation.
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(prod, detail::multiplies,
                         typename std::decay_t<E>::value_type, 1)
...
inline auto mean_division(S&& s, ST e_size) {
  ...
  value_type div = s.size() != ST(0)
                       ? static_cast<value_type>(e_size / s.size())
                       : value_type(0);
...
inline auto mean(E&& e, X&& axes, D const& ddof, EVS es) {
  // sum cannot always be a double. It could be a complex number which cannot
  // operate on std::plus<double>.
...
inline auto mean_noaxis(E&& e, const D& ddof, EVS es) {
  ...
  return std::move(s) /
         static_cast<value_type>((size - static_cast<size_type>(ddof)));
}
}  // namespace detail
 ...
 * \em axes.
 * @param e an \ref xexpression
 * @param weights \ref xexpression containing weights associated with the values
 * in \ref e
 * @param axes the axes along which the mean is computed (optional)
 * @tparam T the value type used for internal computation. The default is
 *           `E::value_type`. `T`is also used for determining the value type of
 * the result, which is the type of `T() + E::value_type(). You can pass
 * `big_promote_value_type_t<E>` to avoid overflow in computation.
 * @return an \ref xexpression
 *
 * @sa mean
 */
template <class T = void, class E, class W, class X,
          class EVS = DEFAULT_STRATEGY_REDUCERS,
          XTL_REQUIRES(is_reducer_options<EVS>,
                       xtl::negation<xtl::is_integral<X>>)>
inline auto average(E&& e, W&& weights, X&& axes, EVS ev = EVS()) {
  ...
  constexpr layout_type L =
      default_assignable_layout(std::decay_t<W>::static_layout);
  auto weights_view =
      reshape_view<L>(std::forward<W>(weights), std::move(broadcast_shape));
  auto scl = sum<T>(weights_view, ax, xt::evaluation_strategy::immediate);
  return sum<T>(std::forward<E>(e) * std::move(weights_view), std::move(ax),
                ev) /
         std::move(scl);
}
...
inline auto average(E&& e, W&& weights, X axis, EVS ev = EVS()) {
  return average(std::forward<E>(e), std::forward<W>(weights), {axis},
                 std::forward<EVS>(ev));
}

template <class T = void, class E, class W, class X, std::size_t N,
          class EVS = DEFAULT_STRATEGY_REDUCERS>
inline auto average(E&& e, W&& weights, const X (&axes)[N], EVS ev = EVS()) {
  ...
  return average<T>(std::forward<E>(e), std::forward<W>(weights),
                    xt::forward_normalize<ax_t>(e, axes), ev);
}

template <class T = void, class E, class W,
          class EVS = DEFAULT_STRATEGY_REDUCERS,
          XTL_REQUIRES(is_reducer_options<EVS>)>
inline auto average(E&& e, W&& weights, EVS ev = EVS()) {
  if (weights.dimension() != e.dimension() ||
      !std::equal(weights.shape().begin(), weights.shape().end(),
                  e.shape().begin())) {
  ...
  auto s = sum<T>(std::forward<E>(e) * std::forward<W>(weights), ev) /
           std::move(div);
  return s;
}
...
namespace detail {
template <typename E>
std::enable_if_t<std::is_lvalue_reference<E>::value, E> shared_forward(
    E e) noexcept {
  return e;
}
 ...
 * @sa stddev, mean
 */
template <class T = void, class E, class X, class D,
          class EVS = DEFAULT_STRATEGY_REDUCERS,
          XTL_REQUIRES(xtl::negation<is_reducer_options<X>>,
                       xtl::is_integral<D>)>
inline auto variance(E&& e, X&& axes, const D& ddof, EVS es = EVS()) {
  ...
  auto inner_mean =
      eval(mean<T>(sc, std::move(axes_copy), evaluation_strategy::immediate));
  ...
  auto mrv = reshape_view<XTENSOR_DEFAULT_LAYOUT>(std::move(inner_mean),
                                                  std::move(keep_dim_shape));
  return detail::mean<T>(square(sc - std::move(mrv)), std::forward<X>(axes),
                         ddof, es);
}
 ...
 *
 * @param e an \ref xexpression
 * @param axes the axes along which the standard deviation is computed
 * (optional)
 * @param es evaluation strategy to use (lazy (default), or immediate)
 * @tparam T the value type used for internal computation. The default is
 *           `E::value_type`. `T`is also used for determining the value type of
...
template <class T = void, class E, class A, std::size_t N,
          class EVS = DEFAULT_STRATEGY_REDUCERS>
...
template <class T = void, class E, class A, std::size_t N, class D,
          class EVS = DEFAULT_STRATEGY_REDUCERS>
 ...
 *
 * Returns an \ref xreducer for the minimum and maximum of an expression's
 * elements.
 * @param e an \ref xexpression
 * @param es evaluation strategy to use (lazy (default), or immediate)
 * @return an \ref xexpression of type ``std::array<value_type, 2>``, whose
 * first and second element represent the minimum and maximum respectively
 */
...
inline auto minmax(E&& e, EVS es = EVS()) {
  ...
  auto init_func =
      init_value_fct(result_type{std::numeric_limits<value_type>::max(),
  ...
  return xt::reduce(
      make_xreducer_functor(std::move(reduce_func), std::move(init_func),
                            std::move(merge_func)),
...
inline auto cumsum(E&& e, std::ptrdiff_t axis) {
  ...
  return accumulate(
      make_xaccumulator_functor(
          detail::plus(), detail::accumulator_identity<init_value_type>()),
      std::forward<E>(e), axis);
}
 ...
 * \em axis (or flattened).
 * @param e an \ref xexpression
 * @param axis the axes along which the cumulative product is computed
 * (optional)
 * @tparam T the value type used for internal computation. The default is
 *           `E::value_type`. `T`is also used for determining the value type of
 * the result, which is the type of `T() * E::value_type()`. You can pass
...
inline auto cumprod(E&& e, std::ptrdiff_t axis) {
  ...
  return accumulate(make_xaccumulator_functor(
                        detail::multiplies(),
                        detail::accumulator_identity<init_value_type>()),
                    std::forward<E>(e), axis);
}
    ...
    return math::isnan(lhs)
               ...
               : (math::isnan(rhs)
                      ...
                      : std::common_type_t<T, U>(
                            detail::make_xfunction<math::minimum<void>>(lhs,
                                                                        rhs)));
  }
};
  ...
  constexpr auto operator()(const T lhs, const U rhs) const {
    ...
    return math::isnan(lhs)
               ...
               : (math::isnan(rhs)
                      ? lhs
                      : std::common_type_t<T, U>(
                            detail::make_xfunction<math::maximum<void>>(lhs,
                                                                        rhs)));
  }
};
 ...
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(nanmin, detail::nan_min,
                         typename std::decay_t<E>::value_type, std::nan("0"))
 ...
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(nanmax, detail::nan_max,
                         typename std::decay_t<E>::value_type, std::nan("0"))
 ...
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(nansum, detail::nan_plus,
                         typename std::decay_t<E>::value_type, 0)
 ...
 * @return an \ref xreducer
 */
XTENSOR_REDUCER_FUNCTION(nanprod, detail::nan_multiplies,
                         typename std::decay_t<E>::value_type, 1)
...
#define COUNT_NON_ZEROS_CONTENT                                             \
  ...
  auto reduce_fct = [](const auto& lhs, const auto& rhs) {                  \
    using value_t =                                                         \
        xt::detail::xreducer_temporary_type_t<std::decay_t<decltype(rhs)>>; \
...
inline auto count_nonzero(E&& e, EVS es = EVS()) {
  ...
  return xt::reduce(
      make_xreducer_functor(std::move(reduce_fct), std::move(init_fct),
                            std::move(merge_func)),
      std::forward<E>(e), es);
}
...
inline auto count_nonnan(E&& e, X&& axes, EVS es = EVS()) {
  return xt::count_nonzero(!xt::isnan(std::forward<E>(e)),
...
inline auto nancumsum(E&& e, std::ptrdiff_t axis) {
  using init_value_type =
      std::conditional_t<std::is_same<T, void>::value,
                         typename std::decay_t<E>::value_type, T>;
  return accumulate(
      make_xaccumulator_functor(detail::nan_plus(),
                                detail::nan_init<init_value_type, 0>()),
      std::forward<E>(e), axis);
}
...
inline auto nancumprod(E&& e, std::ptrdiff_t axis) {
  using init_value_type =
      std::conditional_t<std::is_same<T, void>::value,
                         typename std::decay_t<E>::value_type, T>;
  return accumulate(
      make_xaccumulator_functor(detail::nan_multiplies(),
                                detail::nan_init<init_value_type, 1>()),
      std::forward<E>(e), axis);
}
...
struct diff_impl {
  ...
  inline void operator()(Arg& ad, const std::size_t& n,
                         ...
                         xstrided_slice_vector& slice2, std::size_t saxis) {
...
inline auto nanmean(E&& e, X&& axes, EVS es = EVS()) {
  ...
  using sum_type = typename std::conditional_t<
      ...
      typename std::common_type_t<typename std::decay_t<E>::value_type,
                                  value_type>,
      T>;
  // sum cannot always be a double. It could be a complex number which cannot
  // operate on std::plus<double>.
...
inline auto nanvar(E&& e, X&& axes, EVS es = EVS()) {
  ...
  auto mrv = reshape_view<XTENSOR_DEFAULT_LAYOUT>(std::move(inner_mean),
                                                  std::move(keep_dim_shape));
  return nanmean<result_type>(square(cast<result_type>(sc) - std::move(mrv)),
                              std::forward<X>(axes), es);
}

/**
 * @ingroup nan_functions
 * @brief Compute the standard deviation along the specified axis, excluding
 * nans.
 *
 * Returns the standard deviation, a measure of the spread of a distribution,
 * of the array elements. The standard deviation is computed for the flattened
 ...
 *
 * Calculate the n-th discrete difference along the given axis. This function is
 * not lazy (might change in the future).
 * @param a an \ref xexpression
 * @param n The number of times values are differenced. If zero, the input is
 * returned as-is. (optional)
 * @param axis The axis along which the difference is taken, default is the last
 * axis.
 * @return an xarray
 */
template <class T>
auto diff(const xexpression<T>& a, std::size_t n = 1,
 ...
 *
 * Returns definite integral as approximated by trapezoidal rule. This function
 * is not lazy (might change in the future).
 ...
 * is not lazy (might change in the future).
 * @param y an \ref xexpression
 * @param x an \ref xexpression representing the sample points corresponding to
 * the y values.
 * @param axis the axis along which to integrate.
 * @return an xarray
 */
template <class T, class E>
auto trapz(const xexpression<T>& y, const xexpression<E>& x,
           std::ptrdiff_t axis = -1) {
  auto& yd = y.derived_cast();
...
/**
 * @ingroup basic_functions
 * @brief Returns the one-dimensional piecewise linear interpolant to a function
...
inline auto interp(const E1& x, const E2& xp, const E3& fp, T left, T right) {
  ...
  // (index counts one right, to terminate the reverse loop, without risking
  // being negative)
...
inline auto cov(const E1& x, const E1& y = E1()) {
  ...
  if (y.dimension() == 0) {
    ...
    if (x.dimension() == 1) {
      ...
      covar(0, 0) = std::inner_product(x_norm.begin(), x_norm.end(),
                                       x_norm.begin(), 0.0) /
                    value_type(s[0] - 1);
      return covar;
    }
    ...
    for (size_type i = 0; i < s[0]; i++) {
      ...
      for (size_type j = i; j < s[0]; j++) {
        ...
        covar(j, i) =
            std::inner_product(xi.begin(), xi.end(), xj.begin(), 0.0) /
            value_type(s[1] - 1);
      }
    }
