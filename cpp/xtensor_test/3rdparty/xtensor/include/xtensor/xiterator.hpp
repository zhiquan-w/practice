...
class xindexed_stepper {
 public:
  ...
  using reference =
      std::conditional_t<is_const, typename xexpression_type::const_reference,
  ...
  xindexed_stepper(xexpression_type* e, size_type offset,
...
class xiterator : public xtl::xrandom_access_iterator_base<
                      xiterator<St, S, L>, typename St::value_type,
                      typename St::difference_type, typename St::pointer,
...
template <class C>
struct has_storage_iterator<
    C, void_t<decltype(std::declval<C>().storage_cbegin())>> : std::true_type {
};
}  // namespace detail
...
template <class C>
inline xstepper<C>::xstepper(storage_type* c, subiterator_type it,
...
inline void xstepper<C>::step(size_type dim, size_type n) {
  if (dim >= m_offset) {
    using strides_value_type =
        typename std::decay_t<decltype(p_c->strides())>::value_type;
...
inline auto xstepper<C>::step_simd() -> simd_return_type<T> {
  ...
  simd_type reg =
      detail::step_simd_invoker<subiterator_type>::template apply<simd_type>(
          m_it);
...
void stepper_tools<layout_type::row_major>::increment_stepper(
    S& stepper, IT& index, const ST& shape, typename S::size_type n) {
  using size_type = typename S::size_type;
  size_type i = index.size();
...
template <class St, class S, layout_type L>
inline xiterator<St, S, L>::xiterator(St st, shape_param_type shape,
                                      bool end_index)
    : private_base(shape),
      m_st(st),
      m_index(end_index ? xtl::forward_sequence<index_type, const shape_type&>(
                              this->shape())
                        : xtl::make_sequence<index_type>(this->shape().size(),
                                                         size_type(0))),
      m_linear_index(0) {
  // end_index means either reverse_iterator && !end or !reverse_iterator && end
  if (end_index) {
    ...
    m_linear_index = difference_type(
        std::accumulate(this->shape().cbegin(), this->shape().cend(),
...
template <class St, class S, layout_type L>
inline auto xiterator<St, S, L>::operator-(const self_type& rhs) const
    -> difference_type {
...
template <class It, class BIt>
inline auto xbounded_iterator<It, BIt>::operator+=(difference_type n)
    -> self_type& {
  m_it += n;
  m_bound_it += n;
  return *this;
...
template <class It, class BIt>
inline auto xbounded_iterator<It, BIt>::operator-(const self_type& rhs) const
    -> difference_type {
