...
/**
 * @ingroup digitize
 * @brief Return the indices of the bins to which each value in input array
 * belongs.
 *
 * @param data The data.
...
inline auto digitize(E1&& data, E2&& bin_edges, bool right = false) {
  ...
  return xt::searchsorted(std::forward<E2>(bin_edges), std::forward<E1>(data),
                          right);
}

namespace detail {
template <class R = double, class E1, class E2, class E3>
inline auto histogram_imp(E1&& data, E2&& bin_edges, E3&& weights, bool density,
                          bool equal_bins) {
  using size_type =
      common_size_type_t<std::decay_t<E1>, std::decay_t<E2>, std::decay_t<E3>>;
  ...
  if (equal_bins) {
    std::array<typename std::decay_t<E2>::value_type, 2> bounds =
        xt::minmax(bin_edges)();
    ...
    for (size_t i = 0; i < data.size(); ++i) {
      ...
      if (v >= left && v < right) {
        auto i_bin = static_cast<size_t>(static_cast<double>(n_bins) *
                                         (v - left) * norm);
 ...
 * @param density If true the resulting integral is normalized to 1. [default:
 * false]
...
inline auto histogram(E1&& data, E2&& bin_edges, E3&& weights,
  ...
  return detail::histogram_imp<R>(std::forward<E1>(data),
                                  ...
                                  std::forward<E3>(weights), density, false);
}
...
inline auto histogram(E1&& data, E2&& bin_edges, bool density = false) {
  ...
  return detail::histogram_imp<R>(std::forward<E1>(data),
                                  std::forward<E2>(bin_edges),
                                  xt::ones<value_type>({n}), density, false);
}
...
inline auto histogram(E1&& data, std::size_t bins = 10, bool density = false) {
  ...
  return detail::histogram_imp<R>(
      ...
      xt::ones<value_type>({n}), density, true);
}
 ...
 */
template <class R = double, class E1, class E2>
inline auto histogram(E1&& data, std::size_t bins, E2 left, E2 right,
                      bool density = false) {
  using value_type = typename std::decay_t<E1>::value_type;
...
inline auto histogram(E1&& data, std::size_t bins, E2&& weights,
  ...
  return detail::histogram_imp<R>(std::forward<E1>(data),
                                  ...
                                  std::forward<E2>(weights), density, true);
}
 ...
 */
template <class R = double, class E1, class E2, class E3>
inline auto histogram(E1&& data, std::size_t bins, E2&& weights, E3 left,
                      E3 right, bool density = false) {
  return detail::histogram_imp<R>(
      std::forward<E1>(data),
...
/**
 * @ingroup histogram
 * @brief Compute the bin-edges of a histogram of a set of data using different
 * algorithms.
 *
 * @param data The data.
 * @param weights Weight factors corresponding to each data-point.
...
inline auto histogram_bin_edges(
    E1&& data, E2&& weights, E3 left, E3 right, std::size_t bins = 10,
  ...
  switch (mode) {
    ...
    case histogram_algorithm::logspace: {
      using rhs_value_type =
          std::conditional_t<xtl::is_integral<value_type>::value, double,
                             value_type>;

      xtensor<value_type, 1> bin_edges =
          xt::cast<value_type>(xt::logspace<rhs_value_type>(
    ...
    case histogram_algorithm::uniform: {
      ...
      xt::xtensor<value_type, 1> bin_edges =
          xtensor<value_type, 1>::from_shape(shape);
      // - first/last edge
      bin_edges[0] = left;
...
template <class E1, class E2>
inline auto histogram_bin_edges(
    E1&& data, E2&& weights, std::size_t bins = 10,
    histogram_algorithm mode = histogram_algorithm::automatic) {
  using value_type = typename std::decay_t<E1>::value_type;
...
template <class E1, class E2>
inline auto histogram_bin_edges(
    E1&& data, E2 left, E2 right, std::size_t bins = 10,
    histogram_algorithm mode = histogram_algorithm::automatic) {
  using value_type = typename std::decay_t<E1>::value_type;
 ...
 * @param weights fraction of items per bin: a 1D container whose size is the
 * number of bins
 *
...
inline xt::xtensor<size_t, 1> bin_items(size_t N, E&& weights) {
  ...
  xt::xtensor<double, 1> P =
      xt::cast<double>(weights) / static_cast<double>(xt::sum(weights)());
...
/**
 * Get the number of items in each bin, with each bin having approximately the
 * same number of items in it,under the constraint that the total number of
 * items of all bins is exactly "N".
 *
 * @param N the number of items to distribute
 * @param bins the number of bins
 *
