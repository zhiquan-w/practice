...
template <class P>
void compute_1d_row(std::stringstream& out, P& printer,
                    const std::size_t& row_idx) {
  out << "<tr><td style='font-family:monospace;' title='" << row_idx
      << "'><pre>";
...
template <class P>
void compute_2d_element(std::stringstream& out, P& printer,
                        const std::string& idx_str, const std::size_t& row_idx,
...
void compute_2d_row(std::stringstream& out, P& printer, const T& expr,
  ...
  if (edgeitems == 0 || 2 * edgeitems >= dim) {
    ...
    for (std::size_t column_idx = dim - edgeitems; column_idx < dim;
         ++column_idx) {
      compute_2d_element(out, printer, idx_str, row_idx, column_idx);
    }
  }
...
nlohmann::json mime_bundle_repr_impl(const E& expr) {
  ...
  if (size >
      static_cast<std::size_t>(print_options::print_options().threshold)) {
    edgeitems =
        static_cast<std::size_t>(print_options::print_options().edge_items);
  }
