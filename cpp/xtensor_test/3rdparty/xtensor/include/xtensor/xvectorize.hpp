...
class xvectorizer {
 public:
  ...
  template <class Func, class = std::enable_if_t<!std::is_same<
                            std::decay_t<Func>, xvectorizer>::value>>
  xvectorizer(Func&& f);
...
auto vectorize(F&& f)
    -> decltype(vectorize(std::forward<F>(f),
                          std::declval<detail::get_function_type<F>*>()));
#endif
...
inline auto vectorize(F&& f)
    -> decltype(vectorize(std::forward<F>(f),
                          std::declval<detail::get_function_type<F>*>())) {
  return vectorize(std::forward<F>(f),
                   static_cast<detail::get_function_type<F>*>(nullptr));
}
}  // namespace xt
