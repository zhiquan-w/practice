...
struct xchunk_iterator_array {
  ...
  inline decltype(auto) get_chunk(A& arr, typename A::size_type i,
                                  const xstrided_slice_vector&) const {
...
struct xchunk_iterator_view {
  using reference = decltype(xt::strided_view(
      std::declval<V>().expression(), std::declval<xstrided_slice_vector>()));

  inline auto get_chunk(V& view, typename V::size_type,
                        const xstrided_slice_vector& sv) const {
...
struct xchunk_iterator_base
    : std::conditional_t<
          is_xchunked_array<std::decay_t<T>>::value, xchunk_iterator_array<T>,
          std::conditional_t<is_xchunked_view<std::decay_t<T>>::value,
                             xchunk_iterator_view<T>, invalid_chunk_iterator>> {
};
}  // namespace detail
...
class xchunk_iterator : private detail::xchunk_iterator_base<E> {
 public:
  ...
  xchunk_iterator(E& chunked_expression, shape_type&& chunk_index,
...
template <class E, class DST>
inline void xchunked_assigner<T, CS>::build_and_assign_temporary(
    const xexpression<E>& e, DST& dst) {
...
template <class D>
template <class E>
inline auto xchunked_semantic<D>::assign_xexpression(const xexpression<E>& e)
    -> derived_type& {
...
template <class D>
template <class E>
inline auto xchunked_semantic<D>::computed_assign(const xexpression<E>& e)
    -> derived_type& {
...
template <class D>
template <class E, class F>
inline auto xchunked_semantic<D>::scalar_computed_assign(const E& e, F&& f)
    -> derived_type& {
...
template <class D>
template <class E>
inline auto xchunked_semantic<D>::operator=(const xexpression<E>& e)
    -> derived_type& {
  D& d = this->derived_cast();
...
template <class D>
template <class CS>
inline auto xchunked_semantic<D>::get_assigner(const CS&) const
    -> xchunked_assigner<temporary_type, CS> {
...
template <class E>
inline xchunk_iterator<E>::xchunk_iterator(E& expression,
...
inline decltype(auto) xchunk_iterator<E>::operator*() const {
  return base_type::get_chunk(*p_chunked_expression, m_chunk_linear_index,
                              m_slice_vector);
}
...
template <class E>
inline auto xchunk_iterator<E>::get_slice_vector() const
    -> const slice_vector& {
...
inline auto xchunk_iterator<E>::get_chunk_slice_vector() const -> slice_vector {
  ...
  for (size_type i = 0; i < m_chunk_index.size(); ++i) {
    ...
    size_type end = std::min(chunk_shape, p_chunked_expression->shape()[i] -
                                              m_chunk_index[i] * chunk_shape);
...
inline void xchunk_iterator<E>::fill_slice_vector(size_type i) {
  size_type range_start =
      m_chunk_index[i] * p_chunked_expression->chunk_shape()[i];
  size_type range_end =
      std::min((m_chunk_index[i] + 1) * p_chunked_expression->chunk_shape()[i],
