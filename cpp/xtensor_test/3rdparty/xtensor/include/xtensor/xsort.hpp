...
inline void call_over_leading_axis(E& ev, F&& fct) {
  ...
  if (ev.layout() == layout_type::row_major) {
    ...
    n_iters = std::accumulate(ev.shape().begin() + 1, ev.shape().end(),
                              std::size_t(1), std::multiplies<>());
    secondary_stride =
        adjust_secondary_stride(ev.strides()[1], *(ev.shape().begin()));
  }
...
get_permutations(std::size_t dim, std::size_t ax, layout_type layout) {
  ...
  for (std::size_t i = 0; i < dim; ++i) {
    ...
    reverse_permutation.push_back(
        std::size_t(std::distance(permutation.begin(), it)));
  }
...
inline auto run_lambda_over_axis(const E& e, R& res, std::size_t axis,
                                 F&& lambda) {
...
struct linear_argsort_result_type {
  using type =
      typename flatten_rebind_value_type<typename T::temporary_type::size_type,
                                         typename T::temporary_type>::type;
};
...
template <class E,
          class R = typename detail::linear_argsort_result_type<E>::type>
inline auto flatten_argsort_impl(const xexpression<E>& e) {
  ...
  auto ad =
      xiterator_adaptor<const_iterator, const_iterator>(cit, cit, de.size());
 ...
 *
 * The optional third parameter can either be an axis or ``xnone()`` in which
 * case the xexpression will be flattened.
 *
 * This function uses ``std::nth_element`` internally.
 *
 * \code{cpp}
 * xt::xarray<float> a = {1, 10, -10, 123};
 * std::cout << xt::partition(a, 0) << std::endl; // {-10, 1, 123, 10} the
 * correct entry at index 0 std::cout << xt::partition(a, 3) << std::endl; //
 * {1, 10, -10, 123} the correct entry at index 3 std::cout << xt::partition(a,
 * {0, 3}) << std::endl; // {-10, 1, 10, 123} the correct entries at index 0 and
 * 3 \endcode
 *
 * @param e input xexpression
 * @param kth_container a container of ``indices`` that should contain the
 * correctly sorted value
 * @param axis either integer (default = -1) to sort along last axis or
 * ``xnone()`` to flatten before sorting
 *
...
inline R partition(const xexpression<E>& e, const C& kth_container,
                   placeholders::xtuph /*ax*/) {
  const auto& de = e.derived_cast();
  ...
  std::copy(de.storage_cbegin(), de.storage_cend(),
            ev.storage_begin());  // flatten
  std::size_t k_last = kth_copy.back();
  std::nth_element(ev.storage_begin(), ev.storage_begin() + k_last,
                   ev.storage_end());

  for (auto it = (kth_copy.rbegin() + 1); it != kth_copy.rend(); ++it) {
    std::nth_element(ev.storage_begin(), ev.storage_begin() + *it,
...
inline R partition(const xexpression<E>& e, const I (&kth_container)[N],
                   placeholders::xtuph tag) {
  return partition(
      e,
      xtl::forward_sequence<std::array<std::size_t, N>,
                            decltype(kth_container)>(kth_container),
      tag);
}

template <class E, class R = detail::flatten_sort_result_type_t<E>>
inline R partition(const xexpression<E>& e, std::size_t kth,
                   placeholders::xtuph tag) {
...
template <class E, class C,
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline auto partition(const xexpression<E>& e, const C& kth_container,
...
inline auto partition(const xexpression<E>& e, const T (&kth_container)[N],
                      std::ptrdiff_t axis = -1) {
  return partition(
      e,
      xtl::forward_sequence<std::array<std::size_t, N>,
...
template <class E>
inline auto partition(const xexpression<E>& e, std::size_t kth,
                      std::ptrdiff_t axis = -1) {
 ...
 *
 * Argpartition shuffles the indices to a xexpression in a way so that the index
 * for the kth element in the returned xexpression is in the place it would
 * appear in a sorted array and all elements smaller than this entry are placed
 * (unsorted) before.
 *
 * The optional third parameter can either be an axis or ``xnone()`` in which
 * case the xexpression will be flattened.
 ...
 * \code{cpp}
 * xt::xarray<float> a = {1, 10, -10, 123};
 * std::cout << xt::argpartition(a, 0) << std::endl; // {2, 0, 3, 1} the correct
 * entry at index 0 std::cout << xt::argpartition(a, 3) << std::endl; // {0, 1,
 * 2, 3} the correct entry at index 3 std::cout << xt::argpartition(a, {0, 3})
 * << std::endl; // {2, 0, 1, 3} the correct entries at index 0 and 3 \endcode
 *
 * @param e input xexpression
 * @param kth_container a container of ``indices`` that should contain the
 * correctly sorted value
...
template <class E, class C,
          ...
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline R argpartition(const xexpression<E>& e, const C& kth_container,
                      placeholders::xtuph) {
  using eval_type = typename detail::sort_eval_type<E>::type;
  using result_type =
  ...
  std::size_t k_last = kth_copy.back();
  std::nth_element(ev.storage_begin(), ev.storage_begin() + k_last,
                   ev.storage_end(), arg_lambda);

  for (auto it = (kth_copy.rbegin() + 1); it != kth_copy.rend(); ++it) {
    std::nth_element(ev.storage_begin(), ev.storage_begin() + *it,
                     ev.storage_begin() + k_last, arg_lambda);
    k_last = *it;
  }
...
template <class E, class I, std::size_t N>
inline auto argpartition(const xexpression<E>& e, const I (&kth_container)[N],
                         placeholders::xtuph tag) {
  return argpartition(
      e,
      xtl::forward_sequence<std::array<std::size_t, N>,
...
template <class E>
inline auto argpartition(const xexpression<E>& e, std::size_t kth,
                         placeholders::xtuph tag) {
...
namespace detail {
template <class Ed, class Ei>
inline void argpartition_over_leading_axis(const Ed& data, Ei& inds,
                                           std::size_t kth,
                                           std::ptrdiff_t last) {
  std::size_t n_iters = 1;
  std::ptrdiff_t data_secondary_stride, inds_secondary_stride;
  ...
  if (last == -1)  // initialize
  {
    for (std::size_t i = 0; i < n_iters; ++i, ptr += data_secondary_stride,
                     indices_ptr += inds_secondary_stride) {
      std::iota(indices_ptr, indices_ptr + inds_secondary_stride, 0);
      std::nth_element(indices_ptr, indices_ptr + kth,
                       indices_ptr + inds_secondary_stride, comp);
    }
  } else {
    for (std::size_t i = 0; i < n_iters; ++i, ptr += data_secondary_stride,
                     indices_ptr += inds_secondary_stride) {
      std::nth_element(indices_ptr, indices_ptr + kth, indices_ptr + last,
                       comp);
    }
  }
}
...
template <class E, class C,
          class = std::enable_if_t<!xtl::is_integral<C>::value, int>>
inline auto argpartition(const xexpression<E>& e, const C& kth_container,
                         std::ptrdiff_t axis = -1) {
  using eval_type = typename detail::sort_eval_type<E>::type;
  using result_type = typename detail::argsort_result_type<eval_type>::type;
  ...
  for (auto it = (kth_copy.rbegin() + 1); it != kth_copy.rend(); ++it) {
    detail::argpartition_over_leading_axis(ev, res, *it,
                                           static_cast<std::ptrdiff_t>(kth));
    kth = *it;
  }
 ...
 *             If not set, computes the median along a flattened version of the
 * input.
 * @param e input xexpression
 * @return median value
 */
...
inline auto median(E&& e, std::ptrdiff_t axis) {
  ...
  if (sz % 2 == 0) {
    ...
    std::array<std::size_t, 2> kth = {szh - 1, szh};
    auto values =
        xt::partition(std::forward<E>(e), kth, static_cast<ptrdiff_t>(ax));
...
inline xtensor<std::size_t, 0> arg_func_impl(const E& e, F&& f) {
  return cmp_idx(e.template begin<L>(), e.template end<L>(), 1,
                 std::forward<F>(f));
}
...
inline typename argfunc_result_type<E>::type arg_func_impl(const E& e,
  ...
  std::copy(e.shape().cbegin(), e.shape().cbegin() + std::ptrdiff_t(axis),
            alt_shape.begin());
  std::copy(e.shape().cbegin() + std::ptrdiff_t(axis) + 1, e.shape().cend(),
...
/**
 * Find the set difference of two xexpressions. This returns a flattened xtensor
 * with the sorted, unique values in ar1 that are not in ar2.
 *
...
inline auto setdiff1d(const xexpression<E1>& ar1, const xexpression<E2>& ar2) {
  ...
  auto end = std::set_difference(unique1.begin(), unique1.end(),
                                 unique2.begin(), unique2.end(), tmp.begin());
