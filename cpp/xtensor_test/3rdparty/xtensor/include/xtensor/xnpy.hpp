...
template <class O>
inline void write_magic(O& ostream, unsigned char v_major = 1,
...
inline void write_header(O& out, const std::string& descr, bool fortran_order,
                         const S& shape) {
  ...
  ss_shape << "(";
  for (auto shape_it = std::begin(shape); shape_it != std::end(shape);
       ++shape_it) {
    ss_shape << *shape_it << ", ";
  }
...
struct npy_file {
  ...
  auto cast_impl(bool check_type) {
    ...
    if (check_type && m_typestring != detail::build_typestring<T>()) {
      XTENSOR_THROW(std::runtime_error, "Cast error: formats not matching "s +
                                            m_typestring + " vs "s +
  ...
  auto cast(bool check_type = true) && {
    ...
    return adapt(std::move(std::get<0>(cast_elems)), std::get<1>(cast_elems),
                 acquire_ownership(), std::get<2>(cast_elems),
  ...
  auto cast(bool check_type = true) const& {
    ...
    return adapt(std::get<0>(cast_elems), std::get<1>(cast_elems),
                 no_ownership(), std::get<2>(cast_elems),
                 std::get<3>(cast_elems));
  }
  ...
  char* ptr() { return m_buffer; }

  std::size_t n_bytes() { return m_n_bytes; }
