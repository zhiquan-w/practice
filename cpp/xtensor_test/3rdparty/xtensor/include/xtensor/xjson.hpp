...
inline void json_shape(const nlohmann::basic_json<M>& j, S& s,
                       std::size_t pos = 0) {
  if (j.is_array()) {
    auto size = j.size();
    s[pos] = size;
 ...
 */
template <template <typename U, typename V, typename... Args> class M, class E>
inline enable_xcontainer_semantics<E> from_json(
    const nlohmann::basic_json<M>& j, E& e) {
...
/// @cond DOXYGEN_INCLUDE_SFINAE
template <template <typename U, typename V, typename... Args> class M, class E>
inline enable_xview_semantics<E> from_json(const nlohmann::basic_json<M>& j,
                                           E& e) {
