...
class xfunctor_applier_base : private xaccessible<D> {
 public:
  ...
  using extension_base =
      extension::xfunctor_view_base_t<functor_type, undecay_expression>;
  ...
  using const_stepper =
      xfunctor_stepper<const functor_type,
  ...
  using const_broadcast_iterator = xfunctor_iterator<
      functor_type, xiterator<typename xexpression_type::const_stepper, S, L>>;
  ...
  using const_reverse_broadcast_iterator =
      xfunctor_iterator<functor_type,
                        typename xexpression_type::
                            template const_reverse_broadcast_iterator<S, L>>;
  ...
  using const_iterator =
      xfunctor_iterator<const functor_type,
                        typename xexpression_type::const_iterator>;
  ...
  auto data_element(size_type i) -> decltype(std::declval<FCT>()(
      std::declval<undecay_expression>().data_element(i))) {
    return m_functor(m_e.data_element(i));
  }

  template <class FCT = functor_type>
  auto data_element(size_type i) const -> decltype(std::declval<FCT>()(
      std::declval<const undecay_expression>().data_element(i))) {
    return m_functor(m_e.data_element(i));
  }

  template <class FCT = functor_type>
  auto flat(size_type i) -> decltype(std::declval<FCT>()(
      std::declval<undecay_expression>().flat(i))) {
    return m_functor(m_e.flat(i));
  }

  template <class FCT = functor_type>
  auto flat(size_type i) const -> decltype(std::declval<FCT>()(
      std::declval<const undecay_expression>().flat(i))) {
    return m_functor(m_e.flat(i));
  }
  ...
  auto load_simd(size_type i) const
      -> decltype(std::declval<FCT>()
                      .template proxy_simd_load<align, requested_type, N>(
...
struct has_simd_interface<xfunctor_applier_base<D>, T>
    : xtl::conjunction<
          has_simd_type<T>,
          has_simd_interface<
              typename xfunctor_applier_base<D>::xexpression_type>,
...
struct xfunctor_view_temporary_type {
  using type = typename detail::functorview_temporary_type_impl<
      F, typename E::shape_type, E::static_layout>::type;
};
 ...
 *
 * The xfunctor_view class is an expression addressing its elements by applying
 * a functor to the corresponding element of an underlying expression. Unlike
 * e.g. xgenerator, an xfunctor_view is an lvalue. It is used e.g. to access
 * real and imaginary parts of complex expressions.
 *
 * xfunctor_view has a view semantics and can be used on any expression.
 * For a similar feature with a container semantics, one can use \ref
 * xfunctor_adaptor.
 *
 * xfunctor_view is not meant to be used directly, but through helper functions
 * such as \ref real or \ref imag.
 *
...
/**
 * @class xfunctor_adaptor
 * @brief Adapt a container with a functor, forwarding methods such as resize /
 * reshape.
 *
 * xfunctor_adaptor has a container semantics and can only be used with
 * containers. For a similar feature with a view semantics, one can use \ref
 * xfunctor_view.
 *
 * @tparam F the functor type to be applied to the elements of specified
 * expression.
...
class xfunctor_iterator
    : public xtl::xrandom_access_iterator_base<
          xfunctor_iterator<F, IT>, typename std::decay_t<F>::value_type,
          typename std::iterator_traits<IT>::difference_type,
          typename xproxy_inner_types<
              detail::xfunctor_invoker_t<F, IT>>::pointer,
          typename xproxy_inner_types<
              detail::xfunctor_invoker_t<F, IT>>::reference> {
 public:
...
/**
 * Constructs an xfunctor_applier_base expression wrappering the specified \ref
 * xexpression.
 *
 * @param e the underlying expression
 */
template <class D>
inline xfunctor_applier_base<D>::xfunctor_applier_base(
    undecay_expression e) noexcept
...
template <class D>
template <class Func, class E>
inline xfunctor_applier_base<D>::xfunctor_applier_base(Func&& func,
                                                       E&& e) noexcept
 ...
 */
template <class D>
inline auto xfunctor_applier_base<D>::shape() const noexcept
 ...
 */
template <class D>
inline auto xfunctor_applier_base<D>::strides() const noexcept
 ...
 * Indices must be unsigned integers, the number of indices should be equal or
 * greater than the number of dimensions of the expression.
 */
template <class D>
template <class... Args>
 ...
 * Indices must be unsigned integers, the number of indices must be equal to the
 * number of dimensions of the expression, else the behavior is undefined.
 *
...
template <class D>
template <class... Args>
inline auto xfunctor_applier_base<D>::operator()(Args... args) const
    -> const_reference {
  XTENSOR_TRY(check_index(shape(), args...));
  XTENSOR_CHECK_DIMENSION(shape(), args...);
  return m_functor(m_e(args...));
...
template <class D>
template <class... Args>
inline auto xfunctor_applier_base<D>::unchecked(Args... args) const
    -> const_reference {
  return m_functor(m_e.unchecked(args...));
}
...
template <class D>
template <class IT>
inline auto xfunctor_applier_base<D>::element(IT first, IT last) const
    -> const_reference {
  XTENSOR_TRY(check_element_index(shape(), first, last));
  return m_functor(m_e.element(first, last));
}
 ...
 */
template <class D>
inline auto xfunctor_applier_base<D>::expression() noexcept
    -> xexpression_type& {
  return m_e;
}
 ...
 */
template <class D>
inline auto xfunctor_applier_base<D>::expression() const noexcept
...
template <class D>
template <class S>
inline bool xfunctor_applier_base<D>::broadcast_shape(S& shape,
                                                      bool reuse_cache) const {
  return m_e.broadcast_shape(shape, reuse_cache);
}

/**
 * Checks whether the xfunctor_applier_base can be linearly assigned to an
 * expression with the specified strides.
 * @return a boolean indicating whether a linear assign is possible
 */
template <class D>
template <class S>
inline bool xfunctor_applier_base<D>::has_linear_assign(
    const S& strides) const {
 ...
 * @tparam L order used for the traversal. Default value is \c
 * XTENSOR_DEFAULT_TRAVERSAL.
 */
template <class D>
template <layout_type L>
...
inline auto xfunctor_applier_base<D>::cbegin() const noexcept {
  return xfunctor_iterator<const functor_type,
                           decltype(m_e.template cbegin<L>())>(
...
template <class D>
template <class S, layout_type L>
inline auto xfunctor_applier_base<D>::begin(const S& shape) noexcept
...
template <class D>
template <class S, layout_type L>
inline auto xfunctor_applier_base<D>::end(const S& shape) noexcept
    -> broadcast_iterator<S, L> {
...
inline auto xfunctor_applier_base<D>::cbegin(const S& shape) const noexcept
    -> const_broadcast_iterator<S, L> {
  return const_broadcast_iterator<S, L>(m_e.template cbegin<S, L>(shape),
                                        &m_functor);
}
...
inline auto xfunctor_applier_base<D>::cend(const S& shape) const noexcept
    -> const_broadcast_iterator<S, L> {
  return const_broadcast_iterator<S, L>(m_e.template cend<S, L>(shape),
                                        &m_functor);
}
//@}
...
inline auto xfunctor_applier_base<D>::crbegin() const noexcept {
  return xfunctor_iterator<const functor_type,
                           decltype(m_e.template rbegin<L>())>(
      m_e.template rbegin<L>(), &m_functor);
}
...
inline auto xfunctor_applier_base<D>::rbegin(const S& shape) noexcept
  ...
  return reverse_broadcast_iterator<S, L>(m_e.template rbegin<S, L>(shape),
                                          &m_functor);
}
...
inline auto xfunctor_applier_base<D>::rend(const S& shape) noexcept
    -> reverse_broadcast_iterator<S, L> {
  return reverse_broadcast_iterator<S, L>(m_e.template rend<S, L>(shape),
                                          &m_functor);
}
...
template <class D>
inline auto xfunctor_applier_base<D>::storage_begin() noexcept
    -> storage_iterator {
  return storage_iterator(m_e.storage_begin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_end() noexcept
    -> storage_iterator {
  return storage_iterator(m_e.storage_end(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_begin() const noexcept
    -> const_storage_iterator {
  return const_storage_iterator(m_e.storage_begin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_end() const noexcept
    -> const_storage_iterator {
  return const_storage_iterator(m_e.storage_end(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_cbegin() const noexcept
    -> const_storage_iterator {
  return const_storage_iterator(m_e.storage_cbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_cend() const noexcept
    -> const_storage_iterator {
  return const_storage_iterator(m_e.storage_cend(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_rbegin() noexcept
    -> reverse_storage_iterator {
  return reverse_storage_iterator(m_e.storage_rbegin(), &m_functor);
}

template <class D>
inline auto xfunctor_applier_base<D>::storage_rend() noexcept
    -> reverse_storage_iterator {
...
template <class D>
template <class S>
inline auto xfunctor_applier_base<D>::stepper_begin(const S& shape) noexcept
    -> stepper {
...
template <class D>
template <class S>
inline auto xfunctor_applier_base<D>::stepper_end(const S& shape,
...
template <class D>
template <class S>
inline auto xfunctor_applier_base<D>::stepper_begin(
...
template <class F, class CT>
template <class E>
inline auto xfunctor_view<F, CT>::operator=(const xexpression<E>& e)
    -> self_type& {
  bool cond = (e.derived_cast().shape().size() == this->dimension()) &&
              std::equal(this->shape().begin(), this->shape().end(),
...
template <class F, class CT>
template <class E>
inline auto xfunctor_view<F, CT>::operator=(const E& e)
...
template <class F, class CT>
template <class E>
inline auto xfunctor_view<F, CT>::build_functor_view(E&& e) const
    -> rebind_t<E> {
...
template <class F, class CT>
template <class E>
inline auto xfunctor_adaptor<F, CT>::operator=(const xexpression<E>& e)
    -> self_type& {
...
template <class F, class IT>
inline auto xfunctor_iterator<F, IT>::operator+=(difference_type n)
    -> self_type& {
  m_it += n;
  return *this;
}
...
template <class F, class IT>
inline auto xfunctor_iterator<F, IT>::operator-(xfunctor_iterator rhs) const
    -> difference_type {
...
template <class F, class IT>
auto xfunctor_iterator<F, IT>::less_than(const xfunctor_iterator& rhs) const
    -> bool {
  return m_it < rhs.m_it;
}
