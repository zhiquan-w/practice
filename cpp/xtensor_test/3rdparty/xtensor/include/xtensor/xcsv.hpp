...
using xcsv_tensor =
    xtensor_container<std::vector<T, A>, 2, layout_type::row_major>;

template <class T, class A = std::allocator<T>>
xcsv_tensor<T, A> load_csv(std::istream& stream, const char delimiter = ',',
...
inline std::string lexical_cast(const std::string& cell) {
  ...
  if (first == std::string::npos) return cell;
 ...
 * @param max_rows the number of lines to read after skip_rows lines; the
 * default is to read all the lines. [default: -1]
...
xcsv_tensor<T, A> load_csv(std::istream& stream, const char delimiter,
  ...
  size_type nbrow = 0, nbcol = 0, nhead = 0;
  {
    ...
    while (std::getline(stream, row)) {
      ...
      nbcol = detail::load_csv_row<size_type, T, output_iterator>(
          row_stream, output, cell, delimiter);
      ++nbrow;
    }
  }
...
struct xcsv_config {
  ...
  xcsv_config() : delimiter(','), skip_rows(0), max_rows(-1), comments("#") {}
};

template <class E>
void load_file(std::istream& stream, xexpression<E>& e,
               const xcsv_config& config) {
  e.derived_cast() = load_csv<typename E::value_type>(
      stream, config.delimiter, config.skip_rows, config.max_rows,
      config.comments);
}

template <class E>
void dump_file(std::ostream& stream, const xexpression<E>& e,
               const xcsv_config&) {
  dump_csv(stream, e);
}
}  // namespace xt
