...
template <class CT, class X>
XTENSOR_CONSTEXPR_RETURN auto linear_begin(
    const xbroadcast<CT, X>& c) noexcept {
  return linear_begin(c.expression());
}
...
class xbroadcast : public xsharable_expression<xbroadcast<CT, X>>,
 ...
 public:
  ...
  template <class E, class XCT = CT,
            class = std::enable_if_t<xt::is_xscalar<XCT>::value>>
...
template <class CT, class X>
template <class... Args>
inline auto xbroadcast<CT, X>::unchecked(Args... args) const
    -> const_reference {
 ...
 */
template <class CT, class X>
inline auto xbroadcast<CT, X>::expression() const noexcept
...
template <class CT, class X>
template <class S>
inline bool xbroadcast<CT, X>::has_linear_assign(
...
template <class CT, class X>
template <class S>
inline auto xbroadcast<CT, X>::stepper_end(const S& shape,
