...
template <class T>
inline auto eval(T&& t)
    -> std::enable_if_t<!detail::is_container<std::decay_t<T>>::value,
                        temporary_type_t<T>> {
  return std::forward<T>(t);
}
...
using as_xarray_container_t =
    xarray<typename std::decay_t<E>::value_type, layout_remove_any(L)>;
...
template <class E, layout_type L>
using as_xtensor_container_t =
    xtensor<typename std::decay_t<E>::value_type,
 ...
 * Returning the value returned by this function could lead to a dangling
 * reference.
 *
 * @return The expression when it already provides a data interface with the
 * correct layout, an evaluated xarray or xtensor depending on shape type
 * otherwise.
 *
 * \code{.cpp}
 * xarray<double, layout_type::row_major> a = {1,2,3,4};
 * auto&& b = xt::as_strided(a); // b is a reference to a, no copy!
 * auto&& c = xt::as_strided<layout_type::column_major>(a); // b is
 * xarray<double> with the required layout auto&& a_cast = xt::cast<int>(a); //
 * a_cast is an xexpression auto&& d = xt::as_strided(a_cast); // d is
 * xarray<int>, not an xexpression auto&& e =
 * xt::as_strided<layout_type::column_major>(a_cast); // d is xarray<int> with
 * the required layout \endcode
 */
template <layout_type L = layout_type::any, class E>
