...
class xexpression_assigner_base<xtensor_expression_tag> {
 public:
  template <class E1, class E2>
  static void assign_data(xexpression<E1>& e1, const xexpression<E2>& e2,
                          bool trivial);
};
...
class xexpression_assigner : public xexpression_assigner_base<Tag> {
 public:
  ...
  template <class E1, class E2>
  static void assert_compatible_shape(const xexpression<E1>& e1,
                                      const xexpression<E2>& e2);
...
template <class E1, class E2>
inline void assign_data(xexpression<E1>& e1, const xexpression<E2>& e2,
                        bool trivial) {
...
inline void assign_xexpression(xexpression<E1>& e1, const xexpression<E2>& e2) {
  xtl::mpl::static_if<has_assign_to<E1, E2>::value>(
      [&](auto self) { self(e2).derived_cast().assign_to(e1); }, /*else*/
      [&](auto /*self*/) {
        using tag = xexpression_tag_t<E1, E2>;
...
constexpr bool linear_static_layout() {
  ...
  return compute_layout(
             ...
             select_layout<E2::static_layout,
                           typename E2::shape_type>::value) !=
         layout_type::dynamic;
}
...
inline auto is_linear_assign(const E1& e1, const E2& e2)
  ...
  return (E1::contiguous_layout && E2::contiguous_layout &&
         ...
         (e1.layout() != layout_type::dynamic &&
          e2.has_linear_assign(e1.strides()));
}

template <class E1, class E2>
inline auto is_linear_assign(const E1&, const E2&)
    -> std::enable_if_t<!has_strides<E1>::value, bool> {
  return false;
}
 ...
 * the assignment fails. The reason is that SIMD batches of complex values
 * cannot be implicitly instanciated from batches of scalar values. Making the
 * constructor implicit does not fix the issue since in the end, the assignment
 * is done with vec.store(buffer) where vec is a batch of scalars and buffer an
 * array of complex. SIMD batches of scalars do not provide overloads of store
 * that accept buffer of complex values and that SHOULD NOT CHANGE. Load and
 * store overloads must accept SCALAR BUFFERS ONLY. Therefore, the solution is
 * to explicitly force the instantiation of complex batches in the assignment
 * mechanism. A common situation that triggers this issue is: xt::xarray<double>
 * rhs = { 1, 2, 3 }; xt::xarray<std::complex<double>> lhs = rhs;
 */
template <class T1, class T2>
struct conditional_promote_to_complex {
  static constexpr bool cond =
      xtl::is_gen_complex<T1>::value && !xtl::is_gen_complex<T2>::value;
...
class xassign_traits {
 private:
  ...
  static constexpr bool convertible_types() {
    return std::is_convertible<e2_value_type, e1_value_type>::value &&
           !is_bool_conversion();
  }
  ...
  static constexpr bool simd_size_impl() {
    return xt_simd::simd_traits<T>::size > 1 ||
           (is_bool<T>::value && use_xsimd());
  }
 ...
 public:
  // constexpr methods instead of constexpr data members avoid the need of
  // definitions at namespace scope of these data members (since they are
  // odr-used).

  static constexpr bool simd_assign() {
    return convertible_types() && simd_size() && simd_interface();
  }
  static constexpr bool linear_assign(const E1& e1, const E2& e2,
                                      bool trivial) {
    return trivial && detail::is_linear_assign(e1, e2);
  }
  static constexpr bool strided_assign() {
    return detail::use_strided_loop<E1>::value &&
  ...
  using e2_requested_value_type =
      std::conditional_t<is_bool<e2_value_type>::value,
                         typename E2::bool_load_type, e2_value_type>;
  using requested_value_type =
      detail::conditional_promote_to_complex_t<e1_value_type,
...
inline void xexpression_assigner_base<xtensor_expression_tag>::assign_data(
  ...
  if (linear_assign) {
    if (simd_linear_assign || traits::simd_linear_assign(de1, de2)) {
      ...
      // instantiate this branch even if the runtime condition is false,
      // resulting in compilation error for expressions that do not provide a
      // SIMD interface. simd_assign is true if simd_linear_assign() or
      // simd_linear_assign(de1, de2) is true.
...
template <class Tag>
template <class E1, class E2>
inline void xexpression_assigner<Tag>::assign_xexpression(E1& e1,
                                                          const E2& e2) {
...
template <class Tag>
template <class E1, class E2>
inline void xexpression_assigner<Tag>::computed_assign(
    xexpression<E1>& e1, const xexpression<E2>& e2) {
  using shape_type = typename E1::shape_type;
  ...
  if (dim2 > de1.dimension() ||
      std::lexicographical_compare(shape.begin(), shape.end(),
                                   de1_shape.begin(), de1_shape.end(),
                                   comperator_type())) {
...
template <class Tag>
template <class E1, class E2, class F>
inline void xexpression_assigner<Tag>::scalar_computed_assign(
    xexpression<E1>& e1, const E2& e2, F&& f) {
...
template <class Tag>
template <class E1, class E2>
inline void xexpression_assigner<Tag>::assert_compatible_shape(
...
inline bool xexpression_assigner<Tag>::resize(E1& e1, const E2& e2) {
  // If our RHS is not a xfunction, we know that the RHS is at least potentially
  // trivial We check the strides of the RHS in detail::is_trivial_broadcast to
  // see if they match up! So we can skip a shape copy and a call to
  // broadcast_shape(...)
  e1.resize(e2.shape());
  return true;
}
...
template <class Tag>
template <class E1, class F, class... CT>
inline bool xexpression_assigner<Tag>::resize(E1& e1,
                                              const xfunction<F, CT...>& e2) {
  return xtl::mpl::static_if<
      detail::is_fixed<typename xfunction<F, CT...>::shape_type>::value>(
      [&](auto /*self*/) {
        /*
         * If the shape of the xfunction is statically known, we can compute the
         * broadcast triviality at compile time plus we can resize right away.
         */
        // resize in case LHS is not a fixed size container. If it is, this is a
        // NOP
...
struct has_sign_conversion {
  ...
  static const bool value = xtl::is_signed<argument_type>::value !=
                            xtl::is_signed<result_type>::value;
};
...
inline void stepper_assigner<E1, E2, L>::run() {
  ...
  constexpr bool needs_cast =
      has_assign_conversion<argument_type, result_type>::value;
...
inline void linear_assigner<simd_assign>::run(E1& e1, const E2& e2) {
  ...
  using rhs_align_mode =
      std::conditional_t<is_aligned, inner_aligned_mode, unaligned_mode>;
  using e1_value_type = typename E1::value_type;
  using e2_value_type = typename E2::value_type;
  ...
  constexpr bool needs_cast =
      has_assign_conversion<e1_value_type, e2_value_type>::value;

  size_type align_begin =
      is_aligned ? 0
                 : xt_simd::get_alignment_offset(e1.data(), size, simd_size);
  size_type align_end = align_begin + ((size - align_begin) & ~(simd_size - 1));

  for (size_type i = 0; i < align_begin; ++i) {
    e1.data_element(i) =
        conditional_cast<needs_cast, e1_value_type>(e2.data_element(i));
  }
...
#else
    for (auto i = static_cast<std::ptrdiff_t>(align_begin);
         i < static_cast<std::ptrdiff_t>(align_end);
         i += static_cast<std::ptrdiff_t>(simd_size)) {
      size_type ui = static_cast<size_type>(i);
      e1.template store_simd<lhs_align_mode>(
          ui, e2.template load_simd<rhs_align_mode, value_type>(ui));
    }
#endif
  } else {
    for (size_type i = align_begin; i < align_end; i += simd_size) {
...
inline void linear_assigner<false>::run(E1& e1, const E2& e2) {
  using is_convertible =
      std::is_convertible<typename std::decay_t<E2>::value_type,
                          typename std::decay_t<E1>::value_type>;
  // If the types are not compatible, this function is still instantiated but
  // never called. To avoid compilation problems in effectively unused code
  // trivial_assigner_run_impl is empty in this case.
  run_impl(e1, e2, is_convertible());
}

template <class E1, class E2>
inline void linear_assigner<false>::run_impl(
    E1& e1, const E2& e2, std::true_type /*is_convertible*/) {
...
#if defined(XTENSOR_USE_TBB)
  tbb::parallel_for(std::ptrdiff_t(0), static_cast<std::ptrdiff_t>(n),
                    [&](std::ptrdiff_t i) {
                      *(dst + i) = static_cast<value_type>(*(src + i));
                    });
#elif defined(XTENSOR_USE_OPENMP)
#pragma omp parallel for default(none) shared(src, dst, n)
  for (std::ptrdiff_t i = std::ptrdiff_t(0); i < static_cast<std::ptrdiff_t>(n);
       i++) {
    *(dst + i) = static_cast<value_type>(*(src + i));
  }
#else
...
template <class E1, class E2>
inline void linear_assigner<false>::run_impl(
    E1&, const E2&, std::false_type /*is_convertible*/) {
  XTENSOR_PRECONDITION(
      false, "Internal error: linear_assigner called with unrelated types.");
}
...
struct check_strides_functor {
  ...
  std::enable_if_t<LE == layout_type::row_major, std::size_t> operator()(
      const T& el) {
    auto var = check_strides_overlap<layout_type::row_major>::get(m_strides,
                                                                  el.strides());
    if (var > m_cut) {
      m_cut = var;
    }
  ...
  template <class T, layout_type LE = L>
  std::enable_if_t<LE == layout_type::column_major, std::size_t> operator()(
      const T& el) {
    auto var = check_strides_overlap<layout_type::column_major>::get(
        m_strides, el.strides());
    if (var < m_cut) {
      m_cut = var;
    }
    return m_cut;
...
auto get_loop_sizes(const E1& e1, const E2& e2, bool is_row_major) {
  ...
  if (E1::static_layout == layout_type::row_major || is_row_major) {
    auto csf =
        check_strides_functor<layout_type::row_major, decltype(e1.strides())>(
            e1.strides());
    cut = csf(e2);
  } else if (E1::static_layout == layout_type::column_major || !is_row_major) {
    auto csf = check_strides_functor<layout_type::column_major,
                                     decltype(e1.strides())>(e1.strides());
    cut = csf(e2);
  ...
  std::size_t outer_loop_size = static_cast<std::size_t>(std::accumulate(
      e1.shape().begin(), e1.shape().begin() + static_cast<std::ptrdiff_t>(cut),
      shape_value_type(1), std::multiplies<shape_value_type>{}));
  std::size_t inner_loop_size = static_cast<std::size_t>(std::accumulate(
      e1.shape().begin() + static_cast<std::ptrdiff_t>(cut), e1.shape().end(),
      shape_value_type(1), std::multiplies<shape_value_type>{}));
...
inline void strided_loop_assigner<simd>::run(E1& e1, const E2& e2) {
  ...
  if (E1::static_layout == layout_type::dynamic) {
    ...
    XTENSOR_THROW(std::runtime_error,
                  "Illegal layout set (layout_type::any?).");
  }
  ...
  for (std::size_t ox = 0; ox < outer_loop_size; ++ox) {
    ...
    for (std::size_t i = 0; i < simd_rest; ++i) {
      *(res_stepper) =
          conditional_cast<needs_cast, e1_value_type>(*(fct_stepper));
      res_stepper.step_leading();
      fct_stepper.step_leading();
    }

    is_row_major
        ? strided_assign_detail::idx_tools<layout_type::row_major>::next_idx(
              idx, max_shape)
        : strided_assign_detail::idx_tools<layout_type::column_major>::next_idx(
              idx, max_shape);
